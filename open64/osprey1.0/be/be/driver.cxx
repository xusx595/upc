 ...
 *
 * Module: driver.cxx
 * $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/be/driver.cxx $
 *
 * Revision history:
...
#if defined(_GCC_NO_PRAGMAWEAK) || defined(__CYGWIN__) || (defined (__MACH__) && defined (__APPLE__) )
...
extern "C" {
  ...
  void        Anl_Init(void) { }
  WN_MAP      Anl_Init_Map(MEM_POOL *id_map_pool) { return 0; }
...
load_components (INT argc, char **argv)
{
  ...
  char **phase_argv;
    ...
    if (Compile_Upc && Run_ipl) {
      //if we're running ipl, we must also load wopt to get the symbols defined there
      Run_wopt = 1;
    }
    
    if (Run_cg && !Compile_Upc) {
    ...
    if (Run_wopt || Run_preopt || Run_lno || Run_autopar || Run_ipl ) {
      Get_Phase_Args (PHASE_WOPT, &phase_argc, &phase_argv);
      load_so("wopt" DSOext, WOPT_Path, Show_Progress);
#if defined (__MACH__) && defined (__APPLE__)  && (__GNUC__ == 3)
     
      if(wopt_main_p == NULL) {
	wopt_main_p = dlsym(last_so_loaded, "wopt_main");
	if(wopt_main_p == NULL) {
	  printf("Can't resolve symbol _wopt_main \n");
	  exit(1);
	}
      }

      if(Perform_Global_Optimization_p == NULL) {
	Perform_Global_Optimization_p = dlsym(last_so_loaded, "Perform_Global_Optimization");
	if(Perform_Global_Optimization_p == NULL) {
	  printf("Can't resolve symbol _Perform_Global_Optimization \n");
	  exit(1);
	}
      } 
    ...
    if (Run_lno || Run_autopar) {
      ...
      extern void *last_so_loaded;
      if(lno_main_p == NULL) {
	lno_main_p = dlsym(last_so_loaded, "lno_main");
	if(lno_main_p == NULL) {
	  printf("Can't resolve symbol _lno_main \n");
	  exit(1);
	}
      }
      if(Lno_Init_p == NULL) {
	Lno_Init_p = dlsym(last_so_loaded, "Lno_Init");
	if(Lno_Init_p == NULL) {
	  printf("Can't resolve symbol _Lno_Init \n");
	  exit(1);
	}
      }
     if(Lno_Fini_p == NULL) {
	Lno_Fini_p = dlsym(last_so_loaded, "Lno_Fini");
	if(Lno_Fini_p == NULL) {
	  printf("Can't resolve symbol _Lno_Fini \n");
	  exit(1);
	}
      } 
     if(Perform_Loop_Nest_Optimization_p == NULL) {
	Perform_Loop_Nest_Optimization_p = dlsym(last_so_loaded, "Perform_Loop_Nest_Optimization");
	if(Perform_Loop_Nest_Optimization_p == NULL) {
	  printf("Can't resolve symbol _Perform_Loop_Nest_Optimization \n");
	  exit(1);
	}
      } 
#endif
	...
	(Run_purple && Prp_Needs_Whirl2c()))
    {
      ...
      if(W2C_Process_Command_Line_p == NULL) {
	W2C_Process_Command_Line_p = dlsym(last_so_loaded, "W2C_Process_Command_Line");
	if(W2C_Process_Command_Line_p == NULL) {
	  printf("Can't resolve symbol _W2C_Process_Command_Line \n");
	  exit(1);
	}
      }
#endif
...
Phase_Init (void)
{
	...
	need_wopt_output = TRUE;
      ...
      /* for UPC programs, always use .N file for backend output */
      if (!Run_w2c) {
	need_lno_output = TRUE;
      }
    }

    if (Run_ipl && !Compile_Upc) {
      //again, for UPC always use .N for output
...
Adjust_Opt_Level (PU_Info* current_pu, WN *pu, char *pu_name)
{
    ...
    if ((Opt_Level > 0 || Run_autopar) && PU_Olimit > Olimit && !Olimit_opt && !Run_w2c /* no opt for whirl2c */) {
	if (Show_OPT_Warnings && !Compile_Upc)
...
extern bool No_Opt_for_Func(WN* wn);
...
Backend_Processing (PU_Info *current_pu, WN *pu)
{
	...
	//pu = WN_Lower (pu, LOWER_RETURN_VAL | LOWER_MLDID_MSTID , NULL, "RETURN_VAL & MLDID/MSTID lowering");
	//pu = WN_Lower (pu, LOWER_MLDID_MSTID, NULL, "MLDID/MSTID lowering");
    }
    ...
    if (Compile_Upc && !Run_w2c) {
      //We need to do this lowering before LNO runs,
      //So that the affinity test removal optimization would 
      //not break the semantics of dynamically nested forall loops
    ...
    if (Run_lno || Run_Distr_Array || Run_preopt || Run_autopar) {
      if (!Compile_Upc || Opt_Level > 0) {
	//wei: for upc, turn on lno (and preopt) only when opt level > 0
    ...
    if (Compile_Upc && Opt_Level > 0 && !Run_w2c) {
      Run_wopt = 1;
    }
    ...
    if (Compile_Upc && Run_wopt == 1) {
      //We want to use wopt, but only for its optimizations,
      //not the IA64-specific lowering and cg-related code
      Run_wopt = 0;
    }
    ...
    if (Compile_Upc ) {
      ...
      if (!Run_w2c) {
	need_lno_output = TRUE;
	//Perform the return_val_preg/mldid/mstid lowering after the optimizer,
	//so that UPC pointer-to-shared will still appear as pointers 
	pu = WN_Lower (pu, LOWER_RETURN_VAL | LOWER_MLDID_MSTID, NULL, "RETURN_VAL & MLDID/MSTID lowering");
	if (Opt_Level > 0) {
	  //LNO may produce shared ARRAY wns, and we need to lower them 
	  //into shared ptr arithmetic in the backend
	  pu = WN_Lower (pu, LOWER_ARRAY, NULL, "Array Lowering");	
	}
	pu = WN_Lower (pu, LOWER_UPC_TO_INTR, NULL, "UPC Lowering");	
	pu = WN_Lower (pu, LOWER_UPC_INTRINSIC | LOWER_INTRINSIC , NULL, "UPC Intrinsic lowering");
	//Need another pass of return val/mldid lowering since UPC lowering may introduce comma nodes
        pu = WN_Lower (pu, LOWER_RETURN_VAL | LOWER_MLDID_MSTID | LOWER_UPC_MFIELD, NULL, "UPC RETVAL Lowering");
			/* added by parkcs */
			if (Run_thrille ) {
				pu = WN_Lower (pu, LOWER_UPC_PRIVATE_MEMACC, NULL, "UPC Privatized pointer accesses Lowering");
			}
			/* end parkcs */
      } else {
	pu = WN_Lower (pu, LOWER_RETURN_VAL | LOWER_MLDID_MSTID , NULL, "UPC W2C RETVAL Lowering");
      }
...
Preprocess_PU (PU_Info *current_pu)
{
  ...
  /* I think this is wrong: push should be done once,
     since alias_mgr is shared by all PUs.
     Move it to main() 
  */
...
Postprocess_PU (PU_Info *current_pu)
{
  ...
  /* See comments for MEM_POOL_Push earlier */
...
main (INT argc, char **argv)
{
  ...
  if (Compile_Upc) {
    Run_cg = 0;   
    if (!Run_w2c) {
      Run_lno = 1;  //always fake it to run lno, so cg is not loaded
    }
  }
  ...
  if ((Opt_Level > 0 || Run_autopar) 
      && Max_Src_Olimit > Olimit && !Olimit_opt && Show_OPT_Warnings && !Run_w2c /* no opt for whirl2c */) {
    if (!Compile_Upc) 
