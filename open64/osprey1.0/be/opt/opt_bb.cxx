...
// Module: opt_bb.cxx
// $Revision: 1.5 $
// $Date: 2005/01/28 10:16:07 $
// $Author: wychen $
// $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/opt/opt_bb.cxx $
//
// Revision history:
...
BB_NODE::Add_stmtnode(WN *wn, MEM_POOL *pool)
{
  ...
  stmt->Set_wn(wn);
  stmt->Set_linenum(WN_Get_Linenum(wn));
...
BB_NODE::Branch_stmtrep(void)
{
  ...
  if ( branchstmt != NULL ) {
    if (branchstmt->Opr() == OPR_INTRINSIC_CALL) {
      //don't even ask why...
      return NULL;
    }
