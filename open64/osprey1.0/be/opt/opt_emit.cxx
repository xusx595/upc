...
// $Author: wychen $
// $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/opt/opt_emit.cxx $
//
// Revision history:
...
static char *rcs_id = 	opt_emit_CXX"$Revision: 1.6 $";
#endif /* _KEEP_RCS_ID */
...
Valid_doloop_condition( BB_NODE *bb_end, const OPT_STAB *opt_stab )
{
  ...
  if (found_one_side_use_index == FALSE) {
    DevWarn( "Valid_doloop_condition(BB:%d) can't find index var on comparison",
	    bb_end->Id() );
    //fake this for now, since the vector opt currently generates a do loop like this
    //return FALSE;
  }
...
Raise_doloop_stmt(EMITTER *emitter, BB_NODE **bb)
{
  ...
  if ( ! Valid_doloop_condition( bb_end, emitter->Opt_stab()) ) {
    ...
    //    bb_end->Print(stderr);
    Valid_doloop_condition( bb_end, emitter->Opt_stab());
