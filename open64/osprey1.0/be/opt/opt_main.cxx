...
// $Author: phargrov $
// $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/opt/opt_main.cxx $
//
// ====================================================================
//
...
extern void race_detection (COMP_UNIT* comp_unit);
	      ...
	      ALIAS_MANAGER *alias_mgr)
{
  ...
  if (phase == MAINOPT_PHASE) {
    ...
    //list the UPC specific items here
    if (Compile_Upc) {
	actions = LOWER_UPC_INTRINSIC;
	//disable cvt folding since they could result in compile warning
	//(e.g. by dropping (uintptr_t) casts)
    ...
    if (Only_Unsigned_64_Bit_Ops && ! Delay_U64_Lowering && !Compile_Upc)
  ...
  //fprintf(TFile, "before cfg is constructed\n");
  //fdump_tree(TFile, wn_tree);
  ...
  BOOL lower_fully = (phase == MAINOPT_PHASE) && !Compile_Upc;
  ...
  ALIAS_CLASSIFICATION *ac = new ALIAS_CLASSIFICATION(comp_unit->Opt_stab(),
						      AC_DESTINATION_OPT_STAB,
						      &alias_class_pool);
#if 0
  /* the destructor segfaults for this object for some reason,
   * use a ptr and pay for the mem leak...
   */
  ...
  //Dump_alias_mgr(comp_unit->Alias_mgr(), comp_unit->Input_tree(), stderr);
  ...
  if (Compile_Upc) {
      if(run_ptr_locality && phase == MAINOPT_PHASE) {
	  SET_OPT_PHASE("UPC locality analysis");
	  comp_unit->Opt_stab()->Analyze_Locality(comp_unit->Cfg());
      }
      
    if (phase == MAINOPT_PHASE) {
      SET_OPT_PHASE("UPC Split-phase Optimization");
      comp_unit->Do_UPC_CSE(&Opt_global_pool);
    }
  }
  ...
  for (INT i = 0; i < WOPT_Enable_Extra_Rename_Pass; ++i) {
    ...
    break;  //Is the extra pass really necessary, considering the optimizations are done at least twice?
  ...
  WN *opt_wn;
  if ( phase == MAINOPT_PHASE && !Compile_Upc) {
    ...
    /* FIXME: Probably should do this in WOPT,
       (so we can get global alias information instead of local ones),
       but right now it doesn't work for some reason...
     */
    //race_detection(comp_unit);
	...
	!REGION_has_black_regions(comp_unit->Rid())) {
      ...
      OPT_POOL_Initialize(&alias_class_pool, "Alias classification pool",
			  FALSE, MEM_DUMP_FLAG+20);
      /* work around a nasty segfault in the destructor */
      ALIAS_CLASSIFICATION *ac = new ALIAS_CLASSIFICATION (comp_unit->Opt_stab(),
  ...
  if (phase != MAINOPT_PHASE && Get_Trace(TP_GLOBOPT, 1)) {
    fprintf(TFile, "dump after preopt\n");
