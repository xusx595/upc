...
// $Author: ciancu $
// $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/opt/opt_du.cxx $
//
// Revision history:
...
DU_MANAGER::Du_Add_Use(WN *def, WN *use)
{
  ...
  //  if (uselst != NULL && (UINT) uselst < 0x10) 
  //  fprintf(stderr, "def is %p: map = %d, map_id = %d\n", def, _du_map, WN_map_id(def));
  ...
  if (uselst == NULL) {
    ...
    //    fprintf(stderr, "adding use for %p\n", def);
