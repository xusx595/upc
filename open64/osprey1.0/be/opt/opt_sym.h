...
private:
  ...
  // _default_vsym        default virtual sym for shared variables (0 if none)
  ...
  AUX_ID                    _return_val_preg;  //store the return val preg for fast lookup
  ...
  BOOL     Generate_upc_call_mu_chi(WN* wn, ST* st, MU_LIST *mu, CHI_LIST *chi);
  void     Generate_upc_memcpy_call_mu_chi(WN* addr, MU_LIST *mu, CHI_LIST *chi, READ_WRITE how);
...
public:
  ...
  AUX_ID   Create_temp(TY_IDX ty, const char * name= NULL);
  ...
  //  Default virtual variable for shared variables
  //  we try to treat it the same as default vsym except when computing alias info
  AUX_ID   Default_shared_vsym(void) const     { return _default_shared_vsym; }
  void     Set_default_shared_vsym(AUX_ID vsym){ _default_shared_vsym = vsym; }
  ...
  //UPC specific locality analysis (see opt_upc_locality.cxx)
  void     Analyze_Locality(CFG *cfg);

  void     Analyze_Locality_Block(BB_NODE* bb);

  void     Analyze_Locality_Stmt(CODEREP* pts, CODEREP* rhs);

  void     Analyze_Locality_Cr(CODEREP* cr);

  void     Analyze_Locality_Alloc(CODEREP *pts, ST* func_st);  
  
  void     Analyze_Locality_Ptr_Arith(CODEREP* rhs);  
