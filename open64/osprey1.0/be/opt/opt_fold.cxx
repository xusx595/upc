...
// $Author: wychen $
// $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/opt/opt_fold.cxx $
//
// Revision history:
...
static char *rcs_id = opt_fold_CXX"$Revision: 1.4 $";
#endif /* _KEEP_RCS_ID */
...
FOLD::CR_Simplify_Expr(CODEREP *cr)
{
   ...
   if (opr == OPR_INTRINSIC_OP) {
	...
	//FIXME: may need to do this for other simplifiers 
	if (opr == OPR_TAS) {
	  if (Type_Is_Shared_Ptr(cr->Get_ty(), true) || Type_Is_Shared_Ptr(k0->Get_ty(), true)) {
	    r = NULL;
	  }
	} else {
	  r = SIMPNODE_SimplifyExp1(op, k0);
