...
// $Revision: 1.12 $
// $Date: 2005/10/20 06:42:15 $
// $Author: wychen $
// $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/opt/opt_alias_analysis.cxx $
//
// ====================================================================
//
...
// initializes the POINTS_TO data.  It also generate the occurrence
...
OPT_STAB::Adjust_vsym(AUX_ID vp_idx, OCC_TAB_ENTRY *occ)
{
  ...
  if (vp_idx != Default_vsym() && vp_idx != Default_shared_vsym()) {
  ...
  //ditto for default_shared_vsym
  if (vp_idx == Default_shared_vsym()) {
    pt->Set_default_shared_vsym();
    pt->Set_default_vsym();
  }
...
//For UPC memcpy functions, we can compute a more precise 
//mu and chi list based on the source and dst pointers
//(mu includes everything that could alias with the src pointer,
// chi everything that could alias the dst pointer)
void
OPT_STAB::Generate_upc_memcpy_call_mu_chi(WN* addr, MU_LIST *mu, CHI_LIST *chi, READ_WRITE how) {

    AUX_ID addr_id;
    while (WN_operator(addr) == OPR_TAS) 
	addr = WN_kid0(addr);
    POINTS_TO *addr_pt;
    POINTS_TO ai;
	...
	case OPR_LDA:
	    addr_id = WN_aux(addr);
	    addr_pt = aux_stab[addr_id].Points_to();
	    break;
	case OPR_ILOAD:
	case OPR_MLOAD:
	    addr_id = Get_occ(addr)->Aux_id();
	    addr_pt = aux_stab[addr_id].Points_to();
	    break;
	case OPR_ADD:
	case OPR_SUB: 
	    ...
	    ai.Set_base_kind(BASE_IS_UNKNOWN);
	    ai.Set_ofst_kind(OFST_IS_UNKNOWN);
	    Simplify_Pointer(addr, &ai);
	    addr_pt = &ai;
	    break;
	default:
	    //other possible address wn?
	    FmtAssert(0, ("unknown opr for addr: %d\n", WN_operator(addr)));
    }

    const BS *alias_set = Rule()->Alias_Set_Call_By_Value(this);
    for (AUX_ID idx = BS_Choose( alias_set );
	 idx != (AUX_ID) BS_CHOOSE_FAILURE;
	 idx = BS_Choose_Next ( alias_set, idx )) {
	...
	if (Rule()->Aliased_Memop(addr_pt, pt)) {
	    if (how & WRITE) {
		chi->New_chi_node(idx, Occ_pool());
	    }
	    if (how & READ) {
...
//For UPC library functions that have no side effects, we can provide accurate
//mu and chi information instead of treating it as a black box
BOOL
OPT_STAB::Generate_upc_call_mu_chi(WN *wn, ST *call_st, MU_LIST *mu, CHI_LIST *chi) {

  //find intrinsic calls that could be generated by the front end 
  if (WN_operator(wn) == OPR_INTRINSIC_CALL) {
    if (INTRN_is_pure(WN_intrinsic(wn)) && INTRN_has_no_side_effects(WN_intrinsic(wn))) {
      return true;
    }
    switch (WN_intrinsic(wn)) {
    case INTRN_UPC_MEMGET:
    case INTRN_UPC_MEMPUT:
    case INTRN_UPC_MEMCPY:
      Generate_upc_memcpy_call_mu_chi(WN_kid0(WN_kid0(wn)), mu, chi, WRITE); //dst
      Generate_upc_memcpy_call_mu_chi(WN_kid0(WN_kid1(wn)), mu, chi, READ); //src
      return true;
      default:
	return false;
    }
  } else if (WN_Call_Does_Mem_Alloc(wn)) {
    //alloc calls should not introduce mu/chis
					...
					INT32 num_parms)
{
  ...
  if (Compile_Upc && Generate_upc_call_mu_chi(wn, call_st, mu, chi)) {
    //fprintf(stderr, "alias analysis for upc function\n");
    //add the return val preg to the chi list, if the function returns anything
    if (WN_rtype(wn) != MTYPE_V && Return_val_preg() != -1) {
      chi->New_chi_node(Return_val_preg(), Occ_pool());      
      mu->New_mu_node(Return_val_preg(), Occ_pool());      
    }
    return;
  }
  ...
  for (idx = BS_Choose( alias_set );
    ...
    if (how & WRITE) {
      if (Compile_Upc) {
	ST* st = Aux_stab_entry(idx)->St();
	if (!(st == upc_mythread_st || st == upc_threads_st)) {
	  //MYTHREAD/THREADS can never be modified
	  chi->New_chi_node(idx, Occ_pool());
	}
      }
    }
...
OPT_STAB::Generate_exit_mu(WN *wn)
{
  ...
  FOR_ALL_NODE(idx, aux_stab_iter, Init()) {
    ...
    if (idx == Default_vsym() || idx == Default_shared_vsym()) continue;
