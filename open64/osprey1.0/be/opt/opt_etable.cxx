...
// $Revision: 1.4 $
// $Date: 2005/07/29 00:53:49 $
// $Author: wychen $
// $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/opt/opt_etable.cxx $
//
// ====================================================================
//
...
EXP_OCCURS::Get_temp_cr(EXP_WORKLST *wk, CODEMAP *htable)
{
  ...
  /* check for pointer-to-shared */
  TY_IDX ty = exp->Is_ptr(true);
  if (ty != 0) {
    dtyp = Pointer_Mtype;
  } else {
    /* The backend doesn't like pregs to appear on the rhs
       of a shared store (since its address can't be taken),
       so we always use a temp variable instead of pregs 
       when doing PRE
     */
    ty = exp->Is_ptr(false);
    if (ty == 0) {
      ...
      /* If this pointer results from taking the address of an array,
	 we change the type to be without the array.
	 e.g., use int * instead of int *[...]
      */
      TY_IDX base;
      int i;
      for (base = ty, i = 0; TY_kind(base) == KIND_POINTER; base = TY_pointed(base), i++);
      if (TY_kind(base) == KIND_ARRAY) {
	base = Get_Inner_Array_Type(base);
	for (; i > 0; i--) {
  ...
  if (wk->Preg() == 0) {
	...
	AUX_ID          id = (ty != 0 ) ? htable->Sym()->Create_temp(ty) :  htable->Sym()->Create_preg(dtyp);
  ...
  Set_temp_cr(htable->Add_def(wk->Preg(), wk->Cur_e_version(), NULL, 
			      ...
			      (ty != 0) ? ty : ST_type(MTYPE_To_PREG(dtyp)), 0, TRUE));
  wk->New_e_version();
