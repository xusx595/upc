...
// Module: opt_loop.cxx
// $Revision: 1.4 $
// $Date: 2006/07/19 03:44:35 $
// $Author: ciancu $
// $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/opt/opt_loop.cxx $
//
// Revision history:
...
static char *rcs_id = 	opt_loop_CXX"$Revision: 1.4 $";
#endif /* _KEEP_RCS_ID */
...
BB_LOOP::Invariant_cr_rec( CODEREP *cr ) const
{
  ...
  if (Compile_Upc && Opt_stab() != NULL) {
    if (cr->Kind() == CK_VAR) {
      const char *name = Opt_stab()->Aux_stab_entry(cr->Aux_id())->Base_name();
      if (!strcmp(name, "THREADS") || !strcmp(name, "MYTHREAD")) {
	//THREADS and MYTHREADS are always loop invariant
	return TRUE;
      }
    }    
  }
...
Can_raise_to_doloop(BB_LOOP *loop, BOOL repair, CODEMAP *htable)
{
  ...
  if (Compile_Upc) {
    loop->Set_opt_stab(htable->Opt_stab());
  }
