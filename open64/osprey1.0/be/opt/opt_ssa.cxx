...
// Module: opt_ssa.cxx
// $Revision: 1.5 $
// $Date: 2005/08/17 19:17:08 $
// $Author: wychen $
// $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/opt/opt_ssa.cxx $
//
// Revision history:
...
static char *rcs_id = 	opt_ssa_CXX"$Revision: 1.5 $";
#endif /* _KEEP_RCS_ID */
...
{ 
  ...
  AUX_ID default_shared_vsym = Opt_stab()->Default_shared_vsym();
 
  //  Iterate through the aux symbol table.
  FOR_ALL_NODE(var, opt_stab_iter, Init()) {

    //    if (var == default_vsym) continue;
    if (var == default_vsym || var == default_shared_vsym) continue;
		       ...
		       COPYPROP *copyprop, EXC *exc)
{
  ...
  FOR_ALL_ELEM (wn, stmt_iter, Init(bb->Firststmt(), bb->Laststmt())) {
	...
	if (stmt->Rhs() != NULL && stmt->Rhs()->Has_strict_access()) {
	    //Add forward barrier to simulate a strict read
	    STMTREP* fb = CXX_NEW(STMTREP(OPC_FORWARD_BARRIER), mem_pool);
	    WN* wn = WN_CreateBarrier(true, 0);
	    fb->Set_wn(wn);
	    BOOL proped = false;
	    fb->Set_rhs(htable->Add_expr(wn, opt_stab, fb, &proped, copyprop));
	    fb->Set_pt_list(NULL);
	    bb->Insert_stmtrep_before(fb, stmt);
	}
	if (stmt->Lhs() != NULL && stmt->Lhs()->Has_strict_access()) {
	    //Add backward barrier to simulate a strict write
	    STMTREP* b_b = CXX_NEW(STMTREP(OPC_BACKWARD_BARRIER), mem_pool);
	    WN* wn = WN_CreateBarrier(false, 0);
	    b_b->Set_wn(wn);
	    BOOL proped = false;
	    b_b->Set_rhs(htable->Add_expr(wn, opt_stab, b_b, &proped, copyprop));
	    b_b->Set_pt_list(NULL);
	    bb->Insert_stmtrep_after(b_b, stmt);
      }
    }
