...
// $Revision: 1.22 $
// $Date: 2010/02/19 01:33:29 $
// $Author: phargrov $
// $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/opt/opt_htable.cxx $
//
// ====================================================================
//
...
#include <glob.h>      // Compile_Upc
#include "upc_wn_util.h" //for Enter_Consistency_Info
...
CODEREP::Is_ivar_volatile( void ) const
{
  ...
  /*
  if (TY_is_shared(Ilod_ty()) && Type_is_strict(Ilod_ty())) {
    return TRUE;
  }
  */
...
CODEREP::Copy(const CODEREP &cr)
{
  ...
  if (kind == CK_VAR) {
    ...
    Set_offset(cr.Offset());
    Set_lda_field_id(cr.Lda_field_id());
  }
  else if (kind == CK_OP) {
    Set_opr(cr.Opr());
...
CODEMAP::Hash(CODEREP *cr)
{
  ...
  case CK_LDA: 
    return Hash_lda(cr->Lda_base_st(), (IDTYPE)cr->Offset(), (IDTYPE)cr->Lda_field_id());
  case CK_CONST: 
...
{ 
  ...
  if (exist_cr == NULL) {
    return Append_CR(hash_idx,cr);
     ...
     //wei: Update the void * type to the actual type of the pointer
     if (Compile_Upc && cr->Opr() == OPR_ILOAD) {       
       TY_IDX exist_ty = exist_cr->Ilod_ty();
       if (TY_kind(exist_ty) == KIND_POINTER && TY_kind(TY_pointed(exist_ty)) == KIND_VOID && !TY_is_shared(TY_pointed(exist_ty))) {
	 exist_cr->Set_ilod_ty(cr->Ilod_ty());
       }
     }
			     ...
			     MTYPE to_dsctyp, TY_IDX to_ty, UINT field_id)
{
  ...
  //wei: Do not ever use void * as the default type of a cr, since this causes trouble in whirl2c when generating pointer arithmetic code
  //(this void * could come from whirl node, when a pointer is compared to an int (usually null check), for example)  
  bool is_void_ptr = Compile_Upc && TY_kind(to_ty) == KIND_POINTER && TY_kind(TY_pointed(to_ty)) == KIND_VOID && !TY_is_shared(TY_pointed(to_ty));

  //FIXME: We may need to distinguish between the appearances of the same variable, if each use have 
  //different types, and create a new cr for each (e.g., a pointer var may be typed void * when used in comparison,
  //but uses another type in pointer arithmetic).  Or will the types be preserved through TASes?
  CODEREP * retval = this;
  if ( (Is_flag_set(CF_MADEUP_TYPE) && !is_void_ptr) || Dsctyp() == MTYPE_UNKNOWN ) {
		  ...
		  COPYPROP *copyprop)
{
  ...
  if (OPCODE_is_leaf(op)) {
	...
	retv = Add_unary_node(OPCODE_make_op(OPR_CVT, WN_rtype(wn), 
      ...
      //For UPC, expanding LDA into base and offset causes problem later with offset adjustment. -w
      if (_phase == MAINOPT_PHASE && !Compile_Upc &&
...
//#ifndef linux
#if 0 // Never applicable to source-to-source UPC translation
    ...
    if (oper == OPR_CALL) {
      ST* func_st = WN_st(wn);
      if (ST_keep_name_w2f(func_st)) {
	cr->Set_flag(CF_IS_USER_FUNC);
      } else {
	cr->Reset_flag(CF_IS_USER_FUNC);
      }
    }
	...
	if ((MTYPE_type_class(OPCODE_rtype(op)) & MTYPE_CLASS_INTEGER) != 0) {
	  //Do not allow wopt to rearrange shared pointer arithmetic, since this
	  //1). is not profitable (may result in more runtime calls) and
	  //2). may confuse backend due to missing TAS
	  if (!Type_Is_Shared_Ptr(WN_Get_Ref_TY(wn))) {
...
STMTREP::Enter_rhs(CODEMAP *htable, OPT_STAB *opt_stab, COPYPROP *copyprop, EXC *exc)
{
  ...
  case OPR_PRAGMA:
    if (Compile_Upc) {
      Enter_Consistency_Info(WN_pragma(Wn()));
    }
    break;
...
STMTREP::Enter_lhs(CODEMAP *htable, OPT_STAB *opt_stab, COPYPROP *copyprop)
{
  ...
  case OPR_ISTBITS:
	...
	base_ccr.Set_scale(0);
      ...
      //wei: for struct accesses, we want the ilod_ty (type of the lhs) to be the type of the field 
      //, instead of the type of the struct
      ilod_base_ty = WN_ty(Wn());
      TY_IDX ilod_ty = TY_pointed(ilod_base_ty);
      if (TY_kind(TY_pointed(ilod_base_ty)) == KIND_STRUCT && WN_field_id(Wn()) != 0) {
        ilod_ty = Get_Field_Type(ilod_ty, WN_field_id(Wn()));
      }
...
{ 
 ...
 return_:
  ...
  fprintf(stderr, "cr has no points to!!!!\n");
  Print(0, stderr);
}
...
CODEREP::References_var( AUX_ID var )
{
  switch ( Kind() ) {
    ...
    case CK_IVAR:
      /* can be either load or store, so check both */
      if (Ilod_base() && Ilod_base()->References_var(var) )
	return TRUE;
      if (Istr_base() && Istr_base()->References_var(var) )
	return TRUE;
      if (Opr() == OPR_MLOAD) {
...
/**
 *  Returns the whirl type of the coderep.
 *  We mainly only care about expressions that may be a shared pointer. 
 *
 */

TY_IDX 
CODEREP::Get_ty() const {
  ...
  case CK_OP: {
    ...
    //fall through
  default: 
    return MTYPE_To_TY(Dtyp());
  }
}

/* Check if this cr is a pointer.
   If yes, return that pointer, otherwise return 0
   If check_shared is T, additionally check if it's a pointer-to-shared
 */
TY_IDX
CODEREP::Is_ptr(bool check_shared) {
  ...
  case CK_VAR:
    idx = this->Lod_ty();
    if (check_shared) {
      return Type_Is_Shared_Ptr(idx, true) ? idx : 0;
    } else {
      return TY_kind(idx) == KIND_POINTER ? idx : 0;
    }
  case CK_LDA: 
    idx = this->Lda_ty();   
    if (check_shared) {
  ...
  case CK_OP: {
    ...
    OPERATOR op = this->Opr();
    switch (op) {
    case OPR_ADD:
    case OPR_SUB:
      idx = this->Opnd(0)->Is_ptr(check_shared);
      if (idx != 0) {
	return idx;
      } else {
	return this->Opnd(1)->Is_ptr(check_shared);
      }
    case OPR_TAS:
      idx = this->Ty_index();
      if (check_shared) {
    ...
    case OPR_PARM:
      return this->Opnd(0)->Is_ptr(check_shared);
    default:
      // Do we care about any other cases ?
      return 0;
    }
  }
  }
...
void CODEREP::Set_thread_id(INT16 thread_id) {

  switch (Kind()) {
  case CK_VAR:
  case CK_IVAR:
  case CK_OP:
      if (!Type_Is_Shared_Ptr(Get_ty(), true)) {
	  Print_TY(stderr, Get_ty());
      }
    FmtAssert(Type_Is_Shared_Ptr(Get_ty(), true), ("must be a pointer-to-shared"));
    _thread_id = thread_id;
    break;
  default:
    FmtAssert(FALSE, ("Unexpected CR type")); 
  }

  if (thread_id != POINTS_TO::LOCALITY_TOP) {
    fprintf(stderr, "setting CR(%p)'s thread id to %d\n", this, Thread_id());
    Print(0, stderr);
  }
}

INT16 CODEREP::Thread_id() const {

  return _thread_id;
}

bool CODEREP::Has_thread_id() const {

  if (Kind() == CK_VAR) {
    return Type_Is_Shared_Ptr(Lod_ty(), true);
  }
  if (Kind() == CK_IVAR) {
    return Type_Is_Shared_Ptr(Ilod_ty(), true);
  }
  if (Kind() == CK_OP && Opr() == OPR_ADD) {
      return Type_Is_Shared_Ptr(Get_ty(), true);
  }
