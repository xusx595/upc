...
// $Author: ciancu $
// $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/opt/opt_emit_template.h $
//
// Revision history:
...
extern BOOL WN_has_shared(WN* exp);

extern WN* Strip_TAS(WN* exp);

//Keeps track of the type of the last call, 
//so we can patch the type of the preg_return_val (dropped by the optimizer)
//static TY_IDX preg_ret_type = 0;  
...
Gen_exp_wn(CODEREP *exp, EMITTER *emitter)
{
  ...
  case CK_OP:
	...
	if (WOPT_Enable_Cvt_Folding && !emitter->For_preopt()) {
	  ...
	  if (Compile_Upc && exp->Dsctyp() == MTYPE_UNKNOWN) {
	    //FIXME: I coudln't track down where this comes from...
	    cvt_kind = NOT_AT_ALL;
	  }
	  if (cvt_kind == NEED_CVT) {
	    ...
	    //wei: what's wrong with keeping the CVT, as long as we're genrating C output
	    if (Compile_Upc) {
	      wn = WN_CreateExp1(exp->Op(), opnd);
	    } else {
	      FmtAssert(FALSE, ("Gen_exp_wn: Bad type sequence"));
	    }
	  }
	  break;
	}
    ...
    case OPR_TAS:
      ...
      if (TY_is_shared(WN_ty(wn)) && TY_kind(WN_ty(wn)) == KIND_SCALAR 
	  && !TY_is_shared(WN_ty(WN_kid0(wn)))) {
    ...
    case OPR_SUB:
      //wei: for a SUB between a shared pointer and a integer constant (generated by the optimizer),
      //we convert SUB into ADD, as the backend doesn't seem to catch all cases of subtraction on shared pointers
      if (Compile_Upc && Type_Is_Shared_Ptr(exp->Get_ty(), TRUE)) {
	if (exp->Get_opnd(1)->Kind() == CK_CONST) {
	  WN* kid0 = Gen_exp_wn(exp->Get_opnd(0), emitter);
	  WN* kid1 = Gen_exp_wn(exp->Get_opnd(1), emitter);
	  WN_const_val(kid1) = - (WN_const_val(kid1));
	  wn = WN_Binary(OPR_ADD, exp->Dtyp(), kid0, kid1);
	  break; //fall through for other cases
	}
      }
    default:
      {
  ...
  case CK_IVAR:
    if (exp->Opr() == OPR_MLOAD) {
      ...
      /* WEI: Due to SSAPRE it's possible that at this point,
	 the PARM node has a shared type, but the value 
	 is no longer a shared expression and has been replaced by a tmp variable.
	 In this case use the type of the tmp variable.
	 FIXME: what about functions that expect shared args?
       */
      TY_IDX parm_ty = exp->Ilod_ty();
      if (Type_Is_Shared_Ptr(parm_ty)) {
	CODEREP* kid = exp->Ilod_base();
	if (kid->Kind() == CK_VAR) {
	  ST* ilod_st = emitter->Opt_stab()->Base(kid->Aux_id());
	  if (ST_sym_class(ilod_st) == CLASS_PREG ||
	      ST_is_temp_var(ilod_st)) {
	    parm_ty = ST_type(ilod_st);
	  }
	}
      }
      wn = WN_CreateParm(exp->Dtyp(), wn, parm_ty, exp->Offset());
  ...
  case CK_LDA:
    if (exp->Is_flag_set(CF_LDA_LABEL)) {
      ...
      WN_set_field_id(wn, exp->Lda_field_id());
  ...
  case CK_VAR:
	...
	emitter->Opt_stab()->Aux_stab_entry(exp->Aux_id());
      ...
      ST* st = aux_entry->St();
      if (TY_kind(ST_type(st)) == KIND_STRUCT && field_id == 0) {
	//wei: patch the field id that was lost in the SSA representation here
	if (!Get_Field_By_Offset(ST_type(st), ty_idx, exp->Offset(), field_id)) {
	  fprintf(TFile, "opt_emit warning: Can't find field id of LDID for type(%d) at offset %d\n", ty_idx, exp->Offset());
	  field_id = 0;
	}
	ty_idx = ST_type(st);
      }
	...
	// Do not do this for shared pointers, since we really care about 
	// its type
	if (ty_idx != shared_ptr_idx && ty_idx != pshared_ptr_idx) {
...
#ifdef COMPILE_UPC
  if (exp->Has_thread_id() && exp->Thread_id() != POINTS_TO::LOCALITY_TOP) {
      //fprintf(stderr, "\nthread id for cr %p is %d\n", exp, exp->Thread_id());
      //fdump_tree(stderr, wn);
      //exp->Print(0, stderr);
      emitter->Alias_Mgr()->Enter_locality_info(wn, exp->Thread_id());
  }
#endif
...
//                            emitter->Alias_Mgr());
  ...
  case OPR_DEALLOCA:
	...
	if (opr == OPR_INTRINSIC_CALL) {
	  switch (srep->Rhs()->Intrinsic()) {
	  case INTRN_WAIT_SYNC_MEM:
	    //a placeholder for the sync call, need to fix up the wn_map here
	    sync_handle_t* handle = (sync_handle_t*) WN_MAP_Get(upc_comm_map, srep->Wn());
	    if (handle == NULL) {
	      fprintf(TFile, 
		      "!!!!!!!!!!!!!UPC_CSE warning: handle should not be null in gen_stmt_wn()!!!!!!!!!!\n");
	    } else {
	      WN_MAP_Set(upc_comm_map, rwn, handle);
	    }
	    WN_DELETE_Tree(srep->Wn());
	  }
	}
      }
  ...
  case OPR_STBITS:
	...
	//again, we may need to patch stid here
	if (TY_kind(ty_idx) == KIND_STRUCT && field_id == 0) {
	  if (WN_ty(rhs_wn) != 0) {
	    //wei: patch the field id that was lost in the SSA representation here 
	    if (!Get_Field_By_Offset(ty_idx, WN_ty(rhs_wn), lhs_offset, field_id)) {
	      //fprintf(stderr, "STID: Can't find field id for type(%d) at offset %d\n", ty_idx, lhs_offset);
	      field_id = 0;
	    }
	  }
	}
	...
	sync_handle_t* write_handle = srep->Get_write_handle();
	if (srep->Is_nbi_write()) {
	  WN_MAP32_Set(upc_nbi_map, rwn, 1);
	} else if (write_handle != NULL) {
	  //This is a shared write that we'll generate split-phase communication for	
	  WN_MAP_Set(upc_comm_map, rwn, write_handle);
	}
	if (lhs->Get_handle() != NULL) {
	    //fprintf(stderr, "set load handle %p\n", WN_kid0(rwn));
	    WN_MAP_Set(upc_comm_map, WN_kid0(rwn), lhs->Get_handle());
	    //fprintf(stderr, "handle for wn %p is %p\n", WN_kid0(rwn), WN_MAP_Get(upc_comm_map, WN_kid0(rwn)));
	}
      }
    }
    break;
