...
// Module: opt_htable.h
// $Revision: 1.11 $
// $Date: 2010/05/21 05:27:10 $
// $Author: phargrov $
// $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/opt/opt_htable.h $
//
// Revision history:
...
enum CR_FLAG {
  ...
  CF_IS_USER_FUNC = 0x400, // (UPC) For calls, indicate that the called function is user-defined
...
private:
  ...
  CR_FLAG   flags:11;                // Add one upc-specific flag
...
#define ILLEGAL_BB 0xfffff	     // initialization value for emit_bb
  ...
  union {
...
#ifdef COMPILE_UPC
      sync_handle_t* handle;         // Mapping from the init to the sync call
#endif
      TY_IDX    ty;                  // LOD type  
    } isvar;
    struct {                         // for code kind CK_LDA
      ...
      mUINT16   fieldid;             // wei: need to keep track field id, so whirl2c can work properly
    } islda;
...
#define IVAR_EXTRA_NODE_CNT 3
  ...
  void Init(CODEKIND ck)
	...
	Set_ivar_mu_node(NULL);
      _thread_id = POINTS_TO::LOCALITY_TOP;
    }
...
#define Alloc_stack_cr(cnt) ((CODEREP*)alloca(sizeof(CODEREP)+(cnt)*sizeof(CODEREP *)))
  ...
  void Init_lda(MTYPE wt, IDTYPE st, mINT32 ofst, TY_IDX tt, ST *bas, UINT16 field_id = 0)
    {
      Init(CK_LDA); Set_dtyp(wt);  Set_lda_ty(tt); Set_offset(ofst);
      Set_lda_aux_id(st); Set_lda_base_st(bas);
      u2.islda.fieldid = field_id;
    }
  ...
  void Init_const(MTYPE wt, INT64 v)
	...
	Set_dtyp_const_val(wt, (v << 32) >> 32);
      ...
      if (MTYPE_is_unsigned(wt) && u2.isconst.const_val < 0 
	  && MTYPE_is_signed(_dtyp)) {
	//should not change an unsigned to signed
	Set_dtyp_strictly(wt);
      }
    }
  ...
  void      Set_defstmt(STMTREP *s)   { Is_True(Kind() == CK_VAR,
					...
					u2.isvar.defstmt = s; }
#ifdef COMPILE_UPC
  sync_handle_t*  Get_handle(void) const {
    if (Kind() == CK_VAR) {
      return u2.isvar.handle;
    }
    if (Kind() == CK_IVAR) {
      return u2.isivar.handle;
    }
    FmtAssert(false, ("invalid cr type in %s\n", __func__)); 
  }

  void  Set_handle(sync_handle_t *h) {
    if (Kind() == CK_VAR) {
      u2.isvar.handle = h;
  ...
  void Set_thread_id(INT16 thread_id);
  INT16 Thread_id() const;
  ...
  void      Set_lda_ty(TY_IDX t)      { Is_True(Kind() == CK_LDA,
					...
					u2.islda.ty = t; }
  TY_IDX    Lda_field_id(void) const        { Is_True(Kind() == CK_LDA,
						("CODEREP::Lda_ty, illegal kind"));
                                        return u2.islda.fieldid; }
  void      Set_lda_field_id(UINT field_id)         { Is_True(Kind() == CK_LDA,
							      ("CODEREP::Lda_ty, illegal kind"));
                                                      u2.islda.fieldid = field_id; }
  ...
  void      Set_ilod_ty(TY_IDX ty)    { Is_True(Kind() == CK_IVAR,
					...
					u2.isivar.base[2] = (CODEREP*) CAST(ty); }
  ...
  void      Set_ilod_base_ty(TY_IDX ty){ Is_True(Kind() == CK_IVAR,
					 ...
					 u2.isivar.base[4] = (CODEREP*) CAST(ty); }
  ...
  TY_IDX    Is_ptr(bool check_shared);

  bool      Has_strict_access();

// A convenient wrapper for getting the type of a CODEREP
  TY_IDX    Get_ty() const;
...
private:
  ...
  //wei: must include field id as part of hash
  IDX_32 Hash_lda(ST *st, IDTYPE ofst, IDTYPE field_id)	  { return (ST_st_idx(st)+ofst + field_id)%size; }
...
public:
  ...
  CODEREP    *Hash_Lda(CODEREP *cr)
    ...
    IDX_32 hash_idx = Hash_lda(cr->Lda_base_st(),(IDTYPE)cr->Offset(), (IDTYPE)cr->Lda_field_id());
...
//    MTYPE      _rhs_type;     // rhs type for assignment statements
  ...
  sync_handle_t*  _write_handle;  //UPC specific, to signal that this shared write
                                  //statement has been split-phased.

  bool _is_nbi_write;             //Whether this shared write should use a nbi call
#endif
...
#ifdef COMPILE_UPC
  void      Set_write_handle(sync_handle_t* handle) { _write_handle = handle; }
  sync_handle_t      *Get_write_handle() { return _write_handle; }
  void      Set_nbi_write (void) { _is_nbi_write = true; }
  void      Reset_nbi_write (void) { _is_nbi_write = false; }
  bool      Is_nbi_write (void) { return _is_nbi_write; }
#endif
...
//helper upc functions (defined in opt_upc_cse.cxx)
extern bool Type_is_strict(TY_IDX idx);
