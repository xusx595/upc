...
// Module: opt_prop.cxx
// $Revision: 1.3 $
// $Date: 2004/10/28 22:54:11 $
// $Author: wychen $
// $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/opt/opt_prop.cxx $
//
// Revision history:
		       ...
		       BOOL inside_cse, INT32 *height, BOOL in_array)
{
  ...
  INT32 height0 = 0;
  /* Careful: This can be zero (in the case of function calls*/
  AUX_STAB_ENTRY *aux = (propagating_var != 0) ? Opt_stab()->Aux_stab_entry(propagating_var) : NULL;

  switch (x->Kind()) {
  case CK_LDA: 
    if (Compile_Upc) {
      /* Wei: Do not allow variables to propagate if
	 propagating var has a different type, since the translated
	 code may not type check.
      */
      if (aux != NULL) {
	TY_IDX ty1 = ST_type(aux->St());
	TY_IDX ty2 = x->Lda_ty();
	if (!TY_are_equivalent(ty1, ty2)) {
  ...
  case CK_VAR:
    ...
    // check if it is a physical register
    {
      ...
      if (aux != NULL) {
	TY_IDX ty1 = ST_type(aux->St());
	if (Compile_Upc && !TY_are_equivalent(ty1, ST_type(s))) {
	  return NOT_PROPAGATABLE;
	}
      }
    }
...
COPYPROP::Copy_propagate_stmt(STMTREP *stmt, BB_NODE *bb)
{
  ...
  /*
  if (Compile_Upc && stmt->Opr() == OPR_PRAGMA) {
