...
// $Author: wychen $
// $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/opt/opt_sym.cxx $
//
// ====================================================================
//
...
AUX_ID
OPT_STAB::Create_temp(TY_IDX ty, const char * name) {

  ST* st = Gen_Temp_Symbol(ty, (name) ? name : ".PRE");
  AUX_ID retv = aux_stab.Newidx();
  ...
  sym->Set_stype(VT_NO_LDA_SCALAR);
  sym->Clear_flags();
  sym->Set_mclass(Get_mtype_class(TY_mtype(ty)));
  sym->Set_st(st);
  sym->Set_st_ofst(0);
  sym->Set_nonzerophis(NULL);
...
OPT_STAB::Convert_IO_statement( WN *iown, WN *wn, INT level )
{
  ...
  if ( opr == OPR_IO_ITEM && WN_io_item(wn) == IOC_VARFMT ) {
...
OPT_STAB::Create(COMP_UNIT *cu, REGION_LEVEL rgn_level)
{
  ...
  aux_stab.Alloc_array((UINT32) (aux_sym_cnt*1.2+10));
...
OPT_STAB::Identify_vsym(WN *memop_wn)
{
  ...
  /*
  //FIXME:  this was causing the upc collective suite to fail, disable for now...
  if (Compile_Upc) {
    if (TY_is_shared(WN_object_ty(memop_wn))) {
      if (Default_shared_vsym() == 0) {
	fprintf(stderr, "got here\n");
	Set_default_shared_vsym(Create_vsym(EXPR_IS_ANY));
	Aux_stab_entry(Default_shared_vsym())->Points_to()->Set_default_shared_vsym();
	//we probably should also set it to be default_vsym just to be safe
	Aux_stab_entry(Default_shared_vsym())->Points_to()->Set_default_vsym();
      }
      return Default_shared_vsym();
    }
  }
  */
...
AUX_ID OPT_STAB::Return_val_preg() {

  if (_return_val_preg == -1) {
    AUX_ID i;
    AUX_STAB_ITER aux_stab_iter(this);

    FOR_ALL_NODE(i, aux_stab_iter, Init()) {
      AUX_STAB_ENTRY *var = Aux_stab_entry(i);
      if (var->St() == Return_Val_Preg) {
	_return_val_preg = i;
	break;
      }
    }
  }
  return _return_val_preg;
}
