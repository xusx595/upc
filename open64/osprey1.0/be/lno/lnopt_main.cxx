 ...
 * $Author: ciancu $
 * $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/lno/lnopt_main.cxx $
 *
 * Revision history:
...
//extern "C" WN *Lnoptimizer(PU_Info*, WN *, struct DU_MANAGER *du_mgr, 
			...
			ALIAS_MANAGER *alias_mgr)
{
  ...
  // early exit test
  // Happens when no do_loops are identified in the current pu
  if ( !Du_Built(du_mgr) ) {
  ...
  if(run_msg_vect)
    LNO_Allow_Delinearize = FALSE; // this is true for lno but not
  else                             // for IPL
     LNO_Allow_Delinearize = TRUE;
...

  {
    ...
    if (!Compile_Upc) {
      //for upc we don't want the parallelization optimization
    ...
    //enable fusion/fission only when msg vectorization is enabled
    if (!Compile_Upc || run_msg_vect) {
	...
	if (Get_Trace(TP_LNOPT2, TT_UPC_OPT)) {
	  fprintf(TFile, "======================== UPC Forall Loop Optimization ==================\n");
	}
	Upc_Forall_Opt(func_nd);
      }
      if (run_msg_vect) {
	Upc_Vectorize(func_nd);
        //for the time being the vect code break the loop info
	//stop LNO until this gets fixed
        goto return_point;
      }
      //skip the cg goof
    }
...
return_point:
  ...
  //wei: I believe this is dropping TAS for shared type,
