...
#ifdef _KEEP_RCS_ID
/*REFERENCED*/
static char *rcs_id = "$Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/lno/lego_skew.cxx $ $Revision: 1.6 $";
#endif /* _KEEP_RCS_ID */
...
WN* Lego_Index_From_Access_Vector(ACCESS_VECTOR* av, 
				         WN* wn_index, 
					 DU_MANAGER* du) 
{
  ...
  if (av->Const_Offset != 0) {
    if (wn_new_index == 0) {
      ...
      // LWN_Make_Icon has the params switched 
      wn_new_index = LWN_Make_Icon( WN_rtype(wn_index), av->Const_Offset);
    } else {
