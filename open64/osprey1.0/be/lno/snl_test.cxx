...
*** $Author: phargrov $
*** $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/lno/snl_test.cxx $
**/
                                              ...
                                              BOOL* changed)
{
  ...
  *changed = FALSE;
  ...
  if (smat.Nloops() - i < 2 || sinfo.Body_Deps().All_Stars() ||
    ...
    //The function returns here with failure even for nests that can 
    //be vectorized, e.g
    //   for i
    //     for j
    //         l[a*D1+b*D2*D3...] = r[...]
    //  because of the dependence l to l (when D1,D2  are terms containing variables) .
  ...
  if (outermost >= ni->Depth_Inner()) {
    ...
    if (!Valid_SNL_Region(region))
      DevWarn("Do_Automatic_Transformation: Invalid SNL_REGION [0x%p,0x%p]",
	region.First, region.Last);
    //No transformations applied here. Just run the vectorization.
    /*
    WN *wnn;
    extern WN * SNL_Vectorize_Loops(WN*, INT , SNL_NEST_INFO *, 
				     SNL_ANAL_INFO *, 
				     BOOL *, BOOL );
    if(Get_Do_Loop_Info(wn)->Needs_Vectorization) 
      wnn = SNL_Vectorize_Loops(wn, nloops, ni, &sinfo, changed, TRUE);
    if (!Valid_SNL_Region(region))
      DevWarn("Do_Automatic_Transformation: Invalid SNL_REGION [0x%p,0x%p]",
	      region.First, region.Last);
    */
