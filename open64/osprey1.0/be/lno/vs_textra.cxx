...
*** $Author: wychen $
*** $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/lno/vs_textra.cxx $
**/
...
static char *rcs_id =   vs_textra_CXX "$Revision: 1.3 $";
...
template<> void VECTOR_SPACE<double>::Print_Element(FILE* f, double e)
{
  fprintf(f, "%g", e);
}

template<> void VECTOR_SPACE<FRAC>::Print_Element(FILE* f, FRAC e)
{
  e.Print(f);
}

template<> void VECTOR_SPACE<double>::Reduce_Row(double*, INT)
{
  return;
}

template<> void VECTOR_SPACE<FRAC>::Reduce_Row(FRAC* row, INT elts)
{
  INT j;
