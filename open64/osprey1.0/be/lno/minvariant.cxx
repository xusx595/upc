 ...
 * $Author: phargrov $
 * $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/lno/minvariant.cxx $
 * ====================================================================
 */
...
#ifdef _KEEP_RCS_ID
static char *rcs_id = "$Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/lno/minvariant.cxx $ $Revision: 1.7 $";
#endif
			...
			BOOL 			 messy_only)
{
  ...
  */
  ...
  if (Compile_Upc) {
    ty = WN_Get_Ref_TY(LWN_Get_Parent(p->Wnlist[0]));
    if (TY_is_shared(ty)) {
      //the loaded value should have a private type
      ty = Shared_To_Private_Type(ty);
    }
    pty = Make_Pointer_Type(ty);
    type = TY_mtype(ty);
  }

  //fdump_tree(stderr, LWN_Get_Parent(p->Wnlist[0]));

  sprintf(newname, "$mi%d", unique_mi_num++);
  SYMBOL newsym;
  if (Type_Is_Shared_Ptr(ty, true)) {
    newsym = SYMBOL(Gen_Temp_Symbol(ty, "mi"), 0, TY_mtype(ty));
  } else {
