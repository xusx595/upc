...
class ARA_LOOP_INFO {
  ...
  BOOL              _has_upc_sync;  //loop contains upc_barrier or upc_fence
                                    // or a strict write ??
...
public:
  ...
  BOOL     Has_Upc_Sync() {return _has_upc_sync;}
  BOOL     Set_Upc_Sync() {_has_upc_sync = TRUE;}
  ...
  void Check_Strict_Access(WN* wn);
};
