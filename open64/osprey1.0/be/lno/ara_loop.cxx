...
//check for strict shared loads, whose presence in a loop means it can't be vectorized
void ARA_LOOP_INFO::Check_Strict_Access(WN* wn) {

  if (WN_operator(wn) == OPR_BLOCK) {
    for (WN* stmt = WN_first(wn); stmt != NULL; stmt =WN_next(stmt)) {
      Check_Strict_Access(stmt);
      return;
    }
  }
    
  if (OPCODE_is_load(WN_opcode(wn))) {
    TY_IDX ld_ty = WN_object_ty(wn);
    if (TY_is_shared(ld_ty) && TY_is_strict(ld_ty)) {
      _has_upc_sync = true;
      //fprintf(stderr, "FINDING STRICT:\n");
      return;
    } else if (WN_operator(wn) == OPR_ILOAD) {
      //also check if load_addr_ty is a strict access
      if (Type_Is_Shared_Ptr(WN_load_addr_ty(wn), TRUE) && TY_is_strict(TY_pointed(WN_load_addr_ty(wn)))) {
	_has_upc_sync = true;
	return;
      }
    }
  ...
  //check kids for strict accesses
  for (int i = 0; i < WN_kid_count(wn); i++) {
    Check_Strict_Access(WN_kid(wn, i));
  }
}
  ...
  while (stmt_iter) {
	...
	stmt_iter = LWN_WALK_StmtNext(stmt_iter);
      ...
      continue;
	      ...
	      INTRN_Is_Upc_Sync(WN_intrinsic(stmt))) {
      _has_upc_sync = TRUE;
...
ARA_LOOP_INFO::Walk_Loop()
{
  ...
  if (Compile_Upc) {
    Check_Strict_Access(WN_do_body(_loop));
  }
	...
	for (WN* loop_enc = Enclosing_Loop(wn); 
	  ...
	  if (WN_operator(loop_enc) == OPR_DO_LOOP) {
		...
		WN_kid1(wn) : WN_kid0(wn);
	      //For "void f()" and intrinsics wn_array == 0 
	      //This ugly code tries to work around the fact
	      //that some pointer arithmetic trees for shared arrays
	      //are not in canonical form (don't have the LDA/LDID of
	      //base in the leftmost child of the topmost ADD
	      if(wn_array) {
		if (WN_operator(wn_array) == OPR_PARM) {
		  extern WN* Strip_TAS(WN*, TY_IDX);
		  wn_array = WN_kid0(wn_array);
		  while(WN_operator(wn_array) == OPR_TAS)
		    wn_array = Strip_TAS(wn_array,0);
		} else{
		  switch(WN_operator(wn_array)) {
		  case OPR_LDID:
		    break;
		  case OPR_ADD:
		    if(WN_operator(WN_kid1(wn_array)) == OPR_LDID && TY_kind(WN_ty(WN_kid1(wn_array))) == KIND_POINTER)
		      wn_array = WN_kid1(wn_array);
		    break;
		  default:
		    wn_array = WN_kid0(wn_array);
		  }
		  // if(WN_operator(wn_array) != OPR_LDID ) 
// 		  wn_array = WN_kid0(wn_array);
		}
		while(WN_operator(wn_array) == OPR_ARRAY && WN_operator(WN_array_base(wn_array)))
		  wn_array = WN_array_base(wn_array);
		if(!OPCODE_has_sym(WN_opcode(wn_array)) && !OPCODE_is_leaf(WN_opcode(wn_array)))
		  wn_array = WN_kid0(wn_array);
		if(!OPCODE_has_sym(WN_opcode(wn_array))) {
		  wn_array=0;
		  extern int trace_msg_vect;
		  extern void Print_Vec_Mesg(SRCPOS srcpos, char *reason);
		  if(trace_msg_vect && Get_Do_Loop_Info(ali->Loop())->Needs_Vectorization) 	    
		    Print_Vec_Mesg(WN_Get_Linenum(LWN_Get_Parent(wn)),"Disabling vectorization for nest");
		  Get_Do_Loop_Info(ali->Loop())->Needs_Vectorization = FALSE;
