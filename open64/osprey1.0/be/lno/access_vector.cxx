...
BOOL LNO_Debug_Delinearization = TRUE;
...
static BOOL Sym_Is_Index(STACK<WN* > *ind_vars, ST *sym) 
{
  for(int i = 0; i < ind_vars->Elements(); i++)
    if(WN_st(ind_vars->Bottom_nth(i)) == sym)
      return TRUE;
  return FALSE;
}

WN* ACCESS_VECTOR::Get_Base_WN(STACK<WN* > *ind_vars) const {
  
  WN *result = NULL;
  WN* wn_symb, *term;
  result = WN_Intconst(Integer_type, Const_Offset);
  
  if (Lin_Symb != NULL && !Lin_Symb->Is_Empty()) {
    INTSYMB_CONST_ITER iter(Lin_Symb);
    const INTSYMB_NODE* first = iter.First();
    for (const INTSYMB_NODE *node=first; !iter.Is_Empty(); node = iter.Next()) {
      ST* st = node->Symbol.St();
    
      if(Sym_Is_Index(ind_vars, st))
	continue;

      if (ST_class(st) == CLASS_PREG) {
	//it must be either lower bound or upper bound
	wn_symb =  WN_Ldid(TY_mtype(ST_type(st)), node->Symbol.WN_Offset(), st, ST_type(st), 0);
      } else 
	wn_symb =  WN_Ldid(TY_mtype(ST_type(st)), 0, st, ST_type(st), 0);
      
      if (TY_kind(ST_type(st)) == KIND_SCALAR) {
	wn_symb  = WN_Mpy(Integer_type, wn_symb, WN_Intconst(Integer_type, node->Coeff));
	if(result)
	  result = WN_Add(Integer_type, wn_symb, result);
	else 
	  result = wn_symb;
      } else if (node->Symbol.WN_Offset() != 0) {
	//generate *(&f + offset)
	term  = WN_Lda(Pointer_Mtype, node->Symbol.WN_Offset(), st, 0);
	term = WN_CreateIload(OPR_ILOAD, Integer_type, Pointer_Mtype, 0, MTYPE_To_TY(Integer_type),
			      WN_ty(term), term);
	if(result)
	  result = WN_Add(Integer_type, term, result);
	else
	  result = term;
      } else {
	FmtAssert(0, ("Unimplemented LIN_SYMB access for the access vector"));
	return NULL;
      }
    }
  }
  ...
  if(Non_Lin_Symb != NULL && !Non_Lin_Symb->Is_Empty()) {
    SUMPROD_CONST_ITER iter(Non_Lin_Symb);
    ...
    for(const SUMPROD_NODE* node = first; !iter.Is_Empty(); node = iter.Next()) {
      ...
      BOOL has_idx = FALSE;
      for(int i = 0; i < ind_vars->Elements(); i++) {
	SYMBOL ind_var(ind_vars->Bottom_nth(i));
	...
	for(SYMBOL_NODE* sterm = ft; !term_iter.Is_Empty(); sterm = (SYMBOL_NODE*)term_iter.Next()) {
	  //over here should check if the symbol varies in the loop
	  if(sterm->Is_Loop_Var)
	    {
	      return NULL;
	    }
	  TY_IDX  tidx = ST_type(sterm->Symbol.St());
	  temp = WN_Mpy(Integer_type, temp, WN_Ldid(TY_mtype(tidx), 0, sterm->Symbol.St(), tidx));
	 
	} 
	if(result)
	  result = WN_Add(Integer_type, result,  temp);
	else 
	  result = temp;
      }
    }
...
// Get the base of the  whirl expression that this access vector represents
// (ignoring any loop index variables)
// This is used by the vectorization code to generate the 
// expression for upper and lower bounds
//In here we do not have access to the loop upper/lower bounds so 
//any contribution from the loop induction variable has to
//be added by the callees
WN* ACCESS_VECTOR::Get_Base_WN() const {
  ...
  if(Non_Lin_Symb != NULL && !Non_Lin_Symb->Is_Empty()) {
    FmtAssert(0, ("Unimplemented NON_LIN_SYMB access for the access vector"));
    return NULL;
  }
