 ...
 * $Author: ciancu $
 * $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/lno/can.cxx $
 *
 * Revision history:
...
static char *rcs_id = "$Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/lno/can.cxx $ $Revision: 1.21 $";
...
#include "glob.h"  //for Compile_Upc
		      ...
		      BOOL strict_limit)
{
  ...
  if (opcode == OPC_DO_LOOP) {
    ...
    if (!Array_Dependence_Graph || !Array_Dependence_Graph->Get_Vertex(wn)) {
      for (INT i=0; i<dlistack->Elements()-inside_bound; i++) {
	fprintf(TFile, "has bad mem in can 1\n");
        dlistack->Bottom_nth(i)->Has_Calls = TRUE;
        dlistack->Bottom_nth(i)->Has_Unsummarized_Calls = TRUE;
	...
	for (INT i=0; i<dlistack->Elements()-inside_bound; i++) {
          dlistack->Bottom_nth(i)->Needs_Vectorization = TRUE;
        }
      }
  ...
  if (opcode == OPC_DO_LOOP) {
    ...
    if (!LNO_Ignore_Pragmas && 
	 ((WN_pragma(wn) == WN_PRAGMA_IVDEP) ||
	  ...
	  (WN_pragma(wn) == WN_PRAGMA_UPC_NO_RL_ALIAS))) {
      WN *tmp = WN_next(wn);
	...
	} else	if (WN_pragma(wn) == WN_PRAGMA_UPC_NO_RL_ALIAS) {
          if (!dli->Is_Inner) {
	    dli->No_RL_Alias = TRUE;
	    LWN_Delete_From_Block(LWN_Get_Parent(wn),wn);
	    // ErrMsgSrcpos(EC_LNO_Generic,WN_Get_Linenum(wn),
// 			 "IVDEP on non-inner loop ignored");
	  } else {
...
static bool is_shared_array(WN* wn) {

  wn = Strip_TAS(wn);

  TY_IDX ty = WN_Get_Ref_TY(wn);

  bool res = (WN_operator(wn) == OPR_LDID || WN_operator(wn) == OPR_LDA  
	      /* || WN_operator(wn) == OPR_ILOAD */) &&
    Type_Is_Shared_Ptr(ty, true) && !TY_is_shared(ty);
...
/* shared_ptr arithmetic does not multiply the offset value
   with the element size. The following code finds the pointer and the
   offset for shared_ptr ADD. 
   This is done by doing pattern matching, so we'd need to update
   this function should the front end changes it output for 
   shared array/ptr accesses.
   Return true if we've found a base and offset pair, false otherwise.
*/

static bool Find_ShdPtr_Mult_Base(WN *addr, WN **mult, WN **base, 
				  INT *kid_that_is_mult, INT *kid_that_is_base, TY_IDX tas_idx = 0)
{
  WN *tmp;
  
  //fdump_tree(stderr, addr);
  FmtAssert(WN_operator(addr) == OPR_ADD, (""));
  *base = WN_kid0(addr);
  *mult = WN_kid1(addr);

  if (is_shared_array(*base)) {
    *kid_that_is_base = 0;
    *kid_that_is_mult = 1;
    return true;
  } else if (is_shared_array(*mult)) {
    //kid1 the sharedd pointer, kid0 the offset
    tmp = *mult;
    *mult = *base;
    *base = tmp;
    *kid_that_is_base = 1;
    *kid_that_is_mult = 0;
    if(tas_idx) 
      Is_True(WN_ty(*mult) == tas_idx,(""));
    return true;
  } else if(WN_operator(*base) == OPR_TAS) {
    //wei: I have no idea what this does, Costin?
    //has to be a TAS somewhere
#if 0
    //the ptr arithmetic trees usually have the pointer all the way down on the left
    if(tas_idx == 0)
      tas_idx = WN_ty(*base);
    else 
      Is_True(tas_idx == WN_ty(*base),(""));
    *base = WN_kid0(*base);
    //*base has to be add
    Is_True(WN_operator(*base) == OPR_ADD, (""));
    
    //recombine the adds
    WN_kid1(*base) = WN_Binary(OPR_ADD, WN_rtype(*base), WN_kid1(*base), *mult);
    return Find_ShdPtr_Mult_Base(*base, mult, base, kid_that_is_mult, kid_that_is_base, tas_idx);
#endif
  } else if (WN_operator(WN_kid0(addr)) == OPR_ADD) {
    /* wei: for ar[i+c] where c is a constant, the optimizer associates the additions,
       so we get
       ADD (c, ADD (ar, i)) here. 
       In this case, we try to rearrange the terms back to
       ADD(ar, ADD(i, c)), with kid0 as base and kid1 as mult
    */
    WN* tmp = WN_kid0(WN_kid0(addr));
    if ((OPERATOR_is_load(WN_operator(tmp)) || WN_operator(tmp) == OPR_LDA) && 
	Type_Is_Shared_Ptr(WN_ty(tmp), true)) {
      *base = tmp;
      *mult = WN_Binary(OPR_ADD, WN_rtype(tmp), WN_kid1(WN_kid0(addr)), WN_kid1(addr));
      WN_kid0(addr) = *base;
      WN_kid1(addr) = *mult;
      LWN_Parentize(addr);
      *kid_that_is_base = 0;
  ...
  //fprintf(stderr, "Can't convert into array accesses: \n");
  //fdump_tree(stderr, addr);
  return false;
}
...
static void Promote_Pointer(WN *wn, INT kid_num, INT load_size)
{
  ...
  if (!load_size) {
    if (OPCODE_is_load(opcode) || OPCODE_is_store(opcode)) {
      ...
      if (WN_field_id(wn) != 0) {
	//for field accesses (a[i].x), the size should be the struct,
	//not the field
	if (WN_operator(wn) == OPR_ILOAD) {
	  load_size = TY_adjusted_size(TY_pointed(WN_load_addr_ty(wn)));
	  //fprintf(stderr, "load size is now %d\n", load_size);
	  //fdump_tree(stderr, wn);
	}
      }
    } else {
  ...
  if (OPCODE_is_load(opcode) || OPCODE_is_store(opcode)) {
   if (!Compile_Upc && ((addr_oper == OPR_LDID) || (addr_oper == OPR_LDA))) {
     //wei: why would you ever want to convert scalar load/store into arrays???
  ...
  if (!Compile_Upc && (OPCODE_desc(opcode) == MTYPE_I1 || OPCODE_desc(opcode) == MTYPE_U1)) {
  ...
  //fprintf(stdout, "PP1111 : %d\n", kid_num);
  //fdump_tree(stdout, wn);
  
  BOOL had_TAS = FALSE;
  BOOL had_CVT = FALSE;
  // for UPC shared pointers the addr_oper is TAS 
  if(addr_oper == OPR_TAS && Type_Is_Shared_Ptr(WN_ty(addr))) {
    addr = WN_kid0(addr);
    addr_oper = WN_operator(addr);
    had_TAS = TRUE;
  }

  if(addr_oper == OPR_CVT) {
    addr = WN_kid0(addr);
    addr_oper = WN_operator(addr);
    had_CVT = TRUE;
  }
 ...
 again: 
  if (addr_oper == OPR_ADD) {
    if (WN_operator(kid0) == OPR_MPY) {
      ...
      kid_that_is_mult = 0;
      kid_that_is_base = 1;
    } else if (WN_operator(kid1) == OPR_MPY) {
      mult = kid1;
      base = kid0;
      kid_that_is_mult = 1;
      kid_that_is_base = 0;
    } else if(had_TAS) {
      if (!Find_ShdPtr_Mult_Base(addr,&mult, &base, &kid_that_is_mult, &kid_that_is_base)) {
	//can't promote the shared pointer access
	return;
      }
    } else if (had_CVT) {
      ;
    } else if(WN_operator(kid0) == OPR_CVT || WN_operator(kid1) == OPR_CVT) {
  ...
  //Fix up load size here, for arrays with shared base types
  TY_IDX ety = TY_pointed(WN_Get_Ref_TY(base));
  if (load_size != TY_adjusted_size(ety)) {
    load_size = TY_adjusted_size(ety);
    //fprintf(stderr, "load size is changed to %d\n", load_size);
  }
  ...
  if (Compile_Upc && WN_operator(base) == OPR_LDA) {
    TY_IDX ty = ST_type(WN_st(base));
    if (WN_field_id(base) != 0) {
      ty = Get_Field_Type(ty, WN_field_id(wn));
    }
    if (TY_kind(ty) != KIND_POINTER && TY_kind(ty) != KIND_ARRAY) {
      //base is not either an array or a pointer, and this confuses whirl2c
      return;
    }
  }
  ...
  if(WN_kid_count(mult) == 0 || had_TAS || had_CVT) {
    Is_True(had_TAS || had_CVT,(""));
    index_expr = mult;
    intconst = WN_Intconst(MTYPE_U4,load_size);
  } else if (WN_operator(WN_kid0(mult)) == OPR_INTCONST) {
  ...
  if (base_oper == OPR_ARRAY) { // push the array up
    ...
    //FIXME:  for multid arrays, we should examine the index_expr recursively 
    //to create different dimensions. For now disable the promotion if the base 
    //is a multid array
    if (TY_kind(WN_ty(base)) == KIND_POINTER) {

      int num_dim = 0;
      for (TY_IDX ar = TY_pointed(WN_ty(base)); TY_kind(ar) == KIND_ARRAY; ar = TY_etype(ar)) {
	num_dim ++;
      }
      if (num_dim > 1) {
	fprintf(TFile, "in can.cxx: disable array promotion\n");
	return;
      }
    }
    ...
    //if the array's element size turns out to be larger than the size of the actual element
    //(this happens when the index expression has a constant multiplier, e.g. p[2*i]),
    //we need to normalize it so that array has the correct element size
    TY_IDX ety = TY_pointed(WN_Get_Ref_TY(base));
    
    if (WN_element_size(array) > TY_adjusted_size(ety)) {
      if (WN_element_size(array) % TY_adjusted_size(ety) == 0) {
	//fprintf(stderr, "unfold constant multiplier into index exp\n");
	int factor =  WN_element_size(array) / TY_adjusted_size(ety);
	new_index = WN_Mpy(Integer_type, WN_Intconst(Integer_type, factor), new_index);
	WN_element_size(array) = TY_adjusted_size(ety);
      } else {
	//In UPC we get here for programs that contain structs whose size is adjusted
	// If needed we can reconstruct the arrays based on TY_size/TY_adjusted_Size.
	//For the time being just abort. (see bugzilla/bug518)
	if(TY_kind(ety) == KIND_STRUCT && (WN_element_size(array) % TY_adjusted_size(ety) == 0))
	  return;
	//assert I guess
	Fail_FmtAssertion("inconsistent element size for array node");
      }
    }
