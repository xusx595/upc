...
#ifdef _KEEP_RCS_ID
/*REFERENCED*/
static char *rcs_id = "$Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/lno/ff_utils.cxx $ $Revision: 1.3 $";
#endif /* _KEEP_RCS_ID */
...
extern BOOL scalar_rename(WN* ref, HASH_TABLE<WN*,INT>* checked) {
  ...
  if (can_rename) {
      ...
      //Fix bug1317
      //Keep the type of the original when creating a new tmp symbol.
      //Note that ref is guaranteed to have a ST, since ref_symbol could not be created from
      //ref otherwise
      SYMBOL new_symbol=Compile_Upc ? Create_Stack_Symbol_With_Type(ref_symbol.Name(), WN_Get_Ref_TY(ref)) :
	  Create_Preg_Symbol(ref_symbol.Name(), ref_symbol.Type);
    if (LNO_Verbose) { 
	...
	// while memory does 
      ...
      if (!Compile_Upc) {
	  //Fix bug1317
	  //keep whatever the original type is
	  WN_set_ty(scalar_ref,Be_Type_Tbl(Promote_Type(desc))); 
      }
      WN_offset(scalar_ref)=new_symbol.WN_Offset();
      if (Alias_Mgr) {
