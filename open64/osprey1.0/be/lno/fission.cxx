...
#ifdef _KEEP_RCS_ID
/*REFERENCED*/
static char *rcs_id = "$Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/lno/fission.cxx $ $Revision: 1.4 $";
#endif /* _KEEP_RCS_ID */
...
//FF_STMT_LIST *break_point_list)
{
  ...
  for ( i=1,
	...
	stmt;
	stmt = WN_next(stmt), i++) {
	if ((mUINT32)(uintptr_t)(WN_MAP_Get(dep_graph_map, stmt)) != i)
  ...
  while (begin_stmt) {
    ...
    for (sink_stmt=begin_stmt,
	 sink_v=(mUINT32)(uintptr_t)WN_MAP_Get(dep_graph_map, begin_stmt),
...
WN_MAP dep_graph_map, DYN_ARRAY<FF_STMT_LIST>& loop, MEM_POOL* pool)
{
    ...
    for (WN* stmt = WN_first(WN_do_body(in_loop)); stmt;
      ...
      scc_id = dep_g_p->Get_Scc_Id(
        (mUINT32)(uintptr_t)WN_MAP_Get(dep_graph_map, stmt));
...
DYN_ARRAY<FF_STMT_LIST>& loop, MEM_POOL* pool)
{
  ...
  for (stmt = WN_first(WN_do_body(in_loop));
    ...
    WN_MAP_Set(dep_graph_map, stmt, (void *) CAST(v));
	...
	if (sdg->Level(e) >= in_loop_level-fission_level+1) {
	  ...
	  WN* stmt1 = sdg->Get_Wn(v1);
          ...
          EINDEX16 e1=dep_g_p->Add_Unique_Edge(
            (mUINT32)(uintptr_t)WN_MAP_Get(dep_graph_map, stmt),
            (mUINT32)(uintptr_t)WN_MAP_Get(dep_graph_map, stmt1));
...
WN_MAP loop_map, UINT32 total_loops, FF_STMT_LIST *stl_1, FF_STMT_LIST *stl_2)
{
  ...
  if (no_input_map) {
    ...
    for (wn=WN_first(loop_body); wn; wn=WN_next(wn)) {
      INT loop_id = (INT32)(intptr_t)WN_MAP_Get(loop_map, wn)-1;
