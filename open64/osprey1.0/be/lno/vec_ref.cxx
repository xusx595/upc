...
inline mINT16 VEC_LG::Get_Dim ()     {
  return _myugs->Get_BA()->Get_Dim ();
}
inline mINT16 VEC_LG::Get_Depth ()   {
  return _myugs->Get_BA()->Get_Loop()->Get_Depth();
}
inline LU_FMAT* VEC_LG::Get_Hslu ()   {
  return _myugs->Get_Hslu ();
}
inline VECTOR_SPACE<FRAC>* VEC_LG::Get_KerHs ()   {
  return _myugs->Get_KerHs ();
}
inline VECTOR_SPACE<FRAC>* VEC_LG::Get_KerH  ()   {
  return _myugs->Get_KerH ();
}
inline VEC_LOOPNODE* VEC_LG::Get_Loop () {
  return _myugs->Get_BA()->Get_Loop ();
}
inline WN* VEC_LG::Get_Ref (INT num) {
  return _myugs->Get_Ref (num);
}
// inline mINT16* VEC_LG::Get_Stride () { return _myugs->Get_Stride (); }
inline mINT16  VEC_LG::Get_Stride_One_Loop () {
  return _myugs->Get_Stride_One_Loop ();
}

inline mINT16 VEC_LG::Stride_Forward () {
  return _myugs->Stride_Forward ();
}

inline mINT16  VEC_LG::Get_Stride_One_Size () {
  return _myugs->Get_Stride_One_Size ();
}

inline mINT16  VEC_LG::Get_Stride_In_Enclosing_Loop () {
  return _myugs->Get_Stride_In_Enclosing_Loop ();
}

inline VEC_LOOPNODE* VEC_UGS::Get_Loop ()  {
  return _myba->Get_Loop ();
}
inline mINT16 VEC_UGS::Get_Depth ()       {
  return _myba->Get_Loop()->Get_Depth ();
}
inline mINT16 VEC_BASE_ARRAY::Get_Depth () {
...
//             i, j));
//   global_lvs[i][j] = CXX_NEW (VECTOR_SPACE<FRAC>(i,VEC_mpool,FALSE),
//                               VEC_mpool);
...
//     for (INT j=1; j<=i; j++) {
//       if (global_lvs[i][j])
//         CXX_DELETE (global_lvs[i][j], VEC_mpool);
//     }
// }

VEC_LG::VEC_LG (VEC_LG* lg) : _refvecs(VEC_mpool) {
  INT dim = lg->Get_Dim();
  INT i;

  for (i=0; i<lg->_refvecs.Elements(); i++) {
    _refvecs.Push (CXX_NEW (VEC_REFVEC(lg->_refvecs.Bottom_nth(i)), VEC_mpool));
  }
  _depth = lg->_depth - 1;
  _myugs = lg->_myugs;
  _leading_ref = lg->_leading_ref;
  _c = CXX_NEW_ARRAY (INT64, dim, VEC_mpool);
  for (i=0; i<dim; i++)
    _c[i] = lg->_c[i];
 ...
 *
 ***********************************************************************/
VEC_LG::VEC_LG (WN* ref,
              mINT16 bitpos,
              mINT16 depth,
              VEC_UGS* myugs) : _refvecs(VEC_mpool) {
  ...
  _c = CXX_NEW_ARRAY (INT64, aa->Num_Vec (), VEC_mpool);
  for (i=0; i<aa->Num_Vec(); i++) _c[i] = aa->Dim(i)->Const_Offset;
  _numlines_1L = _numlines_2L = 0;
}

VEC_LG::~VEC_LG () {
  while (_refvecs.Elements()) CXX_DELETE (_refvecs.Pop (), VEC_mpool);
  CXX_DELETE_ARRAY (_c, VEC_mpool);
}
 ...
 *
 ***********************************************************************/
BOOL VEC_LG::Check_Ref (mINT16 refnum) { 
  INT num = _refvecs.Elements();
...
FRAC* VEC_LG::Ref_In_LG (WN* ref) {
  ...
  FRAC *deltac = CXX_NEW_ARRAY (FRAC, indxs, VEC_mpool);
  for (i=0; i<indxs; i++)
    deltac[i] = FRAC (_c[i] - aa->Dim(i)->Const_Offset);

  BOOL soln = Hslu->Particular_Solution (deltac, dvec);
  CXX_DELETE_ARRAY (deltac, VEC_mpool);

  if (!soln) {
    // no solution exists
    CXX_DELETE_ARRAY (dvec, VEC_mpool);
    return NULL;
  }
  ...
  for (i=0; i<_depth; i++) {
    if (dvec[i].N()) {
      ...
      VEC_PRINT(fprintf (TFile, "solution not within desired loop-nest\n"););
      CXX_DELETE_ARRAY (dvec, VEC_mpool);
      return NULL;
    }
  }
  ...
  for (i=_depth; i<maxdepth; i++) {
    ...
    if (dvec[i].N() > 20) {
      ...
      VEC_PRINT(fprintf (TFile, "reuse across more than 20 iters\n"););
      CXX_DELETE_ARRAY (dvec, VEC_mpool);
      return NULL;
    }
  }
  ...
  VEC_LOOPNODE* loopnode = Get_Loop();
 ...
 *
 ***********************************************************************/
INT64 VEC_LG::Distance_LR (WN* ref, FRAC* dvec) {
 ...
 *
 ***********************************************************************/
INT VEC_LG::LR_Compare (mINT16 refvecnum1, mINT16 refvecnum2) {
  INT i;
  mINT16 maxdepth = Get_Depth()+1;
  ...
  FRAC* dvec = CXX_NEW_ARRAY (FRAC, maxdepth+1, VEC_mpool);
  ...
  if (((dvec[nz].N() < 0) && steppos) || ((dvec[nz].N() > 0) && !steppos)) {
    ...
    CXX_DELETE_ARRAY (dvec, VEC_mpool);
    VEC_PRINT (fprintf (TFile, "LR_Compare: ");
              aa2->Print (TFile);
              fprintf (TFile, "     leads  ");
              aa1->Print (TFile));
    return -1;
 ...
 * 
 ***********************************************************************/
BOOL VEC_LG::Add_Ref (WN* ref, mINT16 bitpos) {
  ...
  if (i == maxdepth) {
    ...
    if (aa->Dim(dims-1)->Const_Offset == _c[dims-1]) {
      ...
      if (distance >= 0) {
        ...
        _refvecs.Push (CXX_NEW(VEC_REFVEC(bitpos, maxdepth, dvec, distance),
                               VEC_mpool));
      }
      else {
        ...
        _refvecs.Push (CXX_NEW
                       (VEC_REFVEC(_leading_ref, maxdepth, dvec, -distance),
                        VEC_mpool));
        _leading_ref = bitpos;
      }
 ...
 *
 ***********************************************************************/
BOOL VEC_LG::Add_Group (VEC_LG* lg, WN* lref) {
#ifdef Is_True_On
  if (Debug_Prefetch) {
    Check ();
    lg->Check ();
  ...
  if (((dvec[nz].N() < 0) && steppos) ||
    ...
    for (i=0; i<lg->_refvecs.Elements(); i++) {
      _refvecs.Push (CXX_NEW (VEC_REFVEC(lg->_refvecs.Bottom_nth(i)),
                              VEC_mpool));
    }
    _leading_ref = lg->_leading_ref;
    Is_True (Check (), ("oops - error\n"));
    ...
    Is_True (Check (), ("oops - error\n"));
    _refvecs.Push (CXX_NEW
                   (VEC_REFVEC(lg->_leading_ref, maxdepth, dvec, distance),
                    VEC_mpool));
    Is_True (Check (), ("oops - error\n"));
...
void VEC_LG::Split_LG () {
  ...
  INT64* dist = CXX_NEW_ARRAY (INT64, _refvecs.Elements()+1, VEC_mpool);
  ...
  CXX_DELETE_ARRAY (dist, VEC_mpool);
}
#if 0
/***********************************************************************
 *
 ...
 *
 ***********************************************************************/
VEC_LOOPNODE* Is_Outer_Tile (VEC_LOOPNODE* inner_ln, VEC_LOOPNODE* outer_ln,
                    ACCESS_ARRAY* aa) {
  INT i, j;
  VEC_LOOPNODE* ln;
  WN* outer_wn;
 ...
 *
 ***********************************************************************/
VEC_VOLUME VEC_LG::Volume () {
  VEC_VOLUME myvol (1, 1);       // since vol is a product of terms
  INT i, j;
  ...
  INT num_bvs   = KerHs->D();
  BOOL* done_loop = CXX_NEW_ARRAY (BOOL, num_loops, VEC_mpool);
  ...
  VECTOR_SPACE<FRAC> sKerH  (KerH, VEC_mpool);
  VECTOR_SPACE<FRAC> sKerHs (KerHs, VEC_mpool);

  // Construct a vector space for current loop-nest (depth to num_loops)
  VECTOR_SPACE<FRAC> thisnest (num_loops, VEC_mpool, FALSE);
...
#   endif
      ...
      VEC_LOOPNODE* tmp_loopnode = Get_Loop ();
 ...
 * construct an array of VEC_SORTED_REFS in which the elements
 * occur in sorted order of their distances.
 *
 ***********************************************************************/
static VEC_SORTED_REFS* Sort_Refvecs (VEC_REFVEC_DA* refvecs, mINT16 leadingref){
  // One extra since refvecs doesn't have the leading reference.
  VEC_SORTED_REFS* srefs =
    CXX_NEW_ARRAY (VEC_SORTED_REFS, refvecs->Elements()+1, VEC_mpool);
  srefs[0].dist = 0;
  ...
  for (INT i=0; i<refvecs->Elements(); i++) {
    ...
    VEC_REFVEC* refvec = refvecs->Bottom_nth(i);
    INT j;
    for (j=0; j<(i+1); j++) 
 ...
 *
 ***********************************************************************/
void VEC_LG::LR_Ordering (VEC_SORTED_REFS* srefs, INT start, INT stop) {
  INT i, j;
  ...
  SORT_STR* lr_num = CXX_NEW_ARRAY (SORT_STR, stop-start, VEC_mpool);
  lr_num[0].num = start;
  lr_num[0].eq  = 0;
  ...
  CXX_DELETE_ARRAY (lr_num, VEC_mpool);
}
 ...
 *
 ***********************************************************************/
INT VEC_LG::Get_Bit_Vec (VEC_DESC* pfdesc,
                        VEC_LEVEL level,
                        VEC_SPLIT_VECTOR *split_vec) {
  ...
  VEC_LOOPNODE* loopnode = split_vec->Get_Loop();
...
#ifdef Is_True_On
  {
    VEC_LOOPNODE* leaf = split_vec->Get_Loop ();
    VEC_LOOPNODE* myloop = Get_Loop ();
    while (leaf) {
 ...
 *
 ***********************************************************************/
WN* VEC_LG::Get_Ref_Version (WN* ref, INT bitpos) {
 ...
 *
 ***********************************************************************/
void VEC_LG::Gen_Pref_Node (VEC_SORTED_REFS* srefs, mINT16 start, mINT16 stop,
                           VEC_LEVEL level, VEC_DESC* pfdesc,
                           VEC_SPLIT_VECTOR* split_vec) {
  ...
  while (bitvec) {
    ...
    VEC_LOOPNODE* loopnode = Get_Loop ();
    VEC_LOCLOOP locloop = loopnode->Get_locloop ();
    mINT16 confidence;
      ...
      if (confidence == 3) {
        if (locloop.Loop_1L() == 0) {
          ...
          case 3: 
          {
            ...
            VEC_VOLUME vol = oloop->Get_Total ();
            if (vol.Localized_1L()) {
...
#define MAX_LEN 1024
    ...
    for (j=start; j<stop && !drop_prefetch; j++) {
      ...
      if (tmp == NULL) {
        extern MEM_POOL VEC_CG_mpool;
        tmp = CXX_NEW (PF_POINTER, &VEC_CG_mpool);
 ...
 *
 ***********************************************************************/
void VEC_LG::Gen_Prefetch (VEC_DESC *pfdesc,
                         VEC_SPLIT_VECTOR* split_vec,
                         VEC_LEVEL level) {

  VEC_SORTED_REFS* srefs = Sort_Refvecs (&_refvecs, _leading_ref);
  ...
  CXX_DELETE_ARRAY (srefs, VEC_mpool);
}

void VEC_LG::Print (FILE *fp) {
...
VEC_UGS::VEC_UGS (WN* wn_array, VEC_BASE_ARRAY* myba) : _refs (VEC_mpool) {
  ...
  H  = CXX_NEW (FMAT(_aa->Num_Vec(), num_loops, VEC_mpool), VEC_mpool);
  _Hs = CXX_NEW (FMAT(_aa->Num_Vec()-1, num_loops, VEC_mpool), VEC_mpool);
  ...
  Hlu = CXX_NEW (LU_FMAT(*H, VEC_mpool), VEC_mpool);
  _Hslu = CXX_NEW (LU_FMAT(*_Hs, VEC_mpool), VEC_mpool);
  
  _KerH = CXX_NEW(VECTOR_SPACE<FRAC>(*Hlu, VEC_mpool), VEC_mpool);
  _KerH->Beautify ();
  _KerHs = CXX_NEW(VECTOR_SPACE<FRAC>(*_Hslu, VEC_mpool), VEC_mpool);
  _KerHs->Beautify ();

  CXX_DELETE (H, VEC_mpool); 
  CXX_DELETE (Hlu, VEC_mpool);
...
#   endif
    ...
    _stride = CXX_NEW_ARRAY (mINT16, _KerHs->D(), VEC_mpool);
  ...
  VEC_LOOPNODE* loopnode = myba->Get_Loop();
  ...
  _lg = CXX_NEW_ARRAY (VEC_LG_DA*, num_loops+1, VEC_mpool);
  for (i=0; i<=num_loops; i++) {
    _lg[i] = NULL;
    // allocate lazily
    // CXX_NEW(VEC_LG_DA(VEC_mpool), VEC_mpool);
  }
  _myba = myba;
}
...
VEC_UGS::~VEC_UGS () {
  ...
  for (INT i=0; i<=depth; i++) {
    if (_lg[i]) {
      while (_lg[i]->Elements()) CXX_DELETE (_lg[i]->Pop(), VEC_mpool);
      CXX_DELETE (_lg[i], VEC_mpool);
    }
  }
  CXX_DELETE_ARRAY (_lg, VEC_mpool);
  
  // CXX_DELETE_ARRAY (_stride, VEC_mpool);
  // CXX_DELETE (_H, VEC_mpool);
  // CXX_DELETE (_Hlu, VEC_mpool);
  CXX_DELETE (_Hs, VEC_mpool);
  CXX_DELETE (_Hslu, VEC_mpool);
  CXX_DELETE (_KerH, VEC_mpool);
  CXX_DELETE (_KerHs, VEC_mpool);
}
...
void VEC_UGS::Build_Base_LGs () {
  ...
  _lg[depth] = lglist = CXX_NEW (VEC_LG_DA(VEC_mpool), VEC_mpool);
  for (INT i=0; i<_refs.Elements(); i++) {
    ...
    if (j == lglist->Elements()) {
      ...
      // lglist->Push (CXX_NEW(VEC_LG(tref, i, depth, this), VEC_mpool));
      VEC_LG* tmp_lg = CXX_NEW(VEC_LG(tref, i, depth, this), VEC_mpool);
      lglist->Push (tmp_lg);
    }
  }
}
...
void VEC_UGS::BuildLG (mINT16 depth) {
  if (_lg[depth] == NULL) {
    ...
    for (INT i=0; i<_lg[depth+1]->Elements(); i++) {
      VEC_LG* cur_lg = _lg[depth+1]->Bottom_nth(i);
      INT j;
      ...
      if (j == cur_lglist->Elements()) {
        ...
        cur_lglist->Push (CXX_NEW (VEC_LG(cur_lg), VEC_mpool));
      }
    }
  }
}
 ...
 *
 ***********************************************************************/
VEC_VOLUME VEC_UGS::Volume (mINT16 depth) {

  VEC_PRINT(fprintf (TFile, "Volugs[0x%p] depth (%d)\n",
                    this, depth););
  if (_lg[depth] == NULL) BuildLG (depth);
  VEC_VOLUME myvol (0,0);
  
  for (INT i=0; i<_lg[depth]->Elements(); i++) {
    myvol += _lg[depth]->Bottom_nth(i)->Volume ();
  }
  VEC_PRINT(fprintf (TFile, "Volugs[0x%p] depth (%d) vol ",
                    this, depth);
           myvol.Print (TFile);
  );
  return myvol;
 ...
 *
 ***********************************************************************/
void VEC_UGS::ComputePFVec (VEC_LEVEL level, VEC_LOCLOOP locloop) {
  INT depth = Get_Depth ();
  INT linesize;
  mINT16 loopdepth;
  switch (level) {
  ...
  VECTOR_SPACE<FRAC> sKerH (_KerH, VEC_mpool);
  sKerH  *= lvs;
  ...
  if ((level == level_1) ?
    ...
    VEC_PRINT(fprintf (TFile, "    while temporal locality (no prefetch)\n"));
    return;
  }
  ...
  if (sKerH.D()) {
    ...
    if (not_loopdepth > loopdepth) {
      ...
      VECTOR_SPACE<FRAC> msKerHs (_KerHs, VEC_mpool);
      ...
      mINT16* prefetch_vec = CXX_NEW_ARRAY (mINT16, depth+1, VEC_mpool);
      for (i=0; i<=depth; i++) {
      ...
      VEC_PRINT(fprintf (TFile, "     level %d, spatial locality with vector: ",
                        level);
    ...
    VEC_PRINT(fprintf (TFile, "    temporal locality with basis (no prefetch)\n");
             sKerH.Print (TFile));
    return;
  }
  // Else no temporal locality. Any spatial?
  VECTOR_SPACE<FRAC> sKerHs (_KerHs, VEC_mpool);
  sKerHs *= lvs;
  if (sKerHs.D() == 0) {
    // no spatial locality either
    // Default value of _prefetch_p is already all
    VEC_PRINT(fprintf (TFile, "    no spatial locality (prefetch all)\n"));
    return;
  }

  VEC_PRINT(fprintf (TFile, "    spatial locality, basis is 0x%p\n", this);
           sKerHs.Print (TFile));
 ...
 *
 ***********************************************************************/
VEC_SPLIT_VECTOR* VEC_UGS::Find_Split_Vector () {
  mINT16* prefetch_vec;
  if (((Cache.Levels() > 1) && (prefetch_vec = _pfdesc.Vec(level_2))) ||
    ...
    VEC_SPLIT_VECTOR* split_vec = CXX_NEW (
      VEC_SPLIT_VECTOR(Get_Depth()+1,
                      _pfdesc.Num_Lines (),
                      prefetch_vec,
                      Get_Loop()),
      VEC_mpool);
 ...
 *
 ***********************************************************************/
void VEC_UGS::Find_Loc_Space (VEC_LOCLOOP locloop) {
 ...
 *
 ***********************************************************************/
void VEC_UGS::Gen_Prefetch (VEC_SPLIT_VECTOR* split_vec) {
  if (!_pfdesc.Is_On()) return;

  INT i;
  VEC_LG_DA* curlg;
  VEC_LOCLOOP locloop = Get_Loop()->Get_locloop ();
...
VEC_BASE_ARRAY::~VEC_BASE_ARRAY () {
  CXX_DELETE (_array_base, VEC_mpool);
  while (_ugs.Elements())
    CXX_DELETE (_ugs.Pop(), VEC_mpool);
}

void VEC_BASE_ARRAY::Build_Base_LGs () {
 ...
 *
 ***********************************************************************/
BOOL VEC_BASE_ARRAY::Add_Ref (WN* wn_array, BOOL do_check) {
  if (do_check) {
    ACCESS_ARRAY* aa = (ACCESS_ARRAY*) WN_MAP_Get(LNO_Info_Map, wn_array);
  ...
  if (i == _ugs.Elements ()) {
    ...
    _ugs.Push (CXX_NEW (VEC_UGS(wn_array, this), VEC_mpool));
  }
  return TRUE;
}

VEC_VOLUME VEC_BASE_ARRAY::Volume (mINT16 depth) {
  VEC_VOLUME myvol (0, 0);
  for (INT i=0; i<_ugs.Elements(); i++) {
 ...
 *
 ***********************************************************************/
void VEC_BASE_ARRAY::Find_Loc_Space (VEC_LOCLOOP locloop) {
  for (INT i=0; i<_ugs.Elements(); i++)
    _ugs.Bottom_nth(i)->Find_Loc_Space (locloop);
}

VEC_SPLIT_VECTOR* VEC_BASE_ARRAY::Find_Split_Vector () {
  VEC_SPLIT_VECTOR* split_vec = NULL;
  for (INT i=0; i<_ugs.Elements(); i++) {
    VEC_SPLIT_VECTOR* tmp = _ugs.Bottom_nth(i)->Find_Split_Vector  ();
    if (tmp) {
...
void VEC_BASE_ARRAY::Gen_Prefetch (VEC_SPLIT_VECTOR* split_vec) {
  for (INT i=0; i<_ugs.Elements(); i++)
    _ugs.Bottom_nth(i)->Gen_Prefetch (split_vec);
}

void VEC_BASE_ARRAY::Print (FILE* fp) {
