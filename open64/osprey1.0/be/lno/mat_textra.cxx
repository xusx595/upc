...
/** $Revision: 1.4 $
*** $Date: 2005/11/22 03:56:11 $
*** $Author: wychen $
*** $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/lno/mat_textra.cxx $
**/

#define mat_textra_CXX      "mat_textra.cxx"
static char *rcs_id =   mat_textra_CXX "$Revision: 1.4 $";
...
template<> IMAT MAT<mINT32>::Inv() const
{
  FmtAssert(_r == _c, ("Matrix not square"));
...
template<> FMAT MAT<FRAC>::Inv() const		// XXX compiler bug: confused by FMAT
{
...
template<> DMAT MAT<double>::Inv() const		// XXX compiler bug: confused by DMAT
{
  FmtAssert(_r == _c, ("Matrix not square"));
...
template<> void DMAT::Print_Element(FILE* f, double e)
{
  fprintf(f, "%g", e);
}

template<> void IMAT::Print_Element(FILE* f, mINT32 e)
{
  fprintf(f, "%d", e);
}

template<> void FMAT::Print_Element(FILE* f, FRAC e)
{
  e.Print(f);
}
