 ...
 * Module: oinvar.cxx
 * $Revision: 1.3 $
 * $Date: 2004/10/28 22:54:09 $
 * $Author: wychen $
 * $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/lno/oinvar.cxx $
 *
 * Revision history:
...
static char *rcs_id = "$Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/lno/oinvar.cxx $ $Revision: 1.3 $";
		...
		HASH_TABLE<WN *,BIT_VECTOR *> *htable, MEM_POOL *pool, BOOL outer_only)
{
  ...
  if (OPCODE_is_load(opcode)) {
	...
	/* We can't hoist the entire expressions, but we can look at its lids */
        INT j;
	for (j=1; j<i; j++) {
