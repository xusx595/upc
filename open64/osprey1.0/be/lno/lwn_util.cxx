...
void DBX_WN_Map (WN_MAP map, WN* wn) {
  ...
  printf ("Map = 0x%p (%lld)\n", WN_MAP_Get(map, wn), (INT64)(intptr_t)WN_MAP_Get(map, wn));
}
...
/********************************************
 * 
 * UPC specific helper functions here
 *
 */

//help method to create a call to upcr_threads()
WN* LWN_Get_Threads() {

  if (upc_num_threads != 0) {
    return WN_Intconst(Integer_type, upc_num_threads);
  }
  return WN_Ldid(MTYPE_I4, 0, upc_threads_st, ST_type(upc_threads_st));
}

//Add a tas on wn to the type ty
//This avoids the simplifier dropping the tas because they have the same mtype
WN* LWN_Get_Tas(WN* wn, TY_IDX ty) {
  
  WN* res = WN_Create(OPR_TAS, Pointer_Mtype, MTYPE_V, 1);
  WN_kid0(res) = wn;
  WN_set_ty(res, ty);
  return res;
}

//return one of the upc pointer-to-shared manipulation calls (7.2.3)
//intrn should be one of threadof/phaseof/addrfield
WN* LWN_Get_UPC_Intrinsic_Call(WN* wn, INTRINSIC intrn) {
  
  TY_IDX ty = WN_Get_Ref_TY(wn);
  FmtAssert(Type_Is_Shared_Ptr(ty, true), ("wn must be a pointer-to-shared"));
  bool is_pshared = TY_To_Sptr_Idx(ty) == pshared_ptr_idx;
  switch (intrn) {
  case INTRN_THREADOF_S:
    if (is_pshared) {
      intrn = INTRN_THREADOF_P;
    }
    break;
  case INTRN_PHASEOF_S:
    if (is_pshared) {
      intrn = INTRN_PHASEOF_P;
    }
    break;
  case INTRN_ADDROF_S:
    if (is_pshared) {
      intrn = INTRN_ADDROF_P;
    }
    break;
  default:
    //unsupported for now until we have a need
    FmtAssert(FALSE, ("unexpected intrinsic type"));
  }

  WN* tmp_block = WN_CreateBlock();
  WN* exp = WN_Create(OPR_INTRINSIC_CALL, Integer_type, MTYPE_V, 1);
  WN_intrinsic(exp) = intrn;
  WN_kid0(exp) = WN_CreateParm(Pointer_Mtype, wn, ty, WN_PARM_BY_VALUE);
  WN_INSERT_BlockLast(tmp_block, exp);
  return WN_CreateComma(OPR_COMMA, Integer_type, MTYPE_V, tmp_block,
