...
/** $Revision: 1.6 $
*** $Date: 2006/08/15 02:39:00 $
*** $Author: ciancu $
*** $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/lno/access_vector.h $
**/
...
#ifdef _KEEP_RCS_ID
static char *access_vector_rcs_id = access_vector_INCLUDED "$Revision: 1.6 $";
#endif /* _KEEP_RCS_ID */
...
//gcc on Tru64 doesn't have an abs function for long long, so we have 
//to define one here to avoid overloading conflicts
...
friend ACCESS_VECTOR *Merge(ACCESS_VECTOR *, ACCESS_VECTOR *,
  ...
  WN* Get_Base_WN() const;
  WN* Get_Base_WN(STACK<WN* > *ind_vars) const;
};
