...
* $Revision: 1.8 $
* $Date: 2013/03/20 00:53:20 $
* $Author: phargrov $
* $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/com/opt_alias_rule.cxx $
*
* Revision history:
...
//  Exclude pointers.
//  Use bit 0 to bit 62 for mtypes.
//  Use bit 63 for pointers.
//
static const INT64 MTYPE_PTR = 63;
static const INT64 ALL_TYPE = 0xffffffffffffffffLL;
static const INT64 NO_TYPE  = 0;
...
INT64 ALIAS_RULE::Get_stripped_mtype(TY_IDX ty_idx) const
{
  ...
  case KIND_SCALAR:
    ...
    //    Note that enum is represented as KIND_SCALAR.
    {
      ...
      Is_True(mtype < MTYPE_PTR, ("more than 63 mtypes."));
      switch (mtype) {
      case MTYPE_I1:
  ...
  {
      ret_type = ((INT64)1 << MTYPE_PTR);
      int i =0;
      for (TY_IDX idx = TY_pointed(ty); TY_kind(idx) == KIND_POINTER; idx = TY_pointed(idx)) {
	  ret_type |= ((INT64)1 << (MTYPE_PTR - i));
	  i++;
      }
	  break;
  }
...
// Implement rule U.2
//
BOOL ALIAS_RULE::Aliased_UPC_Call_Rule(ST *call_st, TY_IDX ty) const 

{
  //rule is disabled because it's unsafe (see opt_alias_rule.h)
  return TRUE;

  if (!ST_keep_name_w2f(call_st) && Type_Is_Shared_Ptr(ty, true)) {
    //fprintf(stderr, "applying UPC call rule on %s\n", ST_name(call_st));
    return FALSE;
  }
  return TRUE;
}
...
// Implement rule U.1
//
BOOL ALIAS_RULE::Aliased_UPC_Shared_Rule(const POINTS_TO *p1, 
					 const POINTS_TO *p2,
					 TY_IDX ty1, TY_IDX ty2) const 

{
  //fprintf(stderr, "calling upc shared rule: %d %d\n", p1->Default_shared_vsym(), p2->Default_shared_vsym());
  if (p1->Default_shared_vsym() && ty2 != 0 && !TY_is_shared(ty2)) {
    if (p2->Default_shared_vsym()) 
      fprintf(stderr, " ooops\n");
    //fprintf(stderr, "using upc shared rule\n");
    //p1->Print(stderr);
    //Print_TY(stderr, ty2);
    return FALSE;
  }

  if (p2->Default_shared_vsym() && ty1 != 0 && !TY_is_shared(ty1)) {
    if (p1->Default_shared_vsym()) 
      fprintf(stderr, " 2ooops\n");
    //fprintf(stderr, "using upc shared rule\n");
    //p2->Print(stderr);
    //Print_TY(stderr, ty1);
    return FALSE;
					       ...
					       TY_IDX ty1, TY_IDX ty2) const
 {
  ...
  if (Compile_Upc && !Aliased_UPC_Shared_Rule(p1, p2, ty1, ty2)) 
    return FALSE;
...
READ_WRITE ALIAS_RULE::Aliased_with_Call(ST *st, INT32 flags, const POINTS_TO *mem) const
{
  ...
  if (Compile_Upc && st != NULL) {
    if (!Aliased_UPC_Call_Rule(st, mem->Ty())) {
      return NO_READ_NO_WRITE;
    }
  }
