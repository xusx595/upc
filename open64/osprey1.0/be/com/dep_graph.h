...
/** $Revision: 1.4 $
*** $Date: 2006/08/01 17:49:03 $
*** $Author: ciancu $
*** $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/com/dep_graph.h $
**/
...
#ifdef _KEEP_RCS_ID
static char *dep_graph_rcs_id = dep_graph_INCLUDED "$Revision: 1.4 $";
#endif /* _KEEP_RCS_ID */
...
public:
  ...
  DEP_GRAPH_TYPE Type() { return _type;}
  void Print(FILE *fp);
  ARRAY_DIRECTED_GRAPH16(mUINT16 num_v, mUINT16 num_e, WN_MAP map, 
	DEP_GRAPH_TYPE type) : 
  ...
  friend void Print_ADG(FILE *,  ARRAY_DIRECTED_GRAPH16 *);
  friend int Deps_Say_Is_Vectorizable(ARRAY_DIRECTED_GRAPH16 *adg);
  ...
  VINDEX16 Add_Vertex(WN *wn) {
    ...
    if (result != 0) {
      _v[result].Wn = wn;
#ifdef _LP64
      WN_MAP_Set(_map,wn, (void *) (UINT64) result);
#else
  ...
  void Set_Wn(VINDEX16 v, WN* wn) {
    _v[v].Wn=wn;
#ifdef _LP64
    WN_MAP_Set(_map, _v[v].Wn, (void*)(UINT64)v);
#else
