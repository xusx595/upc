...
* $Revision: 1.7 $
* $Date: 2005/11/11 23:42:21 $
* $Author: wychen $
* $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/com/opt_alias_mgr.cxx $
*
* Revision history:
...
ALIAS_MANAGER::ALIAS_MANAGER(void)
{
  ...
  _upc_locality_map = WN_MAP_Create(&_mem_pool);
}
...
ALIAS_MANAGER::~ALIAS_MANAGER(void) 
{
  ...
  WN_MAP_Delete(_upc_locality_map);
...
void ALIAS_MANAGER::Enter_locality_info(WN* wn, INT thread_id) {

    local_info *info = CXX_NEW(local_info(thread_id), &_mem_pool);
    WN_MAP_Set(_upc_locality_map, wn, info);
}

BOOL ALIAS_MANAGER::Points_to_same_thread(WN* wn1, WN* wn2) {
    
    local_info* info1 = (local_info*) WN_MAP_Get(_upc_locality_map, wn1);
    local_info* info2 = (local_info*) WN_MAP_Get(_upc_locality_map, wn2);

    if (info1 != NULL && info2 != NULL) {
	return info1->_id != POINTS_TO::LOCALITY_TOP && info1->_id == info2->_id; 
    }
    return FALSE;
}

BOOL ALIAS_MANAGER::Points_to_local(WN *wn) {
    
  local_info* info = (local_info*) WN_MAP_Get(_upc_locality_map, wn);
  return info != NULL && info->_id == POINTS_TO::LOCALITY_MYTHREAD;
}

BOOL ALIAS_MANAGER::Points_to_thread(WN *wn, INT id) {

  local_info* info = (local_info*) WN_MAP_Get(_upc_locality_map, wn);
  return info != NULL && info->_id == id;
}
