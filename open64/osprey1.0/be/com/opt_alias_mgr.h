...
* $Revision: 1.2 $
* $Date: 2005/11/05 03:06:25 $
* $Author: wychen $
* $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/com/opt_alias_mgr.h $
*
* Revision history:
...
public:
  ...
  void Enter_locality_info(WN *wn, INT thread_id);

  BOOL Points_to_same_thread(WN *wn1, WN *wn2);
  BOOL Points_to_local(WN *wn);
  BOOL Points_to_thread(WN *wn, int thread_id);
};

//Wrapper for the thread_id values
typedef struct foo {
    INT _id;
    foo(int id) {
	_id = id;
