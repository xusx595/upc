...
// $Revision: 1.5 $
// $Date: 2009/12/04 04:07:29 $
// $Author: phargrov $
// $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/com/wn_verifier.cxx $
//
// Revision history:
...
WN_Verifier::Field_id_valid (WN* wn)
{
	...
	if (TY_kind(TY_pointed(*ty)) == KIND_STRUCT &&
	    ...
	    if (WN_operator(kid) == OPR_INTCONST) {
	      ...
	      //unfortunately, the offset adjsutment code is in such a mess that 
	      //the correct size could either be TY_size() or TY_adjusted_size() 
	      //(depending on which phase of lowering and also on the kind of whirl node,
	      //I think), so we have to forego this check here
	      if (!Compile_Upc) 
	...
	// fall through
    default:
      //Alas, our implementation of ILOADs differ from that of the whirl spec
      //(i.e., for field accesses, WN_ty() returns the field type for us, while 
      //it would return the struct type according to the spec).  Therefore we have 
      //to skip this check here.
      if (!Compile_Upc) {
