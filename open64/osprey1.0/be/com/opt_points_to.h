...
//
// For UPC pointer-to-shared objects, in the points-to set we additionally track the thread that 
// the pointed-to object is located on.  This locality attribute can take on one of the following values:
//
//  TH(p) = {0, 1, ... T-1} U {M, Top}, where M indicates MYTHREAD (i.e., p points to local object for 
//  every thread).
//
//  The analysis is flow-sensitive, and the value is updated for pointer assignment statements 
//
...
enum PT_ATTR {
  ...
  PT_ATTR_DEFAULT_SHARED_VSYM = 0x800000  //UPC specific; like DEFAULT_VSYM, but
                                          //does not alias to any non-shared object
...
public:
  ...
  static const INT32 LOCALITY_TOP = -1;
  static const INT32 LOCALITY_MYTHREAD = -2;
  ...
  BOOL        Default_shared_vsym(void)     const { return ai._attr & PT_ATTR_DEFAULT_SHARED_VSYM; }
  ...
  void Set_default_shared_vsym(void)      { ai._attr = (PT_ATTR) (ai._attr | PT_ATTR_DEFAULT_SHARED_VSYM); }
  ...
  void Reset_default_shared_vsym(void)           { ai._attr = (PT_ATTR) (ai._attr & ~PT_ATTR_DEFAULT_SHARED_VSYM); }
