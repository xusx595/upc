 ...
 * Module: w2c_driver.c
 * $Revision: 1.65 $
 * $Date: 2013/10/15 20:51:51 $
 * $Author: phargrov $
 * $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/whirl2c/w2c_driver.cxx $
 *
 * Revision history:
 ...
 *  for traslating a PU at a time or arbitrary subexpressions.
 *
...
#ifdef _KEEP_RCS_ID
static char *rcs_id = "$Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/whirl2c/w2c_driver.cxx $ $Revision: 1.65 $";
#endif /* _KEEP_RCS_ID */
...
BOOL  W2C_Verbose = FALSE;           /* Show translation information */
			  ...
			  INT argc, char * const argv[])
{
  ...
  static char ORIG[] = "LNO-DUMP-ORIG";
  static char W2CH[] = "LNO-DUMP-W2CH";
  static char W2CC[] = "LNO-DUMP-W2CC";
  static char W2CL[] = "LNO-DUMP-W2CL";
    /* Get the program name
     */
    if (argv[0] != NULL) {
       W2C_Progname = argv[0];
       //this string is empty when called from LNO with trace on
       if(strlen(argv[0]) > 0) {
         W2C_Progpath = W2C_Progname;
         W2C_Progpath.erase(W2C_Progpath.find_last_of('/'));
       }
    }
    ...
    if(strlen(argv[0]) > 0) {
      ...
      /* FIXME: disable verbose output for now, should try to add a flag in the future */
      W2C_Verbose = FALSE;
	...
	W2C_Emit_Prefetch = CLIST_emit_prefetch;
      ...
      //this happens when tracing LNO
      W2C_File_Name[W2C_ORIG_FILE] = ORIG;
      W2C_File_Name[W2C_DOTH_FILE] = W2CH;
      W2C_File_Name[W2C_DOTC_FILE] = W2CC;
      W2C_File_Name[W2C_LOC_FILE] = W2CL;
      W2C_Enabled = 1;
      W2C_Verbose = 1;
      /* FIXME: disable verbose output for now, should try to add a flag in the future */
      //W2C_Verbose = FALSE;
...
W2C_Outfile_Init(BOOL emit_global_decls)
{
   ...
   if (Compile_Upc) {
     ...
     Write_String(W2C_File[W2C_DOTC_FILE], W2C_File[W2C_LOC_FILE], buf);
       ...
       const char *osname = "openbsd";
     #elif defined(__NetBSD__)
       const char *osname = "netbsd";
     #else
     ...
     if (gethostname(hostname, MAXHOSTNAMELEN)) strcpy(hostname, "*unknown_hostname*");
       ...
       const char *gccversion = ", opencc v"__OPEN64__;
     #elif defined(__PATHSCALE__)
       const char *gccversion = ", pathcc v"__PATHSCALE__;
     #elif defined(__INTEL_COMPILER)
       char gccversion[32];
       sprintf(gccversion, ", icc v%d.%d %u",
				(__INTEL_COMPILER / 100),
				((__INTEL_COMPILER / 10) % 10),
				(unsigned int)__INTEL_COMPILER_BUILD_DATE);
     #elif defined(__APPLE_CC__)
       const char *gccversion = ", Apple gcc v"__VERSION__;
     ...
     sprintf(buf, "/* UPC translator version: release 2.20.0, built on %s at %s, host %s %s-%s/%s%s */\n",
     ...
     { /* DOB: output upcr_trans_extra file(s) */

       /*
       //wei: copy the trans_extra files for autonb
       char buf[1024];
       if (run_auto_nb) {
	 string autonb_inname = W2C_Progpath + "/../../libupc/upcr_trans_extra_auto_nb.c";
	 string autonb_outname = "./upcr_trans_extra_autonb.c";
	 FILE *in = Open_Read_File(autonb_inname.c_str());
	 FILE *out = Open_Create_File(autonb_outname.c_str());
	 while (fgets(buf, 1024, in)) {
	   fputs(buf, out);
	 }
	 
	 //FIXME: need to write the declarations of the runtime hook functions
	 //into a header file
       }
       */
