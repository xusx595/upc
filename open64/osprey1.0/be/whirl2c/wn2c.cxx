 ...
 * $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/whirl2c/wn2c.cxx $
 *
 * Revision history:
...
#ifdef _KEEP_RCS_ID
_id = "$Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/whirl2c/wn2c.cxx $ $Revision: 1.165 $";
...
static BOOL WN2C_TY_is_shared(TY_IDX ty) {
  return TY_is_shared(ty) || strcmp(TY_name(ty),"shared_ptr_struct") == 0 || strcmp(TY_name(ty),"pshared_ptr_struct") == 0;
}
...
// TODO WHIRL 0.30: get rid of OPC_I4T1 variants
  ...
  {OPC_I4F4EQ, "=="},
  {OPC_I4C4EQ, "=="},
  /* A res type I8 is possible for ilp64 machines (t3e, etc)*/
  {OPC_I8U8EQ, "=="},
  {OPC_I8FQEQ, "=="},
  {OPC_I8I8EQ, "=="},
  {OPC_I8U4EQ, "=="},
...
//      CONTEXT_set_top_level_expr(context);
      WN2C_translate(opnd_tokens, wn, context);
      if (CONTEXT_ptr_arith(context) && TY_kind(result_ty) == KIND_SCALAR && TY_kind(opnd_ty) == KIND_SCALAR) {
		 ...
		 CONTEXT      context)
{
   ...
   if (intconst != NULL)
   {
	 ...
	 WN_const_val(intconst) = 
	   WN_const_val(intconst) / Adjusted_Type_Size(TY_pointed(expr_ty));
   ...
   else
   {
	 ...
	 if (intconst != NULL)
	 {
	    ...
	    if (TY_size(given_lvalue_ty) > 1)
	    {
	       ...
	       old_intconst = WN_const_val(intconst);
	       WN_const_val(intconst) = 
		 WN_const_val(intconst) / Adjusted_Type_Size(given_lvalue_ty);
		    ...
		    STAB_OFFSET offset)
{
   ...
   if (TY_Is_Array(base_ty))
   {
       ...
       */
      return load_ty;
   } else if (Compile_Upc && (WN2C_TY_is_shared(base_ty) || Type_Is_Shared_Ptr(base_ty))) {
     /* Never try to read into pshared/shared pointer */
     return base_ty;
...
static STAB_OFFSET WN2C_Gen_Field_Access(TOKEN_BUFFER expr_tokens, TY_IDX fld_ty, 
	...
	cur_offset -= FLD_ofst(fld_stack[i]);
	Append_Token_Special(expr_tokens, '.');
	Append_Token_String(expr_tokens, FLD_name(fld_stack[i]));
	if (i != fld_stack.size()-1) {
	    ...
	    if (TY_Is_Array(FLD_type(fld_stack[i]))) {
		cur_offset = WN2C_Gen_Const_Array_Index(expr_tokens, FLD_type(fld_stack[i]), cur_offset);
	    }
	}
    }
    return cur_offset;
		  ...
		  UINT field_id = 0)    
{
   ...
   {
			field_info = TY2C_get_field(base_obj_ty, object_ty, TY_mtype(object_ty), addr_offset);
   }
   ...
   {
			WHIRL2C_parenthesize(expr_tokens);	/* a source of redundant parenthsis for array of struct */
      if (expr_is_lvalue)
	 Append_Token_Special(expr_tokens, '.');
      else
   ...
   else if (addr_offset != (STAB_OFFSET) 0 || incompat_obj_tys)
   {
	...
	if (addr_offset != 0) {
	  /* Make sure we're using the correct size if objects are pointer-to-shared. See bug358 */
	  TY_IDX real_ty = TY_To_Sptr_Idx(object_ty);
	  WN2C_append_addr_plus_const(expr_tokens, 
				      TY_size(real_ty), 
				      addr_offset);
	}
...
static void
WN2C_use_translate_maybe_tld(TOKEN_BUFFER tokens,
			     const ST    *st,
			     CONTEXT      context)
{
  ...
  if (use_TLD
	...
	&& !WN2C_TY_is_shared(ST_type(st))
	&& (strstr(ST_name(st), ".init") == NULL)) {
    Prepend_Token_String(tokens, "UPCR_TLD_ADDR(");
	       ...
	       UINT field_id = 0)
{
   ...
   if (ST_sym_class(st) == CLASS_CONST)
   {
     ...
     /* bug381: THREADS is a signed int in upc, while upcr_threads() returns an unsigned int 
      *	So, we add an explicit cast here
      */
     Append_Token_String(tokens, "((int)");
	 ...
	 addr_ty = Stab_Pointer_To(ST_type(st));
      ...
      WN2C_use_translate_maybe_tld(tokens, st, context);
...

{
  ...
  WN2C_use_translate_maybe_tld(tokens, WN_st(wn), context);
	       ...
	       CONTEXT      context)
{
   ...
   TY_IDX tree_type;
    ...
    if ( WN_operator(kid0) == OPR_ADD ) {
      if  (WN_operator(WN_kid0(kid0)) == OPR_ARRAY )
        struct_type = 2;
      else if ( WN_operator(WN_kid1(kid0)) == OPR_MPY )
        struct_type = 3;
    }
    tree_type = WN_get_Tree_Type(wn, struct_type);
	}
	else
		tree_type = WN_Tree_Type(wn);
   ...
   if (WN_operator(wn) == OPR_LDA) {
     ...
     BOOL reset = TY_Is_Array(TY_To_Sptr_Idx(ST_type(WN_st(wn))));
		       ...
		       CONTEXT       context)
{
   ...
   if (!WN2C_assignment_compatible_types(assign_ty, rhs_ty))
   {
	...
	if (WN2C_TY_is_shared(assign_ty)) {
	 ...
	 Append_Token_Special(tokens, '=');
	 if (null_shared) {
	   if (rhs_ty == shared_ptr_idx) {
	     Append_Token_String (tokens, " (*(upcr_shared_ptr_t *)&upcr_null_shared)");
	   } else {
	     Append_Token_String(tokens, "(*(upcr_pshared_ptr_t *)&upcr_null_pshared)");
	   }
	 } else {
			...
			CONTEXT      context)
{
   ...
   FOREACH_SYMBOL(CURRENT_SYMTAB, st, st_idx)
   {
     ...
     bool inline_func = ST_class(st) == CLASS_FUNC && PU_is_inline_function(Pu_Table[ST_pu(st)]);
	...
	ST_sclass(st) == SCLASS_FSTATIC || 
     ...
     global = global || !ST_keep_name_w2f(st);
     global = global || inline_func; //skip the output of inline function prototypes (for bug438)
	...
	*/
       tmp_tokens = New_Token_Buffer();
       TY_IDX base_ty = WN2C_get_base_type(ST_type(st));
       if (Is_Structure_Type(base_ty) && !TY_is_translated_to_c(base_ty)) {	     
	 TY2C_translate(tmp_tokens, base_ty, context);
       }
     }
		       ...
		       CONTEXT      context)
{
   ...
   else /* Return through a preg; the usual case */
   {
       ...
       */
      if (preg_mtype == MTYPE_V && !(Compile_Upc && TY_mtype(return_ty) != MTYPE_V))
      {
	...
	if (!WN2C_assignment_compatible_types(return_ty, ST_type(result_var))) {
	 ...
	 //Print_ST(stderr, result_var, true);
	 ...
	 if (Compile_Upc && TY_kind(return_ty) == KIND_STRUCT) {
...
WN2C_do_loop(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   if (W2C_Emit_Cgtag && loop_info != NULL)
   {
	...
	Append_Token_String(tokens, Number_as_String((UINT64)(uintptr_t) loop_info, "%ull"));
      } else {
	Append_Token_String(tokens, Number_as_String((UINT32)(uintptr_t) loop_info, "%ul"));
      }
      Append_Token_String(tokens, "*/");
      Append_Indented_Newline(tokens, 1);
   }
...
WN2C_return(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   //write EXIT_FUNCTION before every return
   if (Compile_Upc) {
     Append_Token_String(tokens, "UPCR_EXIT_FUNCTION();");
     WN2C_Stmt_Newline(tokens, CONTEXT_srcpos(context));
   }
...
WN2C_mstore(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   if (WN_operator(WN_kid0(wn)) == OPR_MLOAD)
   {
	 ...
	 Append_Token_Special(tokens, ',');
	 const WN *rhs = WN_kid0(WN_kid0(wn));
	 if (WN_operator(rhs) == OPR_LDA &&
	     ST_sym_class(WN_st(rhs)) == CLASS_CONST &&
	     TCON_ty(STC_val(WN_st(rhs))) == MTYPE_STRING)
	 { /* rhs is string constant - copy the shorter of the two lengths (bug 2698) */
	    const WN *bytes = WN_kid(wn,2);
	    UINT64 string_len = Targ_String_Length(STC_val(WN_st(rhs)));
	    if (WN_operator(bytes) == OPR_INTCONST)
	    {
	       UINT64 length = MIN(string_len, WN_const_val(bytes));
	       TCON2C_translate(tokens, Host_To_Targ(MTYPE_I8, length));
	    }
	    else
	    {
	       Append_Token_String(tokens, "_I8MIN");
	       Append_Token_Special(tokens, '(');
	       TCON2C_translate(tokens, Host_To_Targ(MTYPE_I8, string_len));
	       Append_Token_Special(tokens, ',');
	       (void)WN2C_translate(tokens, bytes, context);
	       Append_Token_Special(tokens, ')');
	    }
	 }
	 else 
   ...
   else
   {
	 ...
	 TCON2C_translate(tokens, 
      ...
      //issue a warning here, since the use of byte munching loops 
      //for mstore almost always indicate a type mismatch/offset error somewhere
      //translator_warning("MSTORE is generated as a for loop");
   }
...
WN2C_call(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   for (arg_idx = first_arg_idx; arg_idx <= last_arg_idx; arg_idx++)
   {
	 ...
	 CONTEXT_set_given_lvalue_ty(context,
				     ...
				     TY_IDX_ZERO); /* means no prototype */
...
WN2C_comment(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
  ...
  if (Compile_Upc && strncmp(s, "#pragma", 7) == 0) {
...
WN2C_array(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   Is_True(WN_operator(wn) == OPR_ARRAY,
	...
	/*
	 * array_dims contains a list of ARRAY nodes in the ARRAY tree
	 */
	struct_type = 1;			/* default is type 1 */
	/* sum_of_missing_arrays: needed for the missing integer constant index expressions of 
	 * arrays of structs (note, only the outer-most and the inner-most arrays are expressed 
	 * using ARRAY nodes)
	 */
	UINT sum_of_missing_arrays;
	WN* base_wn;
	for (	base_wn = (WN*) wn; 
				WN_operator(base_wn) == OPR_ARRAY || WN_operator(base_wn) == OPR_ADD; 
				base_wn = WN_kid0(base_wn)) 
	{
		if (WN_operator(base_wn) == OPR_ARRAY) {
			...
			if (WN_operator(kid1) == OPR_MPY && WN_operator(WN_kid1(kid1)) == OPR_INTCONST) {
				...
				sum_of_missing_arrays = WN_const_val(WN_kid1(kid1));
			} 
			else {
				/* assuption: (WN_operator(base_wn) == OPR_ADD) for a pointer arithmetic */
				struct_type = 2;
				FmtAssert( (WN_operator(kid1) == OPR_INTCONST), 
								("INTCONST is assumed as an operand in the pointer arithmetic") );
				sum_of_missing_arrays = WN_const_val(kid1);	/* kid1 of OPR_ADD is OPR_INTCONST */
			}
		}
	}

	/** An array of struct within a struct has an LDA node in the ARRAY node tree 
	  * where LDA node has a non-zero field_id.
	  */
	if (WN_operator(base_wn) == OPR_LDA && WN_field_id(base_wn) != 0) {
    FLD_list fld_stack;
    UINT cur_fld = 0;
    TY2C_FLD_INFO field_info;
    ...
    if (TY_kind(base_ty) == KIND_ARRAY) {
      base_ty = Get_Inner_Array_Type(base_ty);
		...
		INT32 lda_offset = WN_lda_offset(base_wn);
		FLD_HANDLE found_fld = FLD_get_the_last_array_field(base_ty, lda_offset, struct_type, fld_stack );

    if (!found_fld.Is_Null()) {
	//try to generate symbolic accesses
	UINT cur_offset;
	if (struct_type==2) { cur_offset = sum_of_missing_arrays; }
	else { cur_offset = WN_offset(base_wn); }

	WN2C_use_translate_maybe_tld(tokens, WN_st(base_wn), context);
	base_ty = ST_type(WN_st(base_wn));

		// generate the array base expression; the outer-most struct expression
	    ...
	    if (elt_size == WN_element_size(index)) {
			...
			else { // calculate the integer constant index expression
		int index = cur_offset / elt_size;
		Append_Token_Special(tokens, '[');
		char buf[32];
		sprintf(buf, "%1d", index);
	...
	for (int i = 0; i < fld_stack.size(); i++) {
	  ...
	  Append_Token_Special(tokens, '.');
	  Append_Token_String(tokens, FLD_name(fld_stack[i]));
	  for (TY_IDX tmp_ty = FLD_type(fld_stack[i]); TY_Is_Array(tmp_ty); tmp_ty = TY_etype(tmp_ty)) { 
	...
	if(cur_offset != 0 &&  cur_offset >= TY_size(FLD_type(fld_stack[0])))  {
	...
	//try flattening it - this is hopefully now unreachable!
...
WN2C_tas(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   TY_IDX from = TY_To_Sptr_Idx(WN_Tree_Type(WN_kid0(wn)));
   TY_IDX to = TY_To_Sptr_Idx(WN_ty(wn));

   //if (WN2C_arithmetic_compatible_types(to, from) &&
...
WN2C_ldid(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   if (Compile_Upc && WN_operator(wn) == OPR_LDID) {
...
WN2C_lda(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   else /* The normal case */
   {
	 ...
	 WN2C_lvalue_st(expr_tokens, 
      ...
      if (WN2C_TY_is_shared(object_ty)) {
	/* we'll be returning an lvalue */
	STATUS_set_lvalue(lda_status);
      }
	 ...
	 STATUS_reset_lvalue(lda_status); /* Not returning an lvalue */
      ...
      {
	  if (TY_kind(object_ty) == KIND_ARRAY /* && WN_load_offset(wn) != 0 */) {
...
static BOOL WN2C_int_downcast(TY_IDX from, TY_IDX to) {
  ...
  if (TY_kind(from) == KIND_SCALAR &&
    ...
    if (TY_size(from) > TY_size(to)) {
      ...
      */
      return true;
    } else if (/* TY_is_logical(from) && */ TY_is_logical(to) && !TY_is_shared(to)) {
...
WN2C_parm(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   if (!volatile_arg && WN2C_assignment_compatible_types(parm_ty, arg_ty) &&
		...
		CONTEXT      context)
{
   ...
   */
	if ( WN_operator(lhs) == OPR_ARRAY ) {
		...
		base_ty = WN_get_Tree_Type(lhs, struct_type);
	}
	else {
	      ...
	      UINT field_id)
{
   ...
   else
   {
      ...
      WN2C_SymAccess_Type(&base_ty,
			  ...
			  TY_To_Sptr_Idx(stid_ty),         /* preferred type */
			  dtype,           /* required mtype */
