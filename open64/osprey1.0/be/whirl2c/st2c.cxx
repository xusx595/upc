 ...
 * Module: st2c.c
 * $Revision: 1.21 $
 * $Date: 2009/10/16 03:29:45 $
 * $Author: sjmin $
 * $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/whirl2c/st2c.cxx $
 *
 * Revision history:
 ...
 */
#ifdef _KEEP_RCS_ID
static char *rcs_id = "$Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/whirl2c/st2c.cxx $ $Revision: 1.21 $";
#endif /* _KEEP_RCS_ID */
			  ...
			  TY_IDX        ty)
{
   ...
   if (ty2c_list == NULL)
   {
      ...
      if (TY_kind(ty) != KIND_STRUCT) {  //Avoid outputting duplicate struct types (see bug574) 
...
ST2C_basic_decl(TOKEN_BUFFER tokens, const ST *st, CONTEXT context)
{
   ...
   if(Compile_Upc && debug_requested && Type_Is_Shared_Ptr(tidx)) {
	 ...
	 if (Type_Is_Shared_Ptr(tidx)) {
       ...
       Set_ST_type((ST*)st, TY_To_Sptr_Idx(tidx));
   } else 
...
/* bug2502: omit static qualifiers for global declaration 
