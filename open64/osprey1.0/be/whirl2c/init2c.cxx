 ...
 * Module: init2c.c
 * $Revision: 1.8 $
 * $Date: 2011/01/28 17:23:33 $
 * $Author: phargrov $
 * $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/whirl2c/init2c.cxx $
 *
 * Revision history:
 ...
 */
#ifdef _KEEP_RCS_ID
static char *rcs_id = "$Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/whirl2c/init2c.cxx $ $Revision: 1.8 $";
#endif /* _KEEP_RCS_ID */
	    ...
	    INITV_IDX    initv)
{
   ...
   //bug972 & bug2979
   //Need to catch the case of null pointer-to-shared here
   //including both lowered and unlowered types
   TY_IDX real_ty = Type_Is_Shared_Ptr(ty, true) ? TY_To_Sptr_Idx(ty) : ty;
   if (real_ty == shared_ptr_idx && Targ_To_Host(tcon) == 0LL) {
	   Append_Token_String(tokens, "UPCR_NULL_SHARED");
	   return;
   } else if (real_ty == pshared_ptr_idx && Targ_To_Host(tcon) == 0LL) {
	   Append_Token_String(tokens, "UPCR_NULL_PSHARED");
	   return;
   }
