 ...
 * Module: wn_attr.c
 * $Revision: 1.19 $
 * $Date: 2011/01/19 08:29:13 $
 * $Author: phargrov $
 * $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/whirl2c/wn_attr.cxx $
 *
 * Revision history:
...
#ifdef _KEEP_RCS_ID
static char *rcs_id = "$Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/be/whirl2c/wn_attr.cxx $ $Revision: 1.19 $";
#endif /* _KEEP_RCS_ID */
...
{
	return WN_get_Tree_Type(wn, 0);
} /* WN_Tree_Type */

TY_IDX
WN_get_Tree_Type(const WN *wn, UINT struct_type=0)
{
   ...
   if (OPCODE_is_expression(WN_opcode(wn)))
   {
	...
	break;
	 ...
	 break;
			...
			// bug 2642: field id is not sufficient for other cases; depending on the type of struct,
			//           we need offset to get the desired field in a struct
			if (WN_field_id(wn) > 0) {
				TY_IDX pointed = TY_pointed(ty);
				if (TY_kind(pointed) == KIND_ARRAY) {
					pointed = Get_Inner_Array_Type(pointed);
				}
				...
				if (TY_kind(pointed) == KIND_STRUCT) {
					...
					else {	// added case to handle bug 2642
						/* fld_stack is not used here but used within a get_the_last_array_field */
						FLD_HANDLE found_fld;
						FLD_list fld_stack;
						INT32 lda_offset = WN_lda_offset(wn);
						found_fld = FLD_get_the_last_array_field(pointed, lda_offset, struct_type, fld_stack);
						pointed = FLD_type(found_fld);
					}
					if(TY_kind(pointed) == KIND_ARRAY) {
	  ...
	  * WN2C_array()).
	  */
	 ty = WN_get_Tree_Type(WN_kid0(wn), struct_type);
	 if (!TY_Is_Pointer(ty))
	 {
	 ...
	 if (WN_opc_rtype(wn) == Pointer_Mtype)
	 {
	    ...
	    if (!TY_Is_Pointer(ty))
	    {
	       ty = WN_get_Tree_Type(WN_kid1(wn), struct_type);
	       if (!TY_Is_Pointer(ty))
	 ...
	 if (INTR_is_adrtmp(WN_intrinsic(wn)))
	 {
	    ...
	    {
	       ty = WN_get_Tree_Type(WN_kid0(WN_kid0(wn)), struct_type);
	    }
	    else
	    {
	       ty = Stab_Pointer_To(WN_get_Tree_Type(WN_kid0(wn), struct_type));
	    }
	 }
	 else if (INTR_is_valtmp(WN_intrinsic(wn)))
	 {
		...
		WN_opcode(WN_kid0(wn)) == OPC_VINTRINSIC_CALL)
	    {
	       ty = TY_pointed(WN_get_Tree_Type(WN_kid0(WN_kid0(wn)), struct_type));
	    }
	    else
	    {
	       ty = WN_get_Tree_Type(WN_kid0(wn), struct_type);
	 ...
	 ErrMsg ( EC_Invalid_Case, "WN_get_Tree_Type", __LINE__ );
      } /* switch */
   } /* else */
   
   //return ty;
   return TY_To_Sptr_Idx(ty);  //Convert shared pointers to upcr_shared_ptr_t
} /* WN_Tree_Type */
