...
static WN* affinity_control_wn = 0;
static WN* affinity_test = 0;
...
WFE_Expand_Loop_Continue_Here (void)
{
  ...
  if(affinity_control_idx == loop_info_i) {
    ...
    if (affinity_test == NULL) {
      affinity_test = WFE_Add_Affinity_Test(while_body, affinity_control_wn);
    }
...
WFE_Expand_End_Loop (void)
{
  ...
  --loop_info_i;
  if (loop_info_i < 0 || loop_info_i < affinity_control_idx) {
  ...
  switch (loop_info_stack [loop_info_i].continue_info) {
    ...
    case CONTINUE_ELSEWHERE:
	 ...
	 if (loop_info_i > affinity_control_idx) //act as if continue 
	    ...
	    while_body = WN_then(WN_first(while_body));
	    //add pragma only if it does have an affinity expression
...
WFE_Expand_Return (tree retval)
{
  ...
  if (TREE_CODE (TREE_TYPE (TREE_TYPE (current_function_decl))) == VOID_TYPE) {
	...
	if(rhs_opr == OPR_ILOAD &&
	   Type_Is_Shared_Ptr(ty_idx)) {
	  ST *st = Gen_Temp_Symbol(ret_ty_idx, (char*) ".retspill");
	  WFE_Stmt_Append(WN_Stid (TY_mtype(ret_ty_idx), 0, st,
				   ret_ty_idx, rhs_wn),  Get_Srcpos());
	  rhs_wn = WN_Ldid(TY_mtype(ret_ty_idx), 0, st, ret_ty_idx);
	}
...
void WFE_Expand_Upc_Sync (int syncop, tree arg) {
  ...
  //a unique id for anonymous barriers
  ...
  if(syncop != 4) {
    ...
    if(arg) {
      ...
      int barrierval = 0;
      if (iop == INTRN_UPCBAR) {
	barrierval = string_hash(Src_File_Name) + counter++;
      }
      arg0 = WN_Intconst(MTYPE_I4, barrierval);
    }
...
{ 
  ...
  if (! affinity_control_wn && affinity_control_idx == -1) {
    //bug2396
    // evaluating the affinity control might generate spills
    // create a new context for these to be inserted into
    // later prepend this context before the aff_test
    //bug2956
    // need WFE_Expand_Expr_With_Sequence_Point() to preserve side-effects
    affinity_control_spills = WN_CreateBlock();
    WFE_Stmt_Push(affinity_control_spills, wfe_stmk_unknown, Get_Srcpos());
    affinity_control_wn = WFE_Expand_Expr_With_Sequence_Point(exp, TY_mtype (Get_TY (TREE_TYPE (exp))));
    WFE_Stmt_Pop(wfe_stmk_unknown);
...
WN*
WFE_Add_Affinity_Test(WN *while_body, WN* control_wn)
{
  WN *ltest;
  ...
  // bug2956: may be a comma expression
  WN* wn0 = (WN_operator(control_wn) == OPR_COMMA) ? WN_kid1(control_wn) : control_wn;
  TY_IDX ty = WN_ty(wn0);

  if (Type_Is_Shared_Ptr(ty, true)) {
    ...
    WN_kid0(threadof) = WN_CreateParm(Pointer_Mtype, control_wn, ty, WN_PARM_BY_VALUE);
    WN_INSERT_BlockFirst(tblock, affinity_control_spills);
  ...
  } else if (MTYPE_is_integral(WN_rtype(wn0))) {
    TYPE_ID mtype = WN_rtype(wn0);
    ...
    ST* tmp = Gen_Temp_Symbol(MTYPE_To_TY(mtype), ".Tthreads");
    threads = WN_Stid(mtype, 0, tmp, ST_type(tmp),
                      WN_Ldid (MTYPE_I4, -1, Return_Val_Preg, MTYPE_To_TY(MTYPE_I4)));
    WN_INSERT_BlockLast(tblock, threads);

    WN* mod = WN_Binary(OPR_REM, mtype, control_wn,
                        WN_Ldid(mtype, 0, tmp, ST_type(tmp)));
    ltest = WN_CreateComma(OPR_COMMA, mtype, MTYPE_V, tblock, mod);

    tblock = WN_CreateBlock();
    WN* mythread = WN_Create(OPR_INTRINSIC_CALL, MTYPE_I4, MTYPE_V, 0);
    WN_intrinsic(mythread) = INTRN_MYTHREAD;
    WN_INSERT_BlockLast(tblock, mythread);
    myval = WN_CreateComma(OPR_COMMA, MTYPE_I4, MTYPE_V, tblock,
                           WN_Ldid (MTYPE_I4, -1, Return_Val_Preg, MTYPE_To_TY(MTYPE_I4)));

    tblock = WN_CreateExp2(OPC_I4I4EQ, ltest, myval); 
  } else {
    error("Affinity expression is not an integral/shared address expression");
  }

  //bug2396 - for complex affinity expressions (that compute addresses) there are spills that are
  //  contained inside the while body (these should be the first statements
  // need to extract the spills and insert them at the beginning of tblock, otherwise we end up dereferrencing
  //uninitialized pointers
  // need to look inside while body for any stid into temps that appear in tblock, extract them and insert them in the same order before tblock
