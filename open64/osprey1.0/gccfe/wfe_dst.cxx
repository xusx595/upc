 ...
 * Module: dst.c
 * $Revision: 1.44 $
 * $Date: 2011/07/07 22:35:54 $
 * $Author: phargrov $
 * $Revision: 1.44 $
 * $Date: 2011/07/07 22:35:54 $
 * $Author: phargrov $
 * $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/gccfe/wfe_dst.cxx $
 *
 * Revision history:
...
#ifdef _KEEP_RCS_ID
static char *rcs_id = "$Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/gccfe/wfe_dst.cxx $ $Revision: 1.44 $";
#endif /* _KEEP_RCS_ID */
...
#ifdef __alpha
//on Tru64, getdomainame is a syscall and not available in unistd.h
extern "C" int getdomainname(char* name, int namelen);
#endif

#ifdef sun
//on Solaris, getdomainname is not available and we need a workaround
#include <sys/systeminfo.h>
#endif

#ifdef _AIX 
#include <netdb.h> // getdomainame
#endif
...
Get_Dir_Dst_Info (char *name)
{
	...
	dir_dst_list.push_back (std::make_pair (name2, ++last_dir_num));
	DST_mk_include_dir (name2);
	return last_dir_num;
}
...
Get_File_Dst_Info (char *name, UINT dir)
{
	...
	char *name2 = strdup(name);
	file_dst_list.push_back (std::make_pair (name2, ++last_file_num));
	DST_enter_file (name2, dir);
...
DST_Create_var(ST *var_st, tree decl)
{
	...
	0,  // offset (fortran uses non zero )
        (void*) (uintptr_t) ST_st_idx(var_st), // underlying type here, not typedef.
...
DST_enter_param_vars(tree fndecl,tree parameter_list)
{
	...
	param_idx = DST_mk_formal_parameter(
		...
		name,
		type_idx,
		(void* ) (uintptr_t) ST_st_idx(st), // so backend can get location
		DST_INVALID_IDX, // not inlined
...
DST_Create_Subprogram (ST *func_st,tree fndecl)
{
    ...
    dst = DST_mk_subprogram(
        ...
        (void*) (uintptr_t) ST_st_idx(func_st),  // index to fe routine for st_idx
...
#ifdef sun
	 if (sysinfo(SI_SRPC_DOMAIN, &current_host_dir[host_name_length+1], MAXHOSTNAMELEN-host_name_length) != -1)
#else
...
static bool is_c_header(char *file_name) {
  char *dotpos = strrchr(file_name, '.');

  if (file_name[0] == '<') {
    // Don't do put-back of "<built-in>", "<command-line>", etc.
    return FALSE;
  } else if (!dotpos) {
    // Is this really the best default for no extension?
    return TRUE;
  } else if (strcmp(dotpos, ".h") == 0) {
    return TRUE;
  } else if(strcmp(dotpos,".uph") == 0 ||
            strcmp(dotpos,".upc") == 0 ||
            strcmp(dotpos,".c") == 0) {
    return FALSE;
  } else {
    // Is this really the best default for unrecognized extensions?
    return TRUE;
  }
}
...
WFE_Set_Line_And_File (UINT line, char *file)
{
  ...
  // Don't want header put-back in middle of struct, union, enum, function body, or declaration.
  bool need_decls = defining_type || !WFE_Stmt_Empty() || (current_declspecs != NULL);
  ...
  if (strcmp(top->fname, file) != 0) {
    ...
    if(linedirs.empty() ) {
      ...
      keep_decl_for_w2c = uphext || need_decls;
	...
	if(is_c_header(top->fname) && onedown->state &&
           !is_upc_header(top->fname) && !top->state && !need_decls) {
	  h_files.push_back(top->fname);
	  //printf("FINISH: %s\n", top->fname);
	}
	keep_decl_for_w2c = onedown->state || need_decls;
      } else {
	//new file
...
//a list of pragmas, all with WN_PRAGMA_GENERIC as its id
vector <WN*> global_pragmas;

/* Add a #pragma <name> to the whirl node so it 
 * can be passed along to the translated code
 * The pragma is represented as a COMMENT node
 * so that it does not affect the translator
 * 
 */

void WFE_Add_Pragma(const char * name) {

  if (keep_decl_for_w2c) {
    if (!WFE_Stmt_Empty()) {
      WN* wn = WN_CreateComment((char*) name);
      WFE_Stmt_Append(wn, Get_Srcpos());
    } else {
      //global scope pragmas, store in a list until we hit the next function declaration
      ST *pragma_st = New_ST(CURRENT_SYMTAB);
      ST_Init(pragma_st,
	      Str_To_Index (Save_Str(name), Current_Strtab),
	      CLASS_NAME,
	      SCLASS_UNKNOWN,
	      EXPORT_LOCAL,
	      (TY_IDX) 0);
      global_pragmas.push_back(WN_CreatePragma(WN_PRAGMA_GENERIC, pragma_st, 0, 0));
    }
  }
}

void WFE_Add_Bupc_Pragma(const char * name) {

  WN* wn = NULL;
  if (strcmp(name, "ivdep") == 0) {
    wn = WN_CreatePragma(WN_PRAGMA_IVDEP, (ST*) NULL, 0, 0);
  } else if (strcmp(name, "ivdep_rl") == 0) {
    wn = WN_CreatePragma(WN_PRAGMA_UPC_NO_RL_ALIAS, (ST*) NULL, 0, 0);
  } else if (strcmp(name, "unroll") == 0) {
    const char *depth;
    wn = WN_CreatePragma(WN_PRAGMA_UNROLL, (ST*) NULL, 0, 0);
    if(depth = rindex(name, ' ')) {
      WN_pragma_arg1(wn) = atoi(depth);
    } else {
      //Warning - no value for pragma unroll;
    }
  } else if (strcmp(name, "noopt") == 0) {
    if (WFE_Stmt_Empty()) {
      //noopt pragma should appear only globally before a function
      WFE_Add_Pragma(name);
    } else {
      warning("#pragma bupc noopt should not appear inside a function");
    }
  } else {
    warning("Unrecognized bupc pragma: %s", name);
  }

  if (wn != NULL) {
    if (keep_decl_for_w2c && !WFE_Stmt_Empty()) {
      WFE_Stmt_Append(wn, Get_Srcpos());
    } else {
      warning("bupc pragmas may only appear inside functions");
    }
  }

}

static stack<WN*> last_pragmas;

/** 
 * Handling pragmas associated FOR loops requires 
 * more work, since the translator converts for loop into 
 * while loop, and we want the pragms in the output to 
...
void WFE_Pop_Pragma() {
  ...
  for (WN* last = WFE_Stmt_Last(); last != NULL && WN_operator(last) == OPR_COMMENT; last = WFE_Stmt_Last()) {
    last_pragmas.push(WFE_Stmt_Delete());
  }
}

void WFE_Push_Pragma() {
  
  while (!last_pragmas.empty()) {
    WFE_Stmt_Append(last_pragmas.top(), Get_Srcpos());
    last_pragmas.pop();
  }
}
