...
/* bug 342: attributes which should never generate warnings */
char* external_ignored_attributes[] = {"__nonnull__", 
  "__nothrow__",
...
int  warn_for_attribute (tree name) {
  
  int idx;
  for(idx = 0; idx < sizeof(external_ignored_attributes)/sizeof(char*);
      idx++) {
    if (is_attribute_p (external_ignored_attributes[idx], name)) {
      return 0;
    }
  }
     ...
     tree node, attributes, prefix_attributes;
{
  ...
  for (a = attributes; a; a = TREE_CHAIN (a))
	...
	case A_T_UNION:
	  ...
	  else if (decl != 0 && TREE_CODE (decl) == PARM_DECL //should also check for TYPE_DECL
		   && TREE_CODE (type) == UNION_TYPE
		   && TYPE_MODE (type) == DECL_MODE (TYPE_FIELDS (type)))
	...
	case A_FORMAT:
	  {
		...
		if (!strcmp (p, "printf") || !strcmp (p, "__printf__"))
		  ...
		  {
                    if (warn_for_attribute(format_type_id)) 
     ...
     tree *declspecs, *prefix_attributes;
{
  ...
  for (t = specs_attrs; t; t = next)
      ...
      else
	{
	  ...
	  while (a != NULL_TREE && TREE_CHAIN (a) != NULL_TREE)
	    a = TREE_CHAIN (a);
	}
    }
  ...
  while (attrs)
    {
      ...
      if(warn_for_attribute(TREE_PURPOSE (attrs)))
     ...
     int type_quals;
{
  ...
  if (TREE_CODE (type) == ARRAY_TYPE) {
    ...
    t = build_upc_array_type(t, (type_quals & TYPE_QUAL_SHARED), UPC_TYPE_HAS_THREADS(type), UPC_TYPE_USES_THREADS(type));

    if (elt_shared == 0) {
      //Don't mark is elements as shared 
      //TYPE_SHARED(TREE_TYPE(t)) = 0;
      }
     ...
     tree decl;
{
  ...
  if (type_quals & TYPE_QUAL_RESTRICT)
      ...
      else if (flag_strict_aliasing)
	{
	  ...
	  int quals = TYPE_QUALS(type) | TYPE_QUAL_RESTRICT;
  ...
  if (type_quals & TYPE_QUAL_SHARED)
    {
      ...
      if (TREE_STRICT(decl)) {
	quals |= TYPE_QUAL_STRICT;
      } else if (TREE_RELAXED(decl)) {
	quals |= TYPE_QUAL_RELAXED;
      }
      if (TYPE_QUALS (type) != quals)
	{
	  TREE_TYPE (decl) = build_qualified_type (type, quals);
	}
      TREE_SHARED (decl) = 1;
