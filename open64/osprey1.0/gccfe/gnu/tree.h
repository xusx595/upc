...
struct tree_common
{
  ...
  unsigned lang_flag_0 : 1; /* UPC - mark the node for w2c output */
...
#define STRIP_SIGN_NOPS(EXP) \
  while ((TREE_CODE (EXP) == NOP_EXPR				\
	 ...
	 && (TREE_UNSIGNED (TREE_TYPE (EXP))			\
         ...
         && Can_Strip_NOP(TREE_TYPE(EXP),TREE_TYPE (TREE_OPERAND (EXP, 0)))) \
    (EXP) = TREE_OPERAND (EXP, 0);
...
struct tree_type
{
  ...
  unsigned lang_flag_0 : 1;  /* for upc - set to 1 means that the type declaration
				needs to be marked for w2c output */
  unsigned lang_flag_1 : 1;
  unsigned lang_flag_2 : 1;
  unsigned lang_flag_3 : 1;
  unsigned lang_flag_4 : 1;
...
struct tree_decl
{
  ...
  unsigned lang_flag_0 : 1; /* UPC - mark the decl for w2c output */
  unsigned lang_flag_1 : 1;
  unsigned lang_flag_2 : 1;
  unsigned lang_flag_3 : 1;
  unsigned lang_flag_4 : 1;
...
extern int Can_Strip_NOP(tree t1, tree t2);
