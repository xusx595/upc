  ...
  switch (code)
	  ...
	  || TREE_CODE (TREE_OPERAND (t, 0)) == CONVERT_EXPR)
	{
	  ...
	  if ((((inter_int || inter_ptr) && (inside_int || inside_ptr))
	      ...
	      && ! final_ptr && Can_Strip_NOP(inter_type, inside_type))
      ...
      else if (! FLOAT_TYPE_P (type))
	{
	  ...
	  // expressions like &s->a[i] get the constant folded away form the 
	  // base pointer. If this happens, for UPC can't adjust offsets in the backend. 
	  // psearch.upc, bug380
	  if( TREE_CODE(arg0) == PLUS_EXPR) {
	    tree op0 = TREE_OPERAND(arg0, 0);
	    tree op1 = TREE_OPERAND(arg0, 1);
	    if(TREE_CODE(op0) == NOP_EXPR  && TREE_CODE(op1) == INTEGER_CST && 
	       TREE_CODE(TREE_TYPE(op0)) == POINTER_TYPE &&
	       TREE_CODE(TREE_TYPE(TREE_TYPE(op0))) == ARRAY_TYPE &&
	       TREE_CODE(TREE_OPERAND(op0,0)) == NON_LVALUE_EXPR) {
	       
	      op0 = TREE_OPERAND(op0, 0);
	      if(TREE_CODE(TREE_TYPE(op0)) == POINTER_TYPE && TREE_CODE(TREE_TYPE(TREE_TYPE(op0)))) {
		return expr;
	      }
	    }
      ...
      else
	{
	  ...
	  if (! wins && real_twop (arg1) && global_bindings_p () == 0
	      && ! contains_placeholder_p (arg0))
	    {
	      /* WEI: Do not use save exprs for shared variables, otherwise 
		 we can get in trouble as the pointer_to_shared won't get initialized
	      */
	...
	{
	  return t;
	  /* Disable this optimization for UPC code
