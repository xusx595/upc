...
int is_ia32 = 0;
unsigned int pshared_size = 0, shared_size = 0;
unsigned long max_bsize = 0; 
int keep_decl_for_w2c = 0; 

unsigned base_ty_size[_LAST][2]; 
...
#endif /* SGI_MONGOOSE */
{
  ...
  /* WEI: some compilers do not put the source file as the first #line,
   * and a file that gets included may become the first #line instead.
   * So, always use the input name */
...
#endif /* SGI_MONGOOSE */
{
  ...
  if (compiling_upc) {
    //turn off all gcc front end optimizations,
    //which are untested and usually too low level to be converted back to C
    optimize = 0;
    optimize_size = 0;
  }
