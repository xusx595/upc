     ...
     int different_binding_level;
{
      ...
      DECL_IN_SYSTEM_HEADER (newdecl) = 1;
	   ...
	   && ! (TREE_CODE (olddecl) == FUNCTION_DECL
		 ...
		 && comptypes (TREE_TYPE (oldtype),
			       TREE_TYPE (newtype))
		 && TYPE_ARG_TYPES (newtype) == 0)
	   //bug404 - array types that are declared first incomplete can be later
	   //refined to contain a dimension 
	   // NOTE - this test might be too generic and maybe we should test specifically
	   //for ARRAY_TYPE
	   && ! (TREE_CODE(olddecl) == VAR_DECL &&  
		 comptypes(TREE_TYPE (oldtype),TREE_TYPE (newtype)) && DECL_EXTERNAL(olddecl))
	   )
    {
  ...
  if (types_match)
	...
	{
          /* This (and the use of blocksz below) is required because the
             block size is not preserved by common_type().
             The code to do so exists, but causes other problems. */
          tree blocksz = NULL_TREE;
          tree rettype = TREE_TYPE (newtype);
          if ((rettype != NULL_TREE) &&
              (TREE_CODE(rettype) == POINTER_TYPE) &&
              TREE_SHARED(TREE_TYPE(rettype)))
            blocksz = TYPE_BLOCK_SIZE(TREE_TYPE(rettype));
	  ...
	  if (blocksz != NULL_TREE)
            TYPE_BLOCK_SIZE(TREE_TYPE(TREE_TYPE(TREE_TYPE(newdecl)))) = blocksz;
	}
...
init_decl_processing ()
{
  ...
  if (compiling_upc) {
    if (base_ty_size[_SIZE_TY][0] == base_ty_size[_INT][0]) {
  ...
  if (compiling_upc) {
    /* Set the size of ptrdiff_t for the platform */
    if (base_ty_size[_PTRDIFF][0] == base_ty_size[_INT][0]) {
  ...
  if (compiling_upc) {
    star_layout_node = build_int_2 (~0L, ~0L);
  }
     ...
     tree asmspec_tree;
{
  ...
  if (compiling_upc && asmspec_tree != NULL) {
    if (keep_decl_for_w2c) {
      error_with_decl(decl, "asm is currently not supported by the Berkeley UPC Compiler");
    }
    asmspec_tree = NULL;
  }
...
/*       } */
  ...
  if (init != NULL_TREE
      ...
      && DECL_INITIAL (decl) != error_mark_node)
    {
      ...
      dt = TREE_TYPE(decl);
      it = TREE_TYPE(init);
      if(TREE_CODE(dt) == POINTER_TYPE && TYPE_SHARED(TREE_TYPE(dt)) &&
	 TREE_CODE(it) == POINTER_TYPE && !TYPE_SHARED(TREE_TYPE(it)) &&
	 (TREE_CODE(init) != INTEGER_CST || TREE_INT_CST_LOW(init) != 0)) {
	error_with_decl (decl, "UPC does not allow assignments between local pointers and pointers to shared.\n");
     ...
     int initialized;
{
  ...
  for (spec = declspecs; spec; spec = TREE_CHAIN (spec))
	...
	for (i = (int) RID_FIRST_MODIFIER; i < (int) RID_MAX; i++)
	  {
		...
		else if (compiling_upc && ((i == RID_CONST) || (i == RID_VOLATILE) ||
			                   (i == RID_STRICT) || (i == RID_RELAXED)))
		  (void)0; // avoid the following warning
      ...
      if (TREE_CODE (id) == TREE_LIST) /* a blocking factor, keep it */
	{
	  ...
	  layout_specifier = id;
	  /* for int shared [] the keyword for shared is after 
	     the block in the list */
	  if(TREE_CHAIN(id) && TREE_VALUE(TREE_CHAIN(id)) == ridpointers[(int)RID_SHARED]) {
	     specbits |= 1 << (int)RID_SHARED;
	     layout_specifier = TREE_VALUE(id);
	     //TREE_CHAIN(id) = TREE_CHAIN(TREE_CHAIN(id));
	  }
  ...
  if (constp > 1 && !compiling_upc)
    pedwarn ("duplicate `const'");
  if (restrictp > 1)
    pedwarn ("duplicate `restrict'");
  if (volatilep > 1 && !compiling_upc)
    pedwarn ("duplicate `volatile'");
  if (sharedp > 1) // UPC 1.3 spec WG is still undecided (spec issue #68)
    pedwarn ("duplicate `shared'");
  if (strictp > 1 && !compiling_upc)
    pedwarn ("duplicate `strict'");
  if (relaxedp > 1 && !compiling_upc)
  ...
  if (!declarator || TREE_CODE(declarator) == IDENTIFIER_NODE) {
	...
	error ("UPC layout specifier requires `shared' type qualifier"
      ...
      if ((specbits & 1 << (int) RID_TYPEDEF) && TREE_VALUE(layout_specifier) == star_layout_node)
        error("automatic block size [*] cannot be applied to a typedef");
      layout_specifier = 0;
    }
  }
  ...
  while (declarator && TREE_CODE (declarator) != IDENTIFIER_NODE)
	...
	if (sharedp) {
	  if (TREE_CODE (declarator) == INDIRECT_REF &&
	      layout_specifier && TREE_VALUE(layout_specifier) == star_layout_node)
            error("automatic block size [*] cannot be applied to pointer types");
	  else
	    type = set_upc_blocksize (type, layout_specifier);
	}
	else
	  ...
	  */
	  ...
	  if (sharedp && (declarator == NULL || TREE_CODE(declarator) != INDIRECT_REF))
	      ...
	      if (n_thread_refs)
		{
		  ...
		  if (threads_found || n_thread_refs > 1)
		    error ("In the dynamic translation environment, THREADS may not be referenced more than once"
	  ...
	  if (size)
	      ...
	      else
		{
	      ...
	      */
	      if (sharedp && threads_int != 0 && COMPLETE_TYPE_P(type)) {
		tree maxsize = TYPE_MAX_VALUE(sizetype);
		tree unit_size = fold (build(TRUNC_DIV_EXPR, long_long_unsigned_type_node, TYPE_SIZE(type), build_int_2(BITS_PER_UNIT,0)));
		tree ty_size = fold (build (MULT_EXPR, long_long_unsigned_type_node, size, unit_size));
		//debug_tree(ty_size);
		if (INT_CST_LT_UNSIGNED(maxsize, ty_size)) {
		 ...
		 Handling this case seems like an unnecessary complication.  */
	      // Also error if size overflowed -PHH
	      if (TREE_OVERFLOW (itype) || TREE_CONSTANT_OVERFLOW (size))
		{
		...
		itype = variable_size (itype);
	      ...
	      if (div_by_thread) {
		/* We must not set HAS_THREADS in the "base" itype.
		 * However, variants are otherwise unused for index types,
		 * providing a nice convenient way to handle this.
		 */
		tree tmp = TYPE_NEXT_VARIANT (itype);
		if (!tmp) {
		  tmp = copy_node(itype);
		  UPC_TYPE_HAS_THREADS(tmp) = 1;
		  TYPE_NEXT_VARIANT (itype) = tmp;
		}
		itype = tmp;
	      }
	    }
	  ...
	  if (!COMPLETE_TYPE_P (type)
	      /* Avoid warnings for pointers to array types.  */
	      && TREE_CODE (declarator) != INDIRECT_REF
	  ...
	  if (compiling_upc && sharedp)
	    {
	      ...
	      type = build_upc_array_type(type, 1, has_threads, threads_found);
	    }
  ...
  /* Did [*] get applied indirectly to a pointer type? */

  if ((TREE_CODE (type) == POINTER_TYPE) &&
      (TREE_SHARED (type) || TREE_SHARED (TREE_TYPE (type)))) {
    tree t;
    for (t=type; t; t=TREE_TYPE(t)) {
      if (TYPE_BLOCK_SIZE(t) == star_layout_node)
        error("automatic block size [*] cannot be applied to pointer types");
    }
  }
...

  {
	...
	if (TREE_CODE (type) == ARRAY_TYPE && type_quals)
	  {
	    ...
	    type = build_upc_array_type (type, (type_quals & TYPE_QUAL_SHARED), thread_dim, threads_found);
    ...
    //Fix bug605
    //Don't set type qualifiers if there's an error on the type
...
#ifdef SGI_MONGOOSE /* WHIRL will force integer type */
  if (! int_fits_type_p (value, integer_type_node))
    {
      value = convert (integer_type_node, value);
      TREE_OVERFLOW (value) = 0;
      TREE_CONSTANT_OVERFLOW (value) = 0;
      warning ("large enumerator value implicitly truncated to range of `int'");
    }
#else
...
store_parm_decls ()
{
  ...
  if (specparms != 0 && TREE_CODE (specparms) != TREE_LIST)
      ...
      if (parmdecls != 0)
	{
      ...
      /* For functions that are prototyped right before the definition,
	 there's a mismatch in the arg types versus the function type
	 This causes problems whenever  one of the args is a  pointer to
	 shared. 
	 The correct type is in the parm_decl list. Copy the correct type
	 to the function type.

	 This is a bad hack. The presence of bad types inside the function type
	 indicates a problem with the parsing.
      */
      {
	tree atype, anext;
	tree ftype, fnext;
	fnext = TYPE_ARG_TYPES(TREE_TYPE(fndecl));
	anext = specparms;
	while(anext && fnext) {
	  TREE_VALUE(fnext) = TREE_TYPE(anext);
	  fnext = TREE_CHAIN(fnext);
	  anext = TREE_CHAIN(anext);
	}
     ...
     tree t;
{
  ...
  else if (TYPE_P (t) && TREE_SHARED (t))
    ggc_mark (TYPE_BLOCK_SIZE (t));
}

// Taken from gcc 3.2.2
