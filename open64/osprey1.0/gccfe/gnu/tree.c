...
tree
build_upc_array_type (type, is_shared, has_threads, uses_threads)
     tree type;
     int is_shared, has_threads, uses_threads;
{
  register tree t;
  int type_quals;

  /* ensure only 0 or 1 since type uses 1-bit fields */
  is_shared = !!is_shared;
  has_threads = !!has_threads;
  uses_threads = !!uses_threads;
  
  type_quals = (TYPE_QUALS(type) & ~TYPE_QUAL_SHARED) | (TYPE_QUAL_SHARED * is_shared);

  /* Search the chain of variants, as in build_qualified_type.
   * However, we start the search with 'type', not the main variant
   * and also insert the new variant (if any) after 'type'.
   * This ordering ensures that the "purest" variant occurs first.
   */
  for (t = TYPE_MAIN_VARIANT (type); t; t = TYPE_NEXT_VARIANT (t))
    if ((TYPE_QUALS (t) == type_quals) &&
	(TYPE_NAME (t) == TYPE_NAME (type)) &&
	(TREE_TYPE (t) == TREE_TYPE(type)) &&
	(TYPE_DOMAIN (t) == TYPE_DOMAIN(type)) &&
	(TYPE_SHARED (t) == is_shared) &&
	(UPC_TYPE_HAS_THREADS(t) == has_threads) &&
	(UPC_TYPE_USES_THREADS(t) == uses_threads))
	     return t;

  /* We need a new one but want it after 'type', rather than after the main variant.
     So, we clone logic from build_type_copy() */
  { register struct obstack *ambient_obstack = current_obstack;
     ...
     int signed_char;
{
  ...
  if (compiling_upc) {
    short_integer_type_node = make_signed_type (base_ty_size[_SHORT][0] * 8);
    short_unsigned_type_node = make_unsigned_type (base_ty_size[_SHORT][0] * 8);
    if (base_ty_size[_SHORT][1] != 0) {
      TYPE_ALIGN(short_integer_type_node) = base_ty_size[_SHORT][1] * 8;
      TYPE_ALIGN(short_unsigned_type_node) = base_ty_size[_SHORT][1] * 8;
    }      
    integer_type_node = make_signed_type (base_ty_size[_INT][0] * 8);
    unsigned_type_node = make_unsigned_type (base_ty_size[_INT][0]* 8);
    if (base_ty_size[_INT][1] != 0) {
      TYPE_ALIGN(integer_type_node) = base_ty_size[_INT][1] * 8;
      TYPE_ALIGN(unsigned_type_node) = base_ty_size[_INT][1] * 8;
    }
    long_integer_type_node = make_signed_type (base_ty_size[_LONG][0] * 8);
    long_unsigned_type_node = make_unsigned_type (base_ty_size[_LONG][0] * 8);
    if (base_ty_size[_LONG][1] != 0) {
      TYPE_ALIGN(long_integer_type_node) = base_ty_size[_LONG][1] * 8;
      TYPE_ALIGN(long_unsigned_type_node) = base_ty_size[_LONG][1] * 8;
    }    

    long_long_integer_type_node = make_signed_type (base_ty_size[_LONG_LONG][0] * 8);
    long_long_unsigned_type_node = make_unsigned_type (base_ty_size[_LONG_LONG][0] * 8);
    if (base_ty_size[_LONG_LONG][1] != 0) {
      TYPE_ALIGN(long_long_integer_type_node) = base_ty_size[_LONG_LONG][1] * 8;
      TYPE_ALIGN(long_long_unsigned_type_node) = base_ty_size[_LONG_LONG][1] * 8;
    }
  } else {
     ...
     int short_double;
{
  ...
  if (compiling_upc && base_ty_size[_FLOAT][1] != 0) {
    TYPE_ALIGN(float_type_node) = base_ty_size[_FLOAT][1] * 8;
  }
  ...
  if (compiling_upc && base_ty_size[_DOUBLE][1] != 0) {
    TYPE_ALIGN(double_type_node) = base_ty_size[_DOUBLE][1] * 8;
  }

  long_double_type_node = make_node (REAL_TYPE);
#if 1 // Force long double to double
  TYPE_PRECISION (long_double_type_node) = DOUBLE_TYPE_SIZE;
  
  layout_type (long_double_type_node);

  if (compiling_upc && base_ty_size[_DOUBLE][1] != 0) {
    TYPE_ALIGN(long_double_type_node) = base_ty_size[_DOUBLE][1] * 8;
  }
#else
  TYPE_PRECISION (long_double_type_node) = base_ty_size[_LONGDOUBLE][0] * 8;

  layout_type (long_double_type_node);

  if (compiling_upc && base_ty_size[_LONGDOUBLE][1] != 0) {
    TYPE_ALIGN(long_double_type_node) = base_ty_size[_LONGDOUBLE][1] * 8;
  }
#endif
