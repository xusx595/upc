...
void handle_bupc_pragma                 PARAMS((void));
  ...
  if (! compiling_upc)
    {
      ...
      UNSET_RESERVED_WORD ("bupc_assert_type");
    }
  ...
  if (token == IDENTIFIER)
      ...
      if (!strcmp (name, "pragma"))
	{
	  ...
	  if(!strcmp(IDENTIFIER_POINTER(yylval.ttype),"upc")) {
	    ...
	    goto skipline;
	  } else if (!strcmp(IDENTIFIER_POINTER(yylval.ttype), "bupc")) {
	    handle_bupc_pragma(/*finput*/);
	  }
#ifdef SGI_MONGOOSE
	  else {
	    char * prag_token = IDENTIFIER_POINTER(yylval.ttype);
	    if (compiling_upc) {
	      char buf[256];
	      char c;
	      int slen;
	      buf[0] = '\0';
	      //handle generic pragmas
	      //The front end simply passes the string to the backend without doing any checking
	      strcpy(buf, "#pragma ");
	      strncat(buf, prag_token, 245);
	      slen = strlen(buf);
	      for (c = GETC(); c != '\n' && c!= EOF; c = GETC()) {
		buf[slen++] = c;
...
#ifdef SGI_MONGOOSE
      WFE_Set_Line_And_File (lineno, (char*) input_filename);
#endif /* SGI_MONGOOSE */
...
handle_upc_pragma ()
{
  ...
  if(strcmp(cons,"relaxed") && strcmp(cons,"strict") && strcmp(cons, "upc_code"))
    warning ("Unimplemented value for UPC pragma"); 

  if(strcmp(cons,"upc_code") == 0) 
    push_consistency_value(yylval.ttype, 3);  
  else
  ...
  c = GETC();
  while (c != '\n' && c != EOF) {
    if (c == ' ' || c == '\t' || c == '\f' || c == '\r' || c == '\v' || c == '\b') {
      c = GETC(); /* Skip over whitespace */
    } else {
      warning ("ignoring extra tokens following `#pragma upc %s'", cons);
      break; /* so we warn only once */
    }
  }
  UNGETC(c);
...
/*
  Handle berkeley upc specific pragmas, of the form "#pragma bupc ...".
  So far we only have one:  ivdep
*/
void
handle_bupc_pragma () {
