...
extern char* system_header_dirs;
...
int suppress_sys_header_msg(const char *file)
{
  char buf[4096];
  char *end;
  if(system_header_dirs) {
    end = index(file+1, '/');
    while(end) {
      strncpy(buf, file, end-file);
      buf[end-file]=':';
      buf[end-file+1]='\0';
      if(strstr(system_header_dirs, buf))
	return 1;
      end = index(end+1,'/');
    }
  }
     ...
     va_list ap;
{
  ...
  if(suppress_sys_header_msg(file))
    return;
     ...
     va_list ap;
{
  ...
  if(suppress_sys_header_msg(DECL_SOURCE_FILE(decl)))
    return;
  if (count_error (1))
    {
      report_error_function (DECL_SOURCE_FILE (decl));
