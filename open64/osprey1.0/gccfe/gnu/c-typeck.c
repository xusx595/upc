...
extern tree build_comp_ref_shared_type(tree type, BOOL indef);
...
static void warn_for_assignment		PARAMS ((const char *, const char *,
						 tree, int));
static void error_for_assignment	PARAMS ((const char *, const char *,
						 tree, int));
     ...
     tree t1, t2;
{
  ...
  switch (code1)
    ...
    case INTEGER_TYPE:
    case REAL_TYPE:
      /* if any of the types is shared we need the result to 
	 be non-shared */

      if(TYPE_SHARED(t1) && !TYPE_SHARED(t2)) {
	t1 = copy_node(t1);
	TYPE_SHARED(t1) = 0;
      } else if(!TYPE_SHARED(t1) && TYPE_SHARED(t2)) {
	t2 = copy_node(t2);
	TYPE_SHARED(t2) = 0;
      }
	...
	if (TREE_SHARED(pointed_to_1) && TREE_SHARED(pointed_to_2)) {
	  if (TYPE_BLOCK_SIZE(pointed_to_1) != NULL &&
	      TYPE_BLOCK_SIZE(pointed_to_2) != NULL &&
	      TREE_INT_CST_LOW(TYPE_BLOCK_SIZE(pointed_to_1)) == TREE_INT_CST_LOW(TYPE_BLOCK_SIZE(pointed_to_2))) {
	    //Fix bug975.
	    //set the block size of the common type if the two types have the same
	    //block size.  Otherwise it defaults to 1.
	    if (TREE_CODE(pointed_to_1) != VOID_TYPE && 
		TREE_CODE(pointed_to_2) != VOID_TYPE) {
	      //fprintf(stderr, "got here: %d\n", TREE_INT_CST_LOW(TYPE_BLOCK_SIZE(pointed_to_1)));
	      //debug_tree(pointed_to_1);
	      //debug_tree(pointed_to_2);
	      //TYPE_BLOCK_SIZE(TREE_TYPE(t1)) = TYPE_BLOCK_SIZE(pointed_to_1);
	    }
	  }
	}
     ...
     tree type1, type2;
{
  ...
  /* If the types are shared, block sizes must match */
  if(TREE_SHARED(t1) && TREE_SHARED(t2) && TREE_CODE(t1) == TREE_CODE(t2)) {
    tree bs1 = TYPE_BLOCK_SIZE(t1);
    tree bs2 = TYPE_BLOCK_SIZE(t2);
    if((bs1 ? TREE_INT_CST_LOW(bs1) : 1) != (bs2 ? TREE_INT_CST_LOW(bs2) : 1))
      return 0;
  }
...
#define COMP_TYPE_ATTRIBUTES(t1,t2)	1
  ...
  switch (TREE_CODE (t1))
	...
	/* Size 0 (stored as ~0 in TARGET's size_t) always matches */
	tree zerosize = TYPE_MAX_VALUE(sizetype);
	if (tree_int_cst_equal (TYPE_MAX_VALUE (d1), zerosize)
	    || tree_int_cst_equal (TYPE_MAX_VALUE (d2), zerosize))
	  break;
     ...
     tree type;
{
  ...
  if (code == FUNCTION_TYPE)
    {
      if (compiling_upc)
	error ("sizeof applied to a function type");
  ...
  t = size_ll(get_real_size(get_TY_wrapper(type)));
  
  if (UPC_TYPE_USES_THREADS (type) && (code == ARRAY_TYPE))
     ...
     tree exp;
{
  ...
  if (code == ARRAY_TYPE)
      ...
      if (TREE_SHARED (exp) && !TYPE_SHARED (restype))
	{
	  ...
	  /* restype = */ build_pointer_type (restype);
	}
	...
	restype = build_type_copy(restype);
	UPC_TYPE_USES_THREADS(restype) = 1;
	build_pointer_type (restype);
      }
      
      /*
	if(UPC_TYPE_HAS_THREADS(type)) {
	restype = build_type_copy(restype);
	UPC_TYPE_HAS_THREADS(restype) = 1;
	build_pointer_type (restype);
     ...
     tree datum, component;
{
  ...
  if (code == RECORD_TYPE || code == UNION_TYPE)
	...
	if (TREE_SHARED (datum)) {
	  // the record type is shared
	  // need to mark the type of the field as a shared pointer.
	  // and possibly make sure the block size is []
	  TREE_SHARED (ref) = 1;
	  if(!TREE_SHARED(field)) {
	    TREE_TYPE(ref) = build_comp_ref_shared_type(TREE_TYPE(ref), FALSE);
	  }
	}
     ...
     tree array, index;
{
...

  {
	...
	/* Then we have an array that is a field of shared structures.  We have to ensure
	   ...
	   array whose first dimension was handled in pointer_int_sum. */
	{
	  ...
	  tree newint = TREE_TYPE (TREE_TYPE (ar));
	  if(TREE_CODE(newint) == ARRAY_TYPE) {
	    newint = get_inner_array_type(newint);
	    newint = size_in_bytes(newint);
	    size = build(EXACT_DIV_EXPR, sizetype, size, newint);
	    newint = build (MULT_EXPR, sizetype, ind, size);
	  } else {
	    /*newint = build (MULT_EXPR, sizetype, ind, size); */
	    newint = ind;
	  }
  ...
  switch (code)
      ...
      else if (code0 == POINTER_TYPE && code1 == POINTER_TYPE)
	{
	  ...
	  if (comp_target_types (type0, type1))
	    {
	      //comp_target_types ignores qualifiers so we have
	      //to test for sharedness
	      if(TREE_SHARED(TREE_TYPE(type0)) != TREE_SHARED(TREE_TYPE(type1))
                 && (!integer_zerop (op0) || op0 != orig_op0)
                 && (!integer_zerop (op1) || op1 != orig_op1)) {
	         error ("comparison between ptr to shared and ptr to local requires an explicit cast.");
	         return error_mark_node;
	      }

	      //comp_target_types ignores block size too...
	      if(TREE_SHARED(TREE_TYPE(type0)) && TREE_SHARED(TREE_TYPE(type1))) {
		 tree bs0 = TYPE_BLOCK_SIZE(TREE_TYPE(type0));
		 tree bs1 = TYPE_BLOCK_SIZE(TREE_TYPE(type1));
		 if ((bs0 ? TREE_INT_CST_LOW(bs0) : 1) != (bs1 ? TREE_INT_CST_LOW(bs1) : 1)) {
	           error ("comparing pointers to shared with differing block sizes requires an explicit cast.");
	           return error_mark_node;
		 }
              }
	...
	{
	  if ((TREE_SHARED(TREE_TYPE(type0)) && !COMPLETE_TYPE_P(TREE_TYPE(type0))) ||
              (TREE_SHARED(TREE_TYPE(type1)) && !COMPLETE_TYPE_P(TREE_TYPE(type1)))) {
	     error ("UPC forbids comparison of pointer-to-shared with incomplete type.");
	     return error_mark_node;
	  }
	  if (comp_target_types (type0, type1))
	    {
     ...
     register tree ptrop, intop;
{
  ...
  if (TREE_CODE (TREE_TYPE (result_type)) == VOID_TYPE)
    ...
    tree block_sz = TYPE_BLOCK_SIZE(TREE_TYPE(child_type));
    int bsize =  block_sz ? TREE_INT_CST_LOW(block_sz) : 1;
  ...
  if (TYPE_PRECISION (TREE_TYPE (intop)) != TYPE_PRECISION (sizetype)
    ...
    /* This causes a cast to unsigned if the index expression has signed int type 
       (as is usually the case).  
       Since the cast prevents the Cray C compiler from vectorizing array acceses,
       we disable it (overflow shouldn't be a issue since we basically output original C code)
     */
    if (!compiling_upc) {
  ...
  /* WEI: Is this necessary??? Doesn't seem to make sense to cast
   * the intop to be unsigned when it can be negative.
   * Pointer arithmetic do wrap around so it shouldn't affect correctness, 
   * but this introduces an annoying cast in 64-bit mode
   * that defeats cray compiler's vectorization.
   */
	...
	if (TREE_CODE (arg) == COMPONENT_REF)
	  {
	    ...
	    if (TREE_SHARED (arg)) {
	      ...
	      argtype = build_comp_ref_shared_type(argtype, TRUE);
	      tree t =  build1 (ADDR_EXPR, argtype, arg);
	      /* print_tree(stderr, t); */
	      return t;
	    }
	    ...
	    {	
	      ...
	      /* WEI: Fix the offset here for component_refs of
		 structs that contain shared pointers
	      */
	      /* tree real_ofst = (compiling_upc) ?   */
/*  		size_int(get_real_offset(get_TY_wrapper(TREE_TYPE(TREE_OPERAND(arg, 1))), DECL_FIELD_ID(field))) :  */
/*  		byte_position(field); */  
	       tree real_ofst = byte_position(field); 
	      addr = fold (build (PLUS_EXPR, argtype,
				  convert(argtype, addr), 
				  convert (argtype, real_ofst)));
     ...
     tree expr;
{
  ...
  if (TREE_CODE (type) == POINTER_TYPE
      ...
      && /*allow for null*/ TREE_CODE(expr) != INTEGER_CST)
    {
	 ...
	 TREE_SHARED(TREE_TYPE(TREE_OPERAND(expr,0))))
	;
      else {
	error ("UPC does not allow casts from pointer-to-private to pointer-to-shared.");
	return error_mark_node;
      }
    }
  ...
  if (type == TREE_TYPE (value))
	...
	warning ("cast does not match function type");
	  ...
	  /* Allow constant zero.  */
	  && !integer_zerop(value)) {
	error ("UPC does not allow casts from integer to pointer-to-shared.");
	return error_mark_node;
      }
     ...
     enum tree_code modifycode;
{
  ...
  tree rhstype;
  /* Check for assignments to UPC language constants */
  if(compiling_upc && TREE_CODE(lhs) == VAR_DECL) {
    if(DECL_NAME(lhs) && 
       (strcmp(IDENTIFIER_POINTER (DECL_NAME (lhs)), "MYTHREAD") == 0 ||
	strcmp(IDENTIFIER_POINTER (DECL_NAME (lhs)), "THREADS") == 0)) {
      error("Assignment to UPC run-time constant.\n");
      return error_mark_node;
    }
  }
  ...
  if(TREE_CODE(lhstype) == POINTER_TYPE && TYPE_SHARED(TREE_TYPE(lhstype)) &&
     TREE_CODE(rhstype) == POINTER_TYPE && !TYPE_SHARED(TREE_TYPE(rhstype)) &&
     (TREE_CODE(rhs) != INTEGER_CST || TREE_INT_CST_LOW(rhs) != 0)) {
    //for shared structures we artificially mark the type of the field as 
    //being shared in order to assist code generation further down
    //the FIELD_DECL contains the correct type
    if(TREE_CODE(lhs) == COMPONENT_REF && 
       !TREE_SHARED(TREE_TYPE(TREE_TYPE(TREE_OPERAND(lhs,1)))))
       ;
     else {	
        error("UPC  does not allow assignments between local pointers and pointers to shared.\n");
        return error_mark_node; 
     }
  }
...
static tree find_addr_expr(tree exp) {
  ...
  case PLUS_EXPR: {
    tree kid = find_addr_expr(TREE_OPERAND(exp, 0));
    return kid != NULL ? kid : find_addr_expr(TREE_OPERAND(exp, 1));
  }
    /* Any other cases ??? */
  default:
    return NULL;
     ...
     int parmnum;
{
  ...
  /* Conversions among pointers */
	   ...
	   && (coder == POINTER_TYPE || coder == REFERENCE_TYPE))
    {
      ...
      if (TYPE_SHARED (ttl) && !TYPE_SHARED (ttr) /* && TREE_CODE(ttl) == POINTER_TYPE */)
	...
	if (TREE_CODE(rhs) != INTEGER_CST || TREE_INT_CST_LOW(rhs) != 0) {
	  ...
	  if(TREE_CODE(rhs) == PLUS_EXPR) 
	    warn_for_assignment ("%s makes pointer-to-shared from pointer-to-private without a cast",
				 errtype, funname, parmnum);
	  else  {
	    error_for_assignment("%s makes pointer-to-shared from pointer-to-private",
				 errtype, funname, parmnum);
	    return error_mark_node;
	  }
	}
      if (!TYPE_SHARED (ttl) && TYPE_SHARED (ttr) /* && TREE_CODE(ttr) == POINTER_TYPE */ )
	warn_for_assignment (
			     "%s makes pointer-to-private from pointer-to-shared without a cast",
			     errtype, funname, parmnum);
      
      if(TYPE_SHARED(ttl) && TYPE_SHARED(ttr)) {
	tree bl = TYPE_BLOCK_SIZE(ttl);
	tree br = TYPE_BLOCK_SIZE(ttr);
	if (TREE_CODE(ttr) == VOID_TYPE || TREE_CODE(ttl) == VOID_TYPE) {
	  ; // no warnings for casting from/to shared void *)
	} else {
	  if(!bl) bl = integer_one_node; // default block size 1

	  if(!br) { // default could be 1 or 0
	     tree addr = find_addr_expr(rhs);
	     if (addr && !(TREE_CODE(addr) == PARM_DECL || TREE_CODE(addr) == VAR_DECL)
		 && TREE_CODE(TREE_OPERAND(addr,0)) == COMPONENT_REF) {
		br = integer_zero_node; // address of field in shared struct
	     } else {
		br = integer_one_node;
	     }
	  }
  ...
  else if (codel == POINTER_TYPE && coder == INTEGER_TYPE)
    {
	 ...
	 a cast to integer type.  */
	  ...
	  ! (TREE_CODE (rhs) == NOP_EXPR
	     ...
	     && integer_zerop (TREE_OPERAND (rhs, 0))))
	{
	  if (TYPE_SHARED (TREE_TYPE (type))) { /* Bug 2981 */
	    error_for_assignment("%s makes pointer-to-shared from integer",
				 errtype, funname, parmnum);
	    return null_pointer_node; /* prevents down-stream errors */
	  }
...
static char *
opname_for_parm (function, argnum)
     tree function;
     int argnum;
{
      tree selector = maybe_building_objc_message_expr ();
      ...
      if (function)
	{
	  ...
	  new_opname = (char *) malloc (IDENTIFIER_LENGTH (function)
      ...
      else
	{
	  ...
	  new_opname = (char *) malloc (strlen (argnofun) + 1 + 25 /*%d*/ + 1);
     ...
     int argnum;
{
  if (opname == 0)
    {
      char * new_opname = opname_for_parm(function, argnum);
      pedwarn (msgid, new_opname);
      free (new_opname);
    }
  else
    {
...
extern void WFE_Finish_Aggregate_Init();
void
finish_init ()
{
     ...
     int line;
{
  ...
  if (compiling_upc) {
    if (keep_decl_for_w2c) {
      error("asm is currently not supported in the Berkeley UPC Compiler");
    }
    return;
  }
...
extern void WFE_Null_Return(); //get rid of a warning
  ...
  if (!retval)
    {
      ...
      //bug851: C99 requires this to be an error
...

/* Check expression EXP as argment to upc_{barrier,notify,wait)
   Return converted EXP if it is valid, an error node otherwise.  */

tree
upc_expand_sync_arg(syncop, exp)
     int syncop;
     tree exp;
{
  char *opname;
  switch(syncop) {
  case 1:
    opname = "call to upc_notify";
    break;
  case 2:
    opname = "call to upc_wait";
    break;
  case 3:
    opname = "call to upc_barrier";
    break;
  default:
    return error_mark_node;
  }
  return convert_for_assignment (integer_type_node, exp, _(opname), NULL_TREE, NULL_TREE, 0);
}
