...
/* WEI: Record that the array dimension contains THREADS 
   In static environment, this is marked on the 
   TYPE_DOMAIN of an array type, to indicate that its size is actually per-thread
   (and thus need to be multiplied by THREADS when creating TY)
*/
...
tree build_upc_array_type			PARAMS((tree, int, int,int));

/* in c-parse.in */
...
extern tree upc_expand_sync_arg                 PARAMS ((int, tree));
