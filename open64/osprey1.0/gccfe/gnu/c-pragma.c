...
extern void WFE_Patch_Inner_Scope_Names PARAMS((int));
...
{
  struct consistency_value *new;
  if(globalp == 3) {
    WFE_Set_Consistency (IDENTIFIER_POINTER (exp), START_CONSISTENCY_SCOPE);
    return;
  }
  new  = (struct consistency_value *) malloc (sizeof (struct consistency_value));
