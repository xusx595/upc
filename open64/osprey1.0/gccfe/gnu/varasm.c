...
static int _assemble_integer_unsigned = 0;

     ...
     int force;
{
  ...
  //we need to keep the sign of the constant, otherwise 
  //there may be failures due to implicit signed/unsigned conversion (see bug914).
  WFE_Add_Aggregate_Init_Integer (ival, size, _assemble_integer_unsigned);
  _assemble_integer_unsigned = 0;
  return 1;
#else
...
#endif /* SGI_MONGOOSE */
      _assemble_integer_unsigned = TREE_UNSIGNED(TREE_TYPE(exp));
