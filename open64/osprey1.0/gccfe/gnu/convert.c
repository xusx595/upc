     ...
     tree type, expr;
{
  ...
  switch (TREE_CODE (TREE_TYPE (expr)))
    ...
    case CHAR_TYPE:
      {
	int ptr_size = POINTER_SIZE;
	if (compiling_upc && is_ia32) {
	  /* bug 1771: Prevent the front end from generating superfluous casts 
	   * on 32-bit platforms (gnu front end assumes pointers are 64-bit since 
	   * it targets IA64)
	   */
	  ptr_size = 32;
	}

	if (TYPE_PRECISION (TREE_TYPE (expr)) == ptr_size)
	  return build1 (CONVERT_EXPR, type, expr);
	
	return
	  convert_to_pointer (type,
			      convert (type_for_size (ptr_size, 0), expr));
      }
