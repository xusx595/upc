...
static char *rcs_id = "$Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/driver/main.c $ $Revision: 1.6 $";
...
main (int argc, char *argv[])
{
	...
	//for UPC, never run beyond be since we do source-to-source
 ...
 */
	...
	if (Use_feedback) {
	   ...
	   if (!(stat(fb_file, &stat_buf) != 0 && errno == ENOENT))
	     //#ifdef linux
		fb_file_mod_time = stat_buf.st_mtim;
	   /*
#else
	   ...
	   */
		...
		fb_file_exists = FALSE;
	     ...
	     //#ifdef linux
		count_file_mod_time = stat_buf.st_mtim;
	   /*
#else
