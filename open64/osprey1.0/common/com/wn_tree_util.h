...
#include "cxx_memory.h"                // for CXX_NEW etc
  ...
  /* g++ 3.1 and aboverequires "typename" to appear according to C++ spec,
...
public:
  ...
  template<WHIRL> friend bool operator==(const self &x, const self & y); // defined below
...
public:
  ...
  TYPENAME WN_TREE_ITER<PRE_ORDER, WHIRL>::self &      operator++()    
    { WN_TREE_next(); return *this;} // pre
  TYPENAME WN_TREE_ITER<PRE_ORDER, WHIRL>::self        operator++(INT) { 
    TYPENAME WN_TREE_ITER<PRE_ORDER, WHIRL>::self tmp = *this; 
    WN_TREE_next(); 
    return tmp;}
...
public:
  ...
  TYPENAME WN_TREE_ITER<POST_ORDER, WHIRL>::self & operator++() 
    { WN_TREE_next(); return *this;} // pre
  TYPENAME WN_TREE_ITER<POST_ORDER, WHIRL>::self operator++(INT) { 
    TYPENAME WN_TREE_ITER<POST_ORDER, WHIRL>::self tmp = *this; 
    WN_TREE_next(); 
    return tmp;
  }
