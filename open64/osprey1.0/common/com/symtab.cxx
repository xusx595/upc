...
// 	/* COSTIN TOFIX */
	if (TY_kind(fld_ty) == KIND_STRUCT && !is_upcr_ptr(fld_ty) &&
	    ...
	    fld = FLD_get_to_field(fld_ty, field_id, cur_field_id);
	    if (cur_field_id == field_id)
		return fld;
	}
	/* COSTIN TOFIX*/
...
// this function is the simplified version of the skip_till_next_field in the ty2c.cxx
static FLD_HANDLE
...
// this function is identical to the one in the ty2c.cxx
BOOL FLD_Is_Bitfield(FLD_HANDLE fld, FLD_HANDLE next_fld,
          INT64 max_size)
{
...
FLD_HANDLE
FLD_get_the_last_array_field(TY_IDX struct_ty, INT64  offset, INT32 struct_type, FLD_list &fld_stack) 
{
	/* Try to find a field at any level of nesting, which is at
	 * the given offset from the base of the struct_ty.
	 */
	TY_IDX        this_ty;
	FLD_HANDLE    this_fld, next_fld, found_fld;
	 ...
	 * that FLD_ofst(this_field) <= offset and next_fld==NULL
	 * or FLD_ofst(next_field) > offset.
	 */
	 ...
	 * possible.
	 */
	while (!found_ty && !this_fld.Is_Null () && FLD_ofst(this_fld) <= offset)
	{
		this_ty = FLD_type(this_fld);
		INT64 this_fld_offset = FLD_ofst(this_fld);
		BOOL this_fld_is_ARRAY = (TY_kind(this_ty) == KIND_ARRAY);
		BOOL this_fld_is_STRUCT = (TY_kind(this_ty) == KIND_STRUCT);
		BOOL this_fld_is_ARRAY_of_STRUCT = (this_fld_is_ARRAY) ? 
					(TY_kind(Get_Inner_Array_Type(this_ty)) == KIND_STRUCT) : FALSE;
		fld_stack.push_back(this_fld);

		if ( (!TY_is_union(struct_ty) &&
				FLD_Is_Bitfield(this_fld, FLD_next(this_fld), struct_size - this_fld_offset)))
		{
		...
		else if ( this_fld_is_ARRAY_of_STRUCT ) {
			if ( offset == this_fld_offset ) {
				/* Array field look ahead: if it is an array of struct, check if the first 
				 * field of this struct is also an array 
				 * struct s2 {         // outer-most struct
				 *   struct s1 {       // the current this_fld points to this struct
				 *     float aaa[4];   // we want to find this array
				 *   } bbb[8];
				 * } foo
				 *
				 * For "foo.bbb[i].aaa[j]", array_ety = struct s1, find aaa inside struct s1
				 */
				FLD_list temp_fld_stack;
				TY_IDX array_ty = Get_Inner_Array_Type(this_ty);
				FLD_HANDLE temp_fld = FLD_get_the_last_array_field(array_ty, 0, struct_type, temp_fld_stack);
				if ( !temp_fld.Is_Null() ) {
					found_fld = temp_fld;
					// Vector append:
					fld_stack.insert(fld_stack.end(),
							 temp_fld_stack.begin(),
							 temp_fld_stack.end());
				}
				else {
					found_fld = this_fld;
			...
			else {	/* offset > this_fld_offset */
				offset -= this_fld_offset;
				TY_IDX array_ety = Get_Inner_Array_Type(this_ty);
				if ( struct_type == 1 ) { 
					INT64 inner_struct_size = TY_size(array_ety);
					INT64 index = offset / inner_struct_size;
					offset -= index * inner_struct_size;
				}
				found_fld = FLD_get_the_last_array_field(array_ety, offset, struct_type, fld_stack);
				if ( !found_fld.Is_Null() ) {
					found_ty = TRUE;
				}
			}
		}
		else if ( this_fld_is_STRUCT ) {
			/* 
			 * Always get inside the struct to find the nested field that matches.  
			 */
			offset -= this_fld_offset;
			found_fld = FLD_get_the_last_array_field(this_ty, offset, struct_type, fld_stack);
			if ( !found_fld.Is_Null() ) {
				found_ty = TRUE;
			}
		}
		else if ( this_fld_is_ARRAY ) {
			if ( offset == this_fld_offset ) {
				/* perfect match */
				found_fld = this_fld;
				return found_fld;
...
public:
  TY_IDX idx;
  TY_IDX private_ty;
  Idx_Pair(TY_IDX i, TY_IDX p) {idx = i; private_ty = p;}
  Idx_Pair() : idx(0),private_ty(0) {}
}; 

struct Pair_Cmp {
  bool operator() (const Idx_Pair* t1, const Idx_Pair* t2) const {
    return t1->idx < t2->idx || (t1->idx == t2->idx && t1->private_ty == t2->private_ty);
  }
};
...
TY_IDX Make_Shared_Type (TY_IDX ty_idx, int bsize, CONSISTENCY_class consistency) {
  static map<UINT64, TY_IDX> ty_map;
  UINT64 key = ((UINT64)bsize << 32) + (ty_idx << 2) + consistency;
  ...
  if (TY_is_shared(ty_idx)) {
    ...
    if (ty_bsize == bsize && 
	((TY_is_strict(ty_idx) && consistency == STRICT_CONSISTENCY) ||
	 (TY_is_relaxed(ty_idx) && consistency == RELAXED_CONSISTENCY) ||
	 (!TY_is_strict(ty_idx) && !TY_is_relaxed(ty_idx) && consistency == NO_CONSISTENCY))
	) {
      ty_map[key] = ty_idx;
      return ty_idx;
    }
  ...
  if (res == 0) {
    ...
    Set_TY_adjusted_size(res, TY_adjusted_size(ty_idx));
    ...
    case KIND_SCALAR:
      ...
      if(TY_adjusted_size(res) == 0) {
	Set_TY_adjusted_size(res, TY_size(res));
	Set_TY_adjusted_size(ty_idx, TY_size(ty_idx));
      }
      break;
    case KIND_POINTER:
      Set_TY_block_size(res, bsize);
    ...
    case KIND_ARRAY:
      ...
      break;
    default:
      Fail_FmtAssertion("Unexpected TY_kind (%d) in Make_Shared_Type()", TY_kind(ty_idx) );
    }

    if (consistency == STRICT_CONSISTENCY) {
      Set_TY_is_strict(res);
    } else if (consistency == RELAXED_CONSISTENCY) {
      Set_TY_is_relaxed(res);
    }
    ty_map[key] = res;
  } else {
    // for structs with forward declarations  (aka lists)
    // need to patch the adjusted size since the shared type is seen before the 
    // non-shared one
    if(TY_kind(res) == KIND_STRUCT && TY_adjusted_size(res) != TY_adjusted_size(ty_idx) 
       && TY_adjusted_size(ty_idx)) 
      Set_TY_adjusted_size(res, TY_adjusted_size(ty_idx));
  }
...
{
    fprintf (f, "\t%6lld\t%6lld  %-8s\t", ofst, adjusted_ofst, &Str_Table[name_idx]);
    Print_TY_IDX_verbose (f, type);
    fprintf (f, "\n\t\t\tfl:0x%04x", flags);
    // Print_TY_IDX_verbose (f, orig_type);
...
TY::Print (FILE *f) const
{
	...
	if (flags & TY_IS_WRITTEN)	fprintf (f, " written");
    }
    fprintf (f, ")");

    fprintf (f, " size %lld adjusted %lld %s: ",
	     size, adjusted_size, (mtype != 0) ? Mtype_Name (mtype) : ""); 
...
ST_ATTR::Print (FILE* f) const
{
	...
	break;
    case ST_ATTR_FN_RET_TY_IDX:
      fprintf (f, "(FN_RET_TY_IDX) 0x%x\n", u.value);
      break;
    case ST_ATTR_FN_ARG_TY_IDX:
      fprintf (f, "(FN_ARG_TY_IDX) 0x%x\n", u.value);
      break;
    default:
		       ...
		       ST_CLASS sym_class, ST_SCLASS storage_class)
{
  ...
  STR_IDX str_idx = Save_Str2i("_bupc_", rootname, Temp_Index++);
