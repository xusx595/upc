...
WN *WN_Simplify_Tree(WN *t, ALIAS_MANAGER *alias_manager)
{
   ...
   if (op == OPC_BLOCK) {
	...
	//wei: don't omit TAS between shared and private types                                     
	if (opr == OPR_TAS) {
	  bool keep_tas = Type_Is_Shared_Ptr(WN_ty(k0)) || TY_kind(WN_ty(t)) == KIND_POINTER;
          //if (Type_Is_Shared_Ptr(WN_ty(t), true) || Type_Is_Shared_Ptr(WN_ty(k0), true)) {
	  if (!keep_tas) {
            r = NULL;
          }
        } else {
   ...
   if (opr == OPR_ILOAD) {
	...
	if(WN_operator(t) == OPR_CVTL)
	  ...
	  if(!Type_Is_Shared_Ptr(WN_ty(t))) {
	    //It's probably a bad idea to remove TAS to local pointers, due casts to local
	    bool keep_tas = Type_Is_Shared_Ptr(WN_ty(k0)) || 
	      (TY_kind(WN_ty(t)) == KIND_POINTER /* && WN_operator(k0) == OPR_ARRAY*/);
	    if (!keep_tas) {
	      r = WN_SimplifyExp1(op, k0);
	    }
	  }
