...
inline UINT64
TY_adjusted_size (const TY& ty)			    { return ty.adjusted_size; }
inline UINT64
TY_adjusted_size (TY_IDX ty_idx) {return TY_adjusted_size (Ty_Table[ty_idx]); }
inline void
Set_TY_adjusted_size (TY& ty, UINT64 size)	{ ty.adjusted_size = size; }
inline void
Set_TY_adjusted_size (TY_IDX tyi, UINT64 size)       {
  Set_TY_adjusted_size(Ty_Table[tyi], size);
}
...
#define UPC_ANON_PREFIX "_bupc_anonymous"
inline BOOL
TY_is_anonymous(TY_IDX tyi) {
  const char prefix[] = UPC_ANON_PREFIX;
  const int len = sizeof(prefix) - 1;
  const char *name = TY_name(Ty_Table[tyi]);
  return name && !strncmp(name, prefix, len);
}
...
inline BOOL
TY_uses_threads (const TY& ty)		{ return ty.flags & TY_USES_THREADS; }
inline void
Set_TY_uses_threads (TY& ty)		{ ty.flags |= TY_USES_THREADS; }
inline void
Clear_TY_uses_threads (TY& ty)		{ ty.flags &= ~TY_USES_THREADS; }
inline BOOL
TY_uses_threads (const TY_IDX tyi)     { return TY_uses_threads(Ty_Table[tyi]); }
inline void
Set_TY_uses_threads (TY_IDX tyi)       { Set_TY_uses_threads(Ty_Table[tyi]); }
inline void
Clear_TY_uses_threads (TY_IDX tyi)     { Clear_TY_uses_threads(Ty_Table[tyi]); }
...
inline UINT64
FLD_adjusted_ofst (FLD_HANDLE fld)		{ return fld.Entry()->adjusted_ofst; }
inline void
Set_FLD_adjusted_ofst (FLD_HANDLE fld, UINT64 ofst) { fld.Entry()->adjusted_ofst = ofst; }

inline TY_IDX 
FLD_orig_type (FLD_HANDLE fld) {return fld.Entry()->orig_type; }
inline void
Set_FLD_orig_type (FLD_HANDLE fld, TY_IDX ty) {fld.Entry()->orig_type = ty; }
