 ...
 * Module: wn_map.c
 * $Author: ciancu $
 * $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/common/com/wn_map.cxx $
 *
 * Revision history:
...
WN_MAP_Do_Create(WN_MAP_TAB *maptab, MEM_POOL *pool, WN_MAP_KIND kind)
{
  ...
  //fprintf(stderr, "creating map: %d %p\n", wn_map, pool);
  return wn_map;
}
...
IPA_WN_MAP_Set(WN_MAP_TAB *maptab, WN_MAP wn_map, WN *wn, void *thing)
{
  ...
  WN_MAP_ID old_id = WN_map_id(wn);
  id = WN_MAP_get_map_id(maptab, category, wn);
  //if (wn_map == 7 && thing != NULL) {
  //    fprintf(stderr, "%p: map = %d, cat = %d, old_id = %d\n", wn, wn_map, category, old_id);
  //}

  if (id >= maptab->_map_size[category][wn_map]) {
    WN_MAP_realloc_array(maptab, category, wn_map, id, sizeof(void*));
    //    if (wn_map == 7) {
    //  fprintf(stderr, "set size of map 7: cat = %d, size = %d\n", category, maptab->_map_size[category][wn_map]);
    //}
  }
