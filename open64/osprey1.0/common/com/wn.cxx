...
Types_Are_Compatible ( TYPE_ID ltype, WN * wn )
{
  ...
  if (Compile_Upc && rtype == MTYPE_B) {
    //allow MTYPE_B to be compatible with integer types
    return WTYPE_base_type(ltype) == INT_TYPE ||
      WTYPE_base_type(ltype) == UINT_TYPE;
  }
...
WN *WN_CreateParm(TYPE_ID rtype, WN *parm_node, TY_IDX ty, UINT32 flag)
{
    ...
    if ( parm_node != NULL ) {
	  ...
	  if (!Types_Are_Compatible(rtype, parm_node)) {
	    //during upc lowering we may get such temporary inconsistencies, 
	    //since a pointer-to-shared is MTYPE_M on 32-bit platform.  
	    //The types would eventually match after the last round of upc lowering
	    extern TY_IDX WN_Get_Ref_TY(WN*); //from upc_wn_util.h
	    TY_IDX ty = WN_Get_Ref_TY(parm_node);
	    if (!Type_Is_Shared_Ptr(ty)) {
	      Is_True(0, ("rtype and parm_node's rtype are different in WN_CreateParm"));
	    }
	  }
       }
