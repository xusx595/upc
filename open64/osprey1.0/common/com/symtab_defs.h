...
struct FLD
{
  ...
  mUINT64 adjusted_ofst;              //field offset after account for the size 
                                        //of ptrs to shared
  TY_IDX  orig_type;                  // original type for ptr-to-shared, used for debug
...
enum ST_ATTR_KIND
{
    ...
    ST_ATTR_FN_RET_TY_IDX       = 3,
    ST_ATTR_FN_ARG_TY_IDX       = 4
...
enum TY_FLAGS
{
    ...
    TY_IS_SHARED        = 0x0800,       // UPC shared type
    TY_IS_STRICT        = 0x1000,
    TY_IS_RELAXED       = 0x2000,
    TY_IS_WRITTEN       = 0x4000,        //make w2c to output the type definition
    TY_USES_THREADS     = 0x8000         //for ARRAY - dimension  multiplied  by THREADS
