 ...
 * Module: targ_const.c
 * $Revision: 1.9 $
 * $Date: 2006/03/16 07:05:50 $
 * $Author: wychen $
 * $Source: bitbucket.org:berkeleylab/upc-translator.git/open64/osprey1.0/common/com/ia64/targ_const.cxx $
 *
 * Revision history:
...
#if (defined(_AIX) && __GNUC__ == 3 && __GNUC_MINOR__ < 4) || defined(sun)
...
#define TPB_SIZE 8
   ...
   switch (TCON_ty(c)) {
    ...
    case MTYPE_F4:
      ...
      g_fmt(r, TCON_R4(c));
      /* add a terminating "." if it's a decimal integer,
       * to make it a valid FP literal 
       * the exponent is added because later compiler stages expect it 
       */
      if (strpbrk(r,".eENI") == NULL) strcat(r,".0e+0");
      if (r[0] == '.') { /* append a leading zero if necessary */
        char tmp[80];
        strcpy(tmp, "0"); strcat(tmp, r); strcpy(r, tmp);
      }
      //printf("g_fmt(%#12.7g)=>%s\n", TCON_R4(c), r);
#if !(defined(FRONT_END_C) || defined(FRONT_END_CPLUSPLUS))
      if (re = strchr(r, 'e'))
	*re = 'd';
#endif /* !(defined(FRONT_END_C) || defined(FRONT_END_CPLUSPLUS)) */
...
#define INFNAN_CHECK 1
  ...
  #define Long short
#else
  #error  No 32-bit integer type.  Define in target_directory/be/Makefile one of the the macros SIZEOF_LONG, SIZEOF_INT or SIZEOF_SHORT,  that identifies  a 32 bit integral type for your target.     
